...
These intentionally invoke the canonical ``radia.ltspice.cli`` module in a
subprocess so exit-code propagation and module initialization are exercised.
"""
...
def test_version():
    ...
    assert r.stdout.strip().startswith("radia-ltspice ")
...
def test_check_text_clean_netlist():
    ...
    from radia.ltspice.cli import check_text
    netlist = (
        "* RC Lowpass\n"
        "V1 in 0 AC 1\n"
        "R1 in out 1k\n"
...
def test_schemdraw_generated_script_preserves_raw_spice_lines(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    from radia.ltspice import netlist_to_schemdraw, schemdraw_to_netlist
    from radia.ltspice.topology import topology_equivalent
...
def test_asc_roundtrip_preserves_unknown_subckt_raw_line():
    from radia.ltspice import asc_to_netlist, netlist_to_asc
    from radia.ltspice.topology import topology_equivalent
...
def test_asc_layout_escapes_dense_source_lc_input_node():
    from radia.ltspice import asc_to_netlist, netlist_to_asc
    from radia.ltspice.topology import topology_equivalent

    netlist = (
        "* Dense source/L/C boost input should remain readable\n"
        "V1 in 0 DC 12\n"
        "L1 in sw 10u\n"
        "S1 sw 0 gate 0 SW\n"
        "D1 sw out D\n"
        "CIN in 0 22u\n"
        "RIN in cin_damp 1\n"
        "CDAMP cin_damp 0 2.2u\n"
        "COUT out 0 47u\n"
        "RLOAD out 0 24\n"
        "VCTRL gate 0 PULSE(0 10 0 20n 20n 2u 5u)\n"
        ".model SW SW(Ron=50m Roff=10Meg Vt=4 Vh=0.5)\n"
        ".model D D(Is=10n Rs=40m)\n"
        ".tran 0 2m\n"
        ".end\n"
    )
    asc = netlist_to_asc(netlist)
    ...
    for line in asc.splitlines():
        match = re.match(r"^SYMBOL\s+\S+\s+(-?\d+)\s+(-?\d+)\s+\S+", line)
        if match:
            current = (int(match.group(1)), int(match.group(2)))
            continue
        match = re.match(r"^SYMATTR\s+InstName\s+(\S+)", line)
        if match and current is not None:
            symbols[match.group(1)] = current
            current = None

    assert {"V1", "L1", "CIN"} <= set(symbols)
    v1_l1_distance = math.hypot(
        symbols["V1"][0] - symbols["L1"][0],
        symbols["V1"][1] - symbols["L1"][1],
    )
    assert v1_l1_distance >= 64
...
def test_info_text_counts_by_class():
    ...
    from radia.ltspice.cli import info_text
    netlist = (
        "V1 in 0 AC 1\n"
        "R1 in m 1k\n"
        "R2 m out 1k\n"
...
def test_mcp_tools_registered():
    ...
    expected = {
        ...
        "second_order_allpass_phase_group_delay_gate",
        "second_order_complex_zero_transfer_gate",
        "balanced_three_phase_delta_load_gate",
    ...
    readme = (
        Path(__file__).parents[2] / "docs" / "ltspice" / "README.md"
    ).read_text(encoding="utf-8")
    assert "Exposes thirty-two tools:" in readme
