...
import radia.beam as beam_module

ngsolve = pytest.importorskip("ngsolve")
pytest.importorskip("netgen.occ")

from netgen.occ import Box, Pnt
from ngsolve import CF, GridFunction, HCurl, HDiv, Mesh, TaskManager, VectorH1, x, y, z
...
def test_curvilinear_s_rk_uses_direct_hcurl_a_and_hdiv_b_map(
    ...
    magnetic_flux_density = GridFunction(
        HDiv(vector_potential.space.mesh, order=4), name="B_earlytimes_test"
    )
    orbit = PlanarDesignOrbit(
        positions=np.array(
            [[0.0, 0.0, 0.3], [0.0, 0.0, 0.5], [0.0, 0.0, 0.7]]
    ...
    with TaskManager():
        magnetic_flux_density.Set(source_magnetic_flux_density, use_simd=False)
        batch = compare_hcurl_a_map_to_hdiv_b_map_rk_batch(
            vector_potential,
            magnetic_flux_density,
            orbit,
            np.vstack((initial, -initial)),
            integrator="DOP853",
            maximum_step_m=2.0e-4,
        )
    comparison = batch.cases[0]

    assert comparison.a_map.field_representation == (
        "hcurl-vector-potential-direct-A"
    )
    assert comparison.b_map.field_representation == "hdiv-B-GridFunction"
    ...
    assert batch.initial_states.shape == (2, 6)
    assert batch.final_state_differences.shape == (2, 6)
    assert batch.maximum_final_state_difference < 2.0e-9
    np.testing.assert_allclose(
        batch.componentwise_maximum_final_state_difference[[0, 1, 2, 3, 5]],
        np.max(
            np.abs(batch.final_state_differences[:, [0, 1, 2, 3, 5]]),
            axis=0,
        ),
    )


def test_a_b_batch_affine_fit_separates_bias_and_transfer_matrix_difference():
    basis_indices = (0, 1, 2, 3, 5)
    amplitudes = np.array([2.0e-4, 3.0e-4, 1.0e-4, 2.0e-4, 2.0e-4])
    states = [np.zeros(6)]
    for index, amplitude in zip(basis_indices, amplitudes, strict=True):
        offset = np.zeros(6)
        offset[index] = amplitude
        states.extend((offset, -offset))
    states = np.vstack(states)
    intercept = np.array([1.0e-7, -2.0e-7, 3.0e-8, -4.0e-8, 5.0e-9])
    jacobian = np.arange(25, dtype=float).reshape(5, 5) * 1.0e-4
    comparable_inputs = states[:, basis_indices]
    comparable_differences = intercept + comparable_inputs @ jacobian.T
    differences = np.full((len(states), 6), np.nan)
    differences[:, basis_indices] = comparable_differences
    batch = HCurlAMapBCoefficientRKBatchComparison(
        initial_states=states,
        cases=tuple(
            SimpleNamespace(final_state_difference=row) for row in differences
        ),
    )

    linearization = batch.fit_affine_route_difference()
    assert linearization.input_indices == (0, 1, 2, 3, 5)
    assert linearization.output_indices == (0, 1, 2, 3, 5)
    assert linearization.rank == 6
    assert linearization.intercept.shape == (5,)
    assert linearization.jacobian.shape == (5, 5)
    np.testing.assert_allclose(linearization.intercept, intercept, atol=1.0e-20)
    np.testing.assert_allclose(linearization.jacobian, jacobian, atol=1.0e-15)
    assert linearization.maximum_intercept == pytest.approx(2.0e-7)
    assert linearization.maximum_jacobian_entry == pytest.approx(2.4e-3)
    assert linearization.maximum_fit_residual < 1.0e-20


def test_curved_sector_hcurl_a_and_b_follow_same_design_orbit():
    radius = 4.0
    rigidity = 3.0
    curvature = 1.0 / radius
    field_magnitude = rigidity * curvature
    length = 0.2
    stations = np.linspace(0.0, length, 21)
    angle = stations / radius
    positions = np.column_stack(
        (
            radius * np.sin(angle),
    ...
    radial_distance = ngsolve.sqrt(x * x + (y - radius) ** 2)
    local_x = radius - radial_distance
    metric = 1.0 - curvature * local_x
    covariant_As = (
        field_magnitude * local_x
        - 0.5 * field_magnitude * curvature * local_x**2
    )
    physical_As = covariant_As / metric
    tangent_x = -(y - radius) / radial_distance
    tangent_y = x / radial_distance
    vector_potential = GridFunction(HCurl(tube.mesh, order=5))
    magnetic_flux_density = GridFunction(HDiv(tube.mesh, order=4))
    with TaskManager():
        vector_potential.Set(
            CF((physical_As * tangent_x, physical_As * tangent_y, 0.0)),
            use_simd=False,
        )
        magnetic_flux_density.Set(
            CF((0.0, 0.0, -field_magnitude)), use_simd=False
        )
    ...
    assert comparison.maximum_final_state_difference < 1.0e-11
    assert comparison.a_map.maximum_raw_Ax_t_m < 1.0e-11
...
def test_earlytimes_boundary_projects_hdiv_mmm_coefficients_to_gridfunctions(
    ...
    with TaskManager():
        ...
        comparison = compare_earlytimes_b_representations(
            maps.vector_potential,
            maps.magnetic_flux_density,
            lambda points: np.column_stack(
                (
                    np.zeros(len(points)),
                    2.4 * np.asarray(points)[:, 0],
                    np.zeros(len(points)),
                )
            ),
            np.array(
    ...
    assert maps.earlytimes_representation == (
    ...
    np.testing.assert_allclose(comparison.hdiv_b_t, comparison.source_b_t, atol=2e-12)
    np.testing.assert_allclose(comparison.curl_a_t, comparison.source_b_t, atol=2e-12)
    assert comparison.maximum_hdiv_minus_source_vector_norm_t < 2.0e-12
    assert comparison.maximum_curl_a_minus_source_vector_norm_t < 2.0e-12
    assert comparison.maximum_hdiv_minus_curl_a_vector_norm_t < 2.0e-12


def test_hdiv_b_rk_rejects_unprojected_b_coefficient_function(
    hcurl_cubic_vector_potential,
):
    vector_potential, _, _ = hcurl_cubic_vector_potential
    orbit = PlanarDesignOrbit(
    ...
    with pytest.raises(TypeError, match="GridFunction on HDiv"):
        compare_hcurl_a_map_to_hdiv_b_map_rk(
            vector_potential,
            CF((0.0, 0.0, 0.0)),
            orbit,
...
def test_hcurl_lie_uses_the_negative_rmf_metric_connection(monkeypatch):
    ...
    polynomial_fit = SimpleNamespace(
        ...
        As_coefficients_t_m=As,
        field_certificate=SimpleNamespace(),
    )
    monkeypatch.setattr(
        beam_module,
        "sample_transverse_vector_potential",
    ...
    monkeypatch.setattr(
        beam_module,
        "fit_transverse_vector_potential_polynomials",
        lambda *_args, **_kwargs: polynomial_fit,
    )
    ...
    result = fourth_order_lie_map_from_hcurl_transverse(
        ...
        [-0.005, 0.0, 0.005],
        maximum_step_m=0.02,
        reference_orbit_tolerance=1.0e-12,
    )
    ...
    np.testing.assert_allclose(
        result.lie_map.hamiltonian_linear,
        0.0,
        rtol=0.0,
        atol=2.0e-16,
    )
...
def test_hcurl_volume_recovers_full_p5_xy_jet_and_lie_gradient(
    ...
    B_map = GridFunction(
        HDiv(hcurl_p5_xy_vector_potential.space.mesh, order=4),
        name="B_lie_rk_certificate",
    )
    B_map.Set(B_coefficient)
    initial = np.array(
        [1.0e-4, 2.0e-4, -8.0e-5, -1.0e-4, 0.0, 1.0e-4]
    )
    with TaskManager():
        a_only_comparison = compare_hcurl_lie_map_to_a_rk(
            hcurl_p5_xy_vector_potential,
            result,
            initial,
            maximum_step_m=2.0e-4,
    ...
    assert len(a_only_comparison.cases) == 1
    a_only_case = a_only_comparison.cases[0]
    assert a_only_case.a_field_evaluations > 0
    assert a_only_case.a_rk_accepted_steps > 0
    assert a_only_case.lie_generator_substeps == 1
    assert a_only_case.maximum_raw_Ax_t_m < 1.0e-10
    assert a_only_comparison.maximum_lie_truncation_error < 2.0e-9
    assert a_only_comparison.maximum_polynomial_truncation_error < 2.0e-9
    assert a_only_comparison.maximum_lie_polynomial_difference < 2.0e-12
    ...
    assert comparison.maximum_polynomial_truncation_error < 2.0e-9
    assert comparison.maximum_lie_polynomial_difference < 2.0e-12
    ...
    assert certificate.field_representation == "hdiv-B-GridFunction"
