...
def test_dipole_magnet():
	...
	z_points = [-0.010, -0.005, 0, 0.005, 0.010]
	print(f"\n   {'Z (m)':>10} {'Bx (T)':>12} {'By (T)':>12} {'Bz (T)':>12}")
	print(f"   {'-'*10} {'-'*12} {'-'*12} {'-'*12}")

	for z in z_points:
		field = rad.Fld(container, 'b', [0, 0, z])
		print(f"   {z:10.4f} {field[0]:12.6f} {field[1]:12.6f} {field[2]:12.6f}")

	# Check field uniformity
	center_field = rad.Fld(container, 'b', [0, 0, 0])
	print(f"\n   [OK] Center field (Bz): {center_field[2]:.6f} T")
...
def test_quadrupole():
	...
	size = [0.020, 0.020, 0.100]  # 20x20x100 mm in meters
	offset = 0.025  # 25 mm in meters
	...
	x_points = [0, 0.002, 0.004, 0.006, 0.008, 0.010]
	print(f"\n   {'X (m)':>10} {'Bx (T)':>12} {'By (T)':>12} {'Gradient':>12}")
	print(f"   {'-'*10} {'-'*12} {'-'*12} {'-'*12}")
	...
	for x in x_points:
		...
		if prev_bx is not None and x > 0:
			gradient = (field[0] - prev_bx) / 0.002  # T/m
		else:
			gradient = 0
		print(f"   {x:10.4f} {field[0]:12.6f} {field[1]:12.6f} {gradient:12.6f}")
		prev_bx = field[0]
...
def test_field_integral():
	...
	z_start = -0.030  # -30 mm in meters
	z_end = 0.030     # 30 mm in meters
	n_points = 21

	print(f"\n   Integrating field from Z={z_start} to Z={z_end} m")
	...
	print(f"   Int(Bz*dz) = {integral_bz:.6f} T*m")
...
def main():
	...
	print("\n")
	print("+" + "=" * 58 + "+")
	print("|" + " " * 12 + "RADIA ADVANCED TEST SUITE" + " " * 21 + "|")
	print("+" + "=" * 58 + "+")
	...
	try:
		...
		print("\n" + "=" * 60)
		print("[OK] ADVANCED TESTS COMPLETED")
		print("=" * 60)
