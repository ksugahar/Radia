...
def test_remote_verify_passes_explicit_expectations_only_with_the_release_override(monkeypatch):
    ...
    assert r"radia-mcp=W:\00_CAE\Radia\release-quad\x\packages\radia-mcp" in argv
...
def test_lab_deploy_records_the_installed_source_after_pip_install(monkeypatch):
    ...
    monkeypatch.setattr(release_quad, "_kill_cubit_local", lambda: events.append("kill-cubit"))
    monkeypatch.setattr(release_quad, "_kill_mcp_local", lambda: events.append("kill-mcp"))
    ...
    install = next(i for i, e in enumerate(events) if isinstance(e, str) and "pip install" in e)
    record = events.index(("record", "S:/Radia/release-quad/x"))
    plugin = next(i for i, e in enumerate(events) if e == "cubit-plugin-install")
    assert install < record < plugin
