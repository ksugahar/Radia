...
All coordinates in meters, current density in A/m^2.
"""
...
def analytical_square_coil_field(I, a, z):
	...
	# Standard textbook formula for square coil:
...
def test_square_coil_analytical():
	...
	# Square coil parameters (in meters)
	a = 0.025  # 25 mm half side length, total side = 50 mm
	I = 1.0    # A (total current)

	# Wire dimensions (thin wire approximation, in meters)
	wx = 0.0001  # 0.1 mm wire thickness in x for vertical segments
	wy = 0.0001  # 0.1 mm wire thickness in y for horizontal segments
	h = 0.0001   # 0.1 mm wire height in z

	print(f"  Shape: Square coil (4 straight segments)")
	print(f"  Half-side length a: {a*1000:.1f} mm ({a} m)")
	print(f"  Total side length: {2*a*1000:.1f} mm")
	print(f"  Total current I: {I} A")
	print(f"  Wire dimensions: {wx*1000:.1f}x{wy*1000:.1f}x{h*1000:.1f} mm")

	# Calculate current density J = I / Area
	# For thin segments, area = wy * h (horizontal) or wx * h (vertical)
	J_horizontal = I / (wy * h)  # A/m^2
	J_vertical = I / (wx * h)    # A/m^2

	print(f"  Current density J (horizontal): {J_horizontal:.3e} A/m^2")
	print(f"  Current density J (vertical): {J_vertical:.3e} A/m^2")
	...
	# Test points along Z-axis (in meters)
	print("\n" + "-" * 70)
	print("Calculating fields along Z-axis...")
	print("-" * 70)

	z_values = [0.001, 0.002, 0.005, 0.010, 0.020, 0.030, 0.050, 0.075, 0.100]

	print(f"{'Z (m)':<12} {'Radia Bz (mT)':<16} {'Analytical Bz (mT)':<20} {'Difference':<12} {'Error %':<10}")
	print("-" * 70)
	...
	for z in z_values:
		# Radia calculation (coordinates already in meters)
		pt = [0, 0, z]
		b_radia = rad.Fld(coil, 'b', pt)
		bz_radia_mT = b_radia[2] * 1000  # Convert T to mT

		# Analytical calculation (a and z already in meters)
		bz_analytical_T = analytical_square_coil_field(I, a, z)
		...
		print(f"{z:<12.4f} {bz_radia_mT:<16.6f} {bz_analytical_mT:<20.6f} {diff:<12.6f} {error_percent:<10.2f}")

	# Summary
	print("-" * 70)
	avg_abs_error = total_abs_error / len(z_values)
	print(f"\nSummary:")
	...
	print("ANALYSIS")
	print("=" * 70)
	print(f"\nAnalytical formula: H = (2*I)/(pi*a) * 1/[sqrt(1+y^2)*sqrt(2+y^2)]")
	...
	print(f"\nRadia uses finite wire thickness ({wx*1000:.1f}x{wy*1000:.1f}x{h*1000:.1f} mm), so small")
