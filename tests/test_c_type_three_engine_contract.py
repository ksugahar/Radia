...
def test_three_engine_runner_has_shared_physics_contract():
    ...
    # The source projection order is contract-dependent: the total-Hodge route
    # projects at the field order, every other route needs at least quadratic.
    # This used to pin the single unconditional line that predated the split,
    # and went stale when the split arrived; pin both branches instead, so the
    # contract is what is asserted rather than one line of it.
    electromagnet = (ROOT / "src" / "radia" / "static_electromagnet.py"
                     ).read_text(encoding="utf-8")
    assert ('int(order) if source_potential_contract == "total_hodge"'
            in electromagnet)
...
def test_historical_global_omega_accuracy_certificate_is_not_relabelled():
    ...
    assert ("mixed total/reduced Omega"
            not in certificate["claim"]["scope"])
