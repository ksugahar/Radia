...
% MATLAB's round, upper-bound clamp, and binary divisibility test all
% disagree with Optuna, so every untransform path routes through
% radia.optuna.internal.UpstreamNumerics instead of the built-ins.
...
for index=1:numel(cases)
    ...
    verifyEqual(testCase,high,double(entry.adjusted_high), ...
        sprintf("adjusted high for [%g, %g] step %g", ...
        entry.low,entry.high,entry.step));
    verifyEqual(testCase,adjusted,logical(entry.adjusted), ...
        sprintf("adjustment flag for [%g, %g] step %g", ...
        entry.low,entry.high,entry.step));
...
% Optuna's Trial._suggest returns _get_single_value(distribution) before
% the sampler runs, so degenerate parameters leave the proposal sequence
% untouched.
...
actualBaseline=runSingleDistributionStudy(double(expected.seed), ...
    numel(baseline),false,testCase);
actualWithSingle=runSingleDistributionStudy(double(expected.seed), ...
    numel(withSingle),true,testCase);
...
for index=1:count
    ...
    if withSingle
        ...
        verifyEqual(testCase, ...
            string(trial.suggest_categorical("only",{'solo'})), ...
...
% Upstream reads TPE history from
% study._get_trials(states=(COMPLETE, PRUNED)), so pruned trials both count
% towards n_startup_trials and enter _split_trials, where
% _split_pruned_trials ranks them after every complete trial.
