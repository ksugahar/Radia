...
function testOptimizeRefusesMultiobjectiveContract(testCase)
parameters = radia.optuna.OptimizationParameter.fromStruct(struct("a", [0 1]));
options = radia.optuna.optimoptions(MaxTrials=4, Sampler="nsgaii", ...
    Seed=7, Display="off", Directions=["minimize", "minimize"]);
verifyError(testCase, ...
    @() radia.optuna.optimize(@(v) [v.a, 1-v.a], parameters, options), ...
    "radia:optuna:OptimizeDirections");
end
