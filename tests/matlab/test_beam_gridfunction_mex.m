...
verifyError(testCase,@() radia.beam.propagateGridFunctionLinearMap( ...
    field,0.1,[0 0 4],[0 0 1],2, ...
    FieldRepresentation="magnetic_flux_density"),"radia:mex:Exception");
...
nonlinear = radia.beam.propagateGridFunctionMultipoleMap( ...
    ...
    MaximumStepM=1e-3,FieldRepresentation="magnetic_flux_density");
...
function testHCurlVectorPotentialModeUsesNativeCurl(testCase)
field = radia.ngsolve.GridFunction.create( ...
    string(testCase.TestData.VolPath),"hcurl",4,NoGrads=false);
fieldCleanup = onCleanup(@() delete(field));
field.setVector(zeros(field.DofCount,1));

result = radia.beam.propagateGridFunctionMultipoleMap( ...
    field,0.02,[0 0.2 0.25],[0 0 1],2, ...
    SampleRadiusM=0.02,MultipoleOrder=2,MaximumMapOrder=1);
verifyEqual(testCase,string(result.field_representation), ...
    "hcurl_vector_potential");
verifyEqual(testCase,string(result.magnetic_evaluation), ...
    "ngsolve-native-curl(A)");
verifyEqual(testCase,string(result.grid_function_space_class), ...
    "HCurlHighOrderFESpace");
verifyEqual(testCase,result.grid_function_space_order,4);
verifyEqual(testCase,result.center_field_local_t,zeros(1,3), ...
    "AbsTol",1e-14);
