...
def _numeric_untransform_contract() -> dict[str, object]:
    """Primitives every untransform path depends on.

    MATLAB's built-ins disagree with all three: ``round`` rounds halves away
    from zero instead of to even, a float suggestion clamped to ``high``
    ignores that Optuna returns ``np.nextafter(high, -inf)``, and a binary
    divisibility test reports ranges such as ``[0, 0.7]`` step ``0.1`` as
    non-divisible where ``decimal.Decimal(str(x))`` does not.
    """
...
def _single_distribution_rng_contract() -> dict[str, object]:
    """A single()-valued distribution never reaches the sampler.

    ``Trial._suggest`` short-circuits it with ``_get_single_value`` before
    ``sample_independent`` runs, so it consumes no random number and the
    remaining proposal sequence is unchanged. A port that lets the sampler
    draw for the degenerate parameter desynchronises the whole stream.
    """
...
def _tpe_pruned_history_trials() -> dict[str, object]:
    """TPE history that mixes COMPLETE and PRUNED trials.

    Upstream reads its history from
    ``study._get_trials(states=(COMPLETE, PRUNED))``, so pruned trials count
    towards ``n_startup_trials`` and enter ``_split_trials``, where
    ``_split_pruned_trials`` ranks them after every complete trial by
    ``(-last_step, intermediate_value)``. A port that keeps only COMPLETE
    trials stays in its random startup phase longer and then fits a
    truncated history, so the proposal sequence diverges from trial 7 on.
    """
    study = optuna.create_study(
