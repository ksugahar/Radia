...
function testFFAGFixedDesignOrbitTargetHasNamedFallback(testCase)
angle = 0.08;
radius = 8;
positions = [0, 0, 0; ...
    radius*sin(angle), radius*(1-cos(angle)), 0];
tangents = [1, 0, 0; cos(angle), sin(angle), 0];
orbit = radia.python.acceleratorMagnetTopopt( ...
    "PlanarDesignOrbit", ...
    {positions, tangents, 1.4, [0, 0, 1]});
target = zeros(1, 6, 6);
target(1, :, :) = eye(6);
result = radia.python.buildFFAGFixedDesignOrbitTargetFamily( ...
    {orbit.value}, target, TransferMatrixBand=2e-3, ...
    BendFieldBand=3e-3, ResponseEntries=[0, 0; 0, 1]);
verifyEqual(testCase, result.backend, "python-fallback");
verifyEqual(testCase, result.module, "radia.ffag_topopt");
verifyEqual(testCase, ...
    double(result.value.magnetic_rigidities_tm), 1.4, AbsTol=0);
verifyEqual(testCase, ...
    double(result.value.objective.transfer_matrix_band), ...
    2e-3*ones(1, 6, 6), AbsTol=0);
end

function testFFAGFixedDesignOrbitTargetSelectsSymplecticComponents(testCase)
angle = 0.08;
radius = 8;
positions = [0, 0, 0; ...
    radius*sin(angle), radius*(1-cos(angle)), 0];
...
result = radia.python.buildFFAGFixedDesignOrbitTargetFamily( ...
    {orbit.value}, target, ...
    ControlledComponents=["horizontal_focusing", ...
        "vertical_focusing", "horizontal_dispersion"]);
entries = double(result.value.objective.response_entries);
verifyEqual(testCase, entries, [1, 0; 3, 2; 0, 5; 1, 5], AbsTol=0);
verifyEqual(testCase, ...
    double(result.value.target_symplectic_residuals), 0, AbsTol=0);
verifyEqual(testCase, result.backend, "python-fallback");
end
