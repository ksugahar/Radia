    ...
    st_cHACApK_cluster_t *rc, st_cHACApK_cluster_t *cc)
{
    ...
    /* Probe for a leaf at this exact (rc, cc). */
    st_cHACApK_leafmtx_t *leaf =
        find_leaf_sorted(sorted, n_sorted, rc->nstrt, cc->nstrt);
    if (leaf && leaf->ndl == rc->nsize && leaf->ndt == cc->nsize) {
        node->leaf_mtx  = leaf;
    ...
    if (rc->nnson <= 0 || cc->nnson <= 0) {
        ...
        fprintf(stderr,
                ...
                "(nstrt=%d size=%d) x (nstrt=%d size=%d): "
                "no leaf, no children. Aborting build.\n",
    ...
    for (int j = 0; j < node->ncsons; j++) {
        ...
        for (int i = 0; i < node->nrsons; i++) {
            ...
            st_cHACApK_block_node_t *child =
                build_rec(sorted, n_sorted, rc_son, cc_son);
            if (!child) {
                /* Partial cleanup -- best-effort, hard-fail downstream. */
    ...
    st_cHACApK_cluster_t  *root_col_cluster)
{
    ...
    st_cHACApK_block_node_t *root =
        build_rec(sorted, n_sorted, root_row_cluster, root_col_cluster);
