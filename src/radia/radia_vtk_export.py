...
Functions for exporting Radia magnetic field data to VTS (VTK XML Structured Grid)
format for visualization in ParaView and other VTK-compatible tools.

VTS Format:
    - XML-based structured grid format
    - Ideal for 3D field grids on regular meshes
    - Supports B, H fields and their magnitudes
    - Coordinates automatically converted to meters (VTK standard)

Performance:
    - Use rad.FldVTS() for C++ performance (large grids)
    - Use RadiaVTKOutput class for Python flexibility
