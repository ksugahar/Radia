"""Command-line interface for :mod:`radia.ltspice`.

Three modes -- convert, check, info -- wrapped around the same Python API
the MCP server uses. ``radia-ltspice`` is canonical; the former
``spice-circuit-lab`` and ``ltspice-convert`` command names remain aliases.
...
Convert a single file (auto target by extension):
    ...
    radia-ltspice input.cir -o output.py
...
Lint a circuit (round-trip check + .asy availability):
    ...
    radia-ltspice --check dimmer.asc
    radia-ltspice --check --strict *.asc    # exit 1 on any warning

Show a summary of a circuit:

    radia-ltspice --info input.asc
    radia-ltspice --info --json input.asc   # machine-readable

Add a third-party symbol library to the .asy search path:

    radia-ltspice --asy-dir /path/to/MyLib/sym input.asc -o output.cir
...
Examples:
  ...
  radia-ltspice --info input.asc --json
""",
    )
    p.add_argument(
