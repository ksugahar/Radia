"""MCP server for :mod:`radia.ltspice`.
...
Run via ``mcp-server-radia-ltspice``. The former
``mcp-spice-circuit-lab`` and ``mcp-ltspice`` commands remain aliases.
"""
...
@mcp.tool()
def second_order_complex_zero_transfer_gate(summary: dict) -> dict:
    """Gate a second-order two-pole/two-complex-zero transfer and replay."""

    try:
        return _second_order_complex_zero_gate(summary)
    except (TypeError, ValueError) as exc:
        return {
...
def check_circuit(text: str, fmt: str,
    ...
    Same logic as the ``radia-ltspice --check`` CLI command, exposed
...
def info_circuit(text: str, fmt: str,
    ...
    Same logic as ``radia-ltspice --info --json``.
...
def main() -> int:
    """Entry point for the ``mcp-server-radia-ltspice`` console script."""
    try:
        mcp.run()
