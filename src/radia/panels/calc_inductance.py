...
def _write_nastran_ctria6(filename, mesh_surf):
    ...
    # Write Nastran BDF (long format GRID*, 8-char field elements)
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        # GRID* cards (16-char fields, same format as cubit_mesh_export.py)
        for i, (x, y, z) in enumerate(nodes):
            nid = i + 1
            f.write(f'GRID*   {nid:>16}{0:>16}{x:>16.5f}{y:>16.5f}\n')
            f.write(f'*       {z:>16.5f}\n')
        ...
        # CTRIA6 cards (8-char fields)
        pid = 1
        for i, conn in enumerate(elements):
            eid = i + 1
            n = [c + 1 for c in conn]  # 1-indexed
            f.write(f'CTRIA6  {eid:>8}{pid:>8}'
                    f'{n[0]:>8}{n[1]:>8}{n[2]:>8}{n[3]:>8}{n[4]:>8}{n[5]:>8}\n')
