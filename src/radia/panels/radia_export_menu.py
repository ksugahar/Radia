...
# ----------------------------------------------------------------------
# Mesh Evaluation — p-convergence (order 1..5) + format QA
# ----------------------------------------------------------------------

def _run_mesh_evaluation(cubit_mod, parent):
    """Export _p1.vol .. _p5.vol + per-format QA exports, then evaluate."""
    global _last_jou_path
    ...
    base = jou_path
    if base.lower().endswith(".jou"):
        base = base[:-4]
    out_dir = os.path.dirname(base).replace("\\", "/")
    os.makedirs(out_dir, exist_ok=True)
    cubit_mod.cmd(f'cd "{out_dir}"')

    # --- Phase 1: C++ exports ---
    max_order = 5
    print(f"Exporting .vol order 1-{max_order}...")
    for p in range(1, max_order + 1):
        ...
        if os.path.isfile(vp):
            ...
            print(f"  WARN: order {p} export failed")

    # Format QA: GMSH 1-3, Nastran/VTK 1-2
    ...
    for name, ext, cmd_prefix, max_ord in fmt_specs:
        for p in range(1, max_ord + 1):
            fpath = f"{base}_qa_{name}_o{p}.{ext}"
            cmd = f'{cmd_prefix} "{fpath}"'
            if p >= 2:
                cmd += f" order {p}"
            cmd += " overwrite"
            cubit_mod.cmd(cmd)
    ...
    python = _find_external_python()
    script = _find_calc_script(python, "calc_mesh_eval")
    if not script:
        QMessageBox.warning(
            parent, "Mesh Evaluation",
            "calc_mesh_eval.py not found. Is `radia` installed?")
        return

    print("Evaluating mesh with NGSolve + GMSH...")
    ...
    try:
        rc, stdout, stderr = _run_subprocess_utf8(
            [python] + args, timeout=600)
    except subprocess.TimeoutExpired:
        QMessageBox.warning(parent, "Mesh Evaluation",
                            "Mesh evaluation timed out.")
        return

    if rc != 0:
        QMessageBox.warning(
            parent, "Mesh Evaluation",
            f"Mesh evaluation failed:\n\n{stderr[:2000]}")
        return

    lines = [ln for ln in stdout.splitlines() if ln.strip()]
    if not lines:
        QMessageBox.warning(parent, "Mesh Evaluation",
                            "No output from calc_mesh_eval.py.")
        return
    try:
        r = json.loads(lines[-1])
    except json.JSONDecodeError:
        QMessageBox.warning(parent, "Mesh Evaluation",
                            "Invalid JSON from calc_mesh_eval.py.")
        return
    ...
    _show_mesh_eval_result(r, parent)


def _fit_table(tbl):
    """Resize columns/rows and fix the widget size to its contents."""
    tbl.resizeColumnsToContents()
    tbl.resizeRowsToContents()
    w = tbl.verticalHeader().width() if tbl.verticalHeader().isVisible() else 0
    for c in range(tbl.columnCount()):
        w += tbl.columnWidth(c)
    w += tbl.frameWidth() * 2 + 4
    h = tbl.horizontalHeader().height()
    for r in range(tbl.rowCount()):
        h += tbl.rowHeight(r)
    h += tbl.frameWidth() * 2 + 2
    tbl.setFixedSize(w, h)


def _show_mesh_eval_result(r, parent):
    """Modal dialog with two tables: p-convergence + format QA."""
    dlg = QDialog(parent)
    dlg.setWindowTitle("Mesh Evaluation - Volume / Area / Length")
    layout = QVBoxLayout(dlg)

    cad_v = r.get("cad_vol_total", 0.0)
    cad_a = r.get("cad_area_total", 0.0)
    cad_l = r.get("cad_length_total", 0.0)
    layout.addWidget(QLabel(
        f"CAD Volume: {cad_v:.6e} m^3     "
        f"CAD Area: {cad_a:.6e} m^2     "
        f"CAD Length: {cad_l:.6e} m"))

    orders = r.get("orders", [])
    nrows = len(orders)
    table = QTableWidget(nrows, 7, dlg)
    table.setHorizontalHeaderLabels(
        ["p", "Volume", "V err [%]", "Area", "A err [%]",
         "Length", "L err [%]"])
    table.verticalHeader().setVisible(False)
    table.setEditTriggers(QTableWidget.NoEditTriggers)

    for i, o in enumerate(orders):
        table.setItem(i, 0, QTableWidgetItem(str(o.get("order", ""))))
        if "error" in o:
            table.setItem(i, 1, QTableWidgetItem(o["error"]))
        else:
            table.setItem(i, 1, QTableWidgetItem(
                f"{o.get('ng_volume', 0.0):.6e}"))
            table.setItem(i, 2, QTableWidgetItem(
                f"{o.get('vol_error_pct', 0.0):.2e}"))
            table.setItem(i, 3, QTableWidgetItem(
                f"{o.get('ng_area', 0.0):.6e}"))
            table.setItem(i, 4, QTableWidgetItem(
                f"{o.get('area_error_pct', 0.0):.2e}"))
            table.setItem(i, 5, QTableWidgetItem(
                f"{o.get('ng_length', 0.0):.6e}"))
            table.setItem(i, 6, QTableWidgetItem(
                f"{o.get('len_error_pct', 0.0):.2e}"))
    _fit_table(table)
    layout.addWidget(table)

    fmt_arr = r.get("format_qa", [])
    nfmt = len(fmt_arr)
    if nfmt > 0:
        layout.addWidget(QLabel(
            "\nFormat QA vs CAD (GMSH API Jacobian verification):"))
        fmt_table = QTableWidget(nfmt, 7, dlg)
        fmt_table.setHorizontalHeaderLabels(
            ["Format", "Order", "Volume", "V err [%]",
             "Area", "A err [%]", "neg_det"])
        fmt_table.verticalHeader().setVisible(False)
        fmt_table.setEditTriggers(QTableWidget.NoEditTriggers)
        for i, fo in enumerate(fmt_arr):
            fmt_table.setItem(i, 0, QTableWidgetItem(
                fo.get("format", "")))
            fmt_table.setItem(i, 1, QTableWidgetItem(
                str(fo.get("order", ""))))
            if "error" in fo:
                err_item = QTableWidgetItem(fo["error"])
                err_item.setBackground(QColor(255, 200, 200))
                fmt_table.setItem(i, 2, err_item)
            else:
                fmt_table.setItem(i, 2, QTableWidgetItem(
                    f"{fo.get('volume', 0.0):.6e}"))
                fmt_table.setItem(i, 3, QTableWidgetItem(
                    f"{fo.get('vol_error_pct', 0.0):.2e}"))
                fmt_table.setItem(i, 4, QTableWidgetItem(
                    f"{fo.get('area', 0.0):.6e}"))
                fmt_table.setItem(i, 5, QTableWidgetItem(
                    f"{fo.get('area_error_pct', 0.0):.2e}"))
                fmt_table.setItem(i, 6, QTableWidgetItem(
                    str(fo.get("neg_det", 0))))
        _fit_table(fmt_table)
        layout.addWidget(fmt_table)
    ...
    def _copy_tsv():
        ...
        if nfmt > 0:
            tsv_lines.append("")
            tsv_lines.append(
                "Format\tOrder\tVolume\tV err [%]\tArea\tA err [%]\t"
                "neg_det")
            for fo in fmt_arr:
                if "error" in fo:
                    tsv_lines.append(
    ...
    buttons.button(QDialogButtonBox.Save).clicked.connect(_copy_tsv)
    dlg.exec()
...
def install_menu():
    ...
    _action(
        "Mesh Evaluation...",
        "Volume/Area p-convergence check (order 1-5) against CAD",
        lambda: _run_mesh_evaluation(cubit_mod, main))

    menu_bar.addMenu(menu)
    print("[Radia] Radia Export menu installed "
          "(7 actions, PySide6 toolbar)")
    return menu
