...
design. The EM application (`calc_accel_magnet` / `calc_accel_hdiv`)
accepts **only** a Python module. The PEEC headless path (
`calc_inductance.py --coil-solver peec` for inductance-only and
weak-coupled workpiece modes) accepts **only** STEP.

| Application / calc | Coil input | Loader |
|--------------|-----------|--------|
...
rescued STEP fixtures now live under
...
5. Compare against the committed JSON under
   `validation_test/panels/golden/`. The owning validation test checks the
   quantities and tolerances; ordinary unit tests do not regenerate evidence.

## BH curve (nonlinear material)

For nonlinear-iron analyses (`--material steel`), the application can either
use the built-in `STEEL_BH` curve in
...
equivalent (max relative diff 2e-5).
  ...
  - Pass via the Simulink mask: select the BH material mode and the
    `em_sample_bh.txt` file (or any equivalent 2-column file)
...
The full canonical EM application trio (all shipped in this directory
or its parent `panels/samples/`):

| Artifact | File | Role |
|---|---|---|
| Coil | `em_sample_coil.py` (`build_coil() -> CoilBuilder`) | Analytical Biot-Savart source (`--coil-script`) |
| Mesh | `em_sample.jou` -> `em_sample.vol` (yoke + air + Kelvin) | FEM domain (`--vol`) |
| Material | `em_sample_bh.txt` (or built-in `STEEL_BH`) | Nonlinear iron BH curve (`--bh-file`) |
