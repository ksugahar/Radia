...
def _example3_occ() -> dict[str, Any]:
    ...
    for _ in range(num_per):
        y += 0.5 * lm[1]
        magnets.append(_box_from_center(
    ...
    magnets = mirror_all(magnets, X, Y)
    poles = mirror_all(poles, Z, X)
    magnets = mirror_all(magnets, Z, X)
    poles = mirror_all(poles, Y, X)
    magnets = mirror_all(magnets, Y, X)
    return {"iron": _compound(poles, "iron"),
            "magnet": _compound(magnets, "permanent_magnet")}
...
def get_esrf_cubit_mesh_policy(number: int) -> dict[str, Any]:
    ...
    The HDiv-MMM/BDM2 route is air-mesh-free, so its solver mesh contains
    only magnetic material.  The independent HCurl FEM route is conforming
    and prefers an all-HEX iron/air domain; TET outer air with an explicit
    WEDGE/PYRAMID transition is the allowed fallback.  Permanent magnets are
    also meshed with Cubit's HEX-capable ``auto`` scheme when their topology
    permits it.  Coils are
    never solver-meshed: their STEP is visualization/provenance only and the
    physical source is always supplied by :class:`CoilBuilder`.
    ...
    return {
        ...
        "solver_routes": {
            "bdm2_ima": {
                "mesh": "Cubit magnetic-body mesh only",
...
def export_esrf_cubit_assets(number: int, output_dir: str | Path,
    ...
    """Write STEP categories, a HEX-preferred Cubit journal, and metadata."""
...
def export_esrf_cubit_suite(output_dir: str | Path,
    ...
    for manifest in manifests:
        lines = Path(manifest["journal"]).read_text(encoding="utf-8").splitlines()
