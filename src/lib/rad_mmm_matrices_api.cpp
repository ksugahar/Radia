...
#ifdef RADIA_USE_HACAPK
/**
 * Python-friendly HACApK H-Matrix Solver
 *
 * Provides O(N log N) matrix-vector products for large problems.
 */
class PyMMMHACApKSolver {
public:
    PyMMMHACApKSolver() : solver_(std::make_unique<MMMHACApKSolver>()) {}

    /**
     * Set elements from MMMBuilder
     */
    void set_from_builder(const PyMMMBuilder& builder,
                          py::array_t<int, py::array::c_style | py::array::forcecast> dof_offset) {
        auto dof_info = dof_offset.request();
        std::vector<int> dof_vec(static_cast<int*>(dof_info.ptr),
                                  static_cast<int*>(dof_info.ptr) + dof_info.size);
        solver_->SetElements(builder.get_elements(), dof_vec);
    }

    /**
     * Set interaction matrix from numpy array
     */
    void set_matrix(py::array_t<double, py::array::c_style | py::array::forcecast> N) {
        auto N_info = N.request();
        if (N_info.ndim != 2 || N_info.shape[0] != N_info.shape[1]) {
        ...
        int total_dof = static_cast<int>(N_info.shape[0]);
        std::vector<double> N_vec(static_cast<double*>(N_info.ptr),
                                   static_cast<double*>(N_info.ptr) + total_dof * total_dof);
        solver_->SetInteractionMatrix(N_vec, total_dof);
    }

    /**
     * Build H-matrix approximation
     */
    bool build_hmatrix(py::array_t<double, py::array::c_style | py::array::forcecast> inv_chi,
                       double eps = 1e-4, int leaf_size = 32, double eta = 2.0,
                       int max_rank = 200, int print_level = 0) {
        auto inv_chi_info = inv_chi.request();
        std::vector<double> inv_chi_vec(static_cast<double*>(inv_chi_info.ptr),
                                         static_cast<double*>(inv_chi_info.ptr) + inv_chi_info.size);
        ...
        return solver_->BuildHMatrix(params, inv_chi_vec);
    }

    /**
     * Solve linear system using H-matrix accelerated BiCGSTAB
     */
    py::dict solve(py::array_t<double, py::array::c_style | py::array::forcecast> inv_chi,
                   py::array_t<double, py::array::c_style | py::array::forcecast> H_ext,
        ...
        std::vector<double> inv_chi_vec(static_cast<double*>(inv_chi_info.ptr),
                                         static_cast<double*>(inv_chi_info.ptr) + inv_chi_info.size);
        std::vector<double> H_ext_vec(static_cast<double*>(H_ext_info.ptr),
                                       static_cast<double*>(H_ext_info.ptr) + H_ext_info.size);

        std::vector<double> M(solver_->GetNDOF());
        int iters = solver_->Solve(inv_chi_vec, H_ext_vec, M, tol, max_iter);

        py::dict result;
        result["M"] = py::array_t<double>(M.size(), M.data());
    ...
    py::array_t<double> matvec(py::array_t<double, py::array::c_style | py::array::forcecast> x) {
        ...
        std::vector<double> x_vec(static_cast<double*>(x_info.ptr),
                                   static_cast<double*>(x_info.ptr) + x_info.size);
        std::vector<double> y(solver_->GetNDOF());
        solver_->MatVec(x_vec, y);
        return py::array_t<double>(y.size(), y.data());
    }

    /**
     * Update diagonal for nonlinear iteration
     */
    void update_diagonal(py::array_t<double, py::array::c_style | py::array::forcecast> inv_chi) {
        auto inv_chi_info = inv_chi.request();
        std::vector<double> inv_chi_vec(static_cast<double*>(inv_chi_info.ptr),
                                         static_cast<double*>(inv_chi_info.ptr) + inv_chi_info.size);
        solver_->UpdateDiagonal(inv_chi_vec);
    }
    ...
    py::dict get_stats() const {
        ...
        py::dict result;
        result["n_lowrank"] = stats.n_lowrank;
        result["n_dense"] = stats.n_dense;
        result["max_rank"] = stats.max_rank;
        result["n_leaves"] = stats.n_leaves;
        result["n_dof"] = stats.n_dof;
        result["compression"] = stats.compression;
        result["build_time"] = stats.build_time;
        result["memory_mb"] = stats.memory_mb;
        result["dense_memory_mb"] = stats.dense_memory_mb;
        return result;
    }

    bool is_valid() const { return solver_->IsValid(); }
    int total_dof() const { return solver_->GetNDOF(); }

private:
    std::unique_ptr<MMMHACApKSolver> solver_;
};
...
#ifdef RADIA_USE_HACAPK
    // MMMHACApKSolver class - H-matrix accelerated BiCGSTAB
    py::class_<PyMMMHACApKSolver>(m, "MMMHACApKSolver",
        R"doc(
        MMM H-Matrix Solver (HACApK)

        H-matrix accelerated BiCGSTAB solver for large problems.
        Provides O(N log N) complexity for matrix-vector products.

        Example:
            from mmm_core import MMMBuilder, MMMHACApKSolver
            import numpy as np
            ...
            # Create H-matrix solver
            solver = MMMHACApKSolver()
            solver.set_from_builder(builder, dof_offset)
            solver.set_matrix(N)

            # Solve
            n_elem = len(dof_offset) - 1
            mu_r = 1000
            inv_chi = np.full(n_elem, 1.0 / (mu_r - 1))

            # Build H-matrix with ACA+ compression
            solver.build_hmatrix(inv_chi, eps=1e-4, leaf_size=32, eta=2.0)

            result = solver.solve(inv_chi, H_ext)
            M = result['M']
        )doc")
        .def(py::init<>(), "Create a new H-matrix MMM solver")

        .def("set_from_builder", &PyMMMHACApKSolver::set_from_builder,
             py::arg("builder"), py::arg("dof_offset"),
             R"doc(
             Set elements from MMMBuilder.

             Args:
                 builder: MMMBuilder instance with elements
                 dof_offset: (n_elem + 1,) DOF offset array
             )doc")

        .def("set_matrix", &PyMMMHACApKSolver::set_matrix,
             py::arg("N"),
             R"doc(
             Set interaction matrix.
        ...
        .def("build_hmatrix", &PyMMMHACApKSolver::build_hmatrix,
             py::arg("inv_chi"),
             py::arg("eps") = 1e-4, py::arg("leaf_size") = 32,
             py::arg("eta") = 2.0, py::arg("max_rank") = 200,
             py::arg("print_level") = 0,
             R"doc(
             Build H-matrix approximation using ACA+.

             Args:
                 inv_chi: Inverse susceptibility per element (n_elements,)
                 ...
                 print_level: 0=silent, 1=standard, 2=debug

             Returns:
                 True if successful
             )doc")

        .def("solve", &PyMMMHACApKSolver::solve,
             py::arg("inv_chi"), py::arg("H_ext"),
             py::arg("tol") = 1e-8, py::arg("max_iter") = 1000,
             R"doc(
             Solve linear system using H-matrix BiCGSTAB.

             Args:
                 inv_chi: Inverse susceptibility (n_elements,)
                 H_ext: External field (total_dof,)
                 tol: Convergence tolerance
                 max_iter: Maximum iterations

             Returns:
                 dict with M, iterations, converged
             )doc")

        .def("matvec", &PyMMMHACApKSolver::matvec,
             py::arg("x"),
             R"doc(
        ...
        .def("update_diagonal", &PyMMMHACApKSolver::update_diagonal,
             py::arg("inv_chi"),
             R"doc(
             Update diagonal for nonlinear iteration.

             Args:
                 inv_chi: New inverse susceptibility (n_elements,)
             )doc")

        .def("get_stats", &PyMMMHACApKSolver::get_stats,
             R"doc(
             Get H-matrix statistics.

             Returns:
                 dict with n_lowrank, n_dense, max_rank, n_leaves,
                 n_dof, compression, build_time, memory_mb, dense_memory_mb
             )doc")

        .def_property_readonly("is_valid", &PyMMMHACApKSolver::is_valid,
            "Check if H-matrix is built and valid")
        .def_property_readonly("total_dof", &PyMMMHACApKSolver::total_dof,
            "Total degrees of freedom");
#endif // RADIA_USE_HACAPK
