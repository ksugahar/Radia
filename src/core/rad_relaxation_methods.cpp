...
//=========================================================================
// Method 2: Implicit Successive Substitution (Gauss-Seidel)
//=========================================================================
// Reference: Original Radia radTRelaxationMethNo_3 (ochubar/Radia)
//
// Algorithm:
// For each element i (sequentially):
//   1. QuasiExtField[i] = sum(N[i][j] * M[j], j!=i) + H_ext[i]
//   2. Solve (I - chi*N[i][i]) * H[i] = chi*Mr + QuasiExtField[i] (3x3 inversion)
//   3. M_new[i] = M(H[i]) from material constitutive relation
//   4. Apply under-relaxation: M[i] = omega*M_new + (1-omega)*M_old
//   5. Update M[i] immediately (Gauss-Seidel style)
//
// Benefits:
// - No full system matrix storage (O(N^2) memory for N matrix is shared)
// - Better convergence for nonlinear materials
// - Under-relaxation for stability with high permeability
//=========================================================================

int radTRelaxationMethNo_2::Matrix3d_inv(const double A[3][3], double Ainv[3][3])
{
	// Compute determinant
	...
	Ainv[0][0] = inv_det * (A[1][1] * A[2][2] - A[1][2] * A[2][1]);
	Ainv[0][1] = inv_det * (A[0][2] * A[2][1] - A[0][1] * A[2][2]);
	Ainv[0][2] = inv_det * (A[0][1] * A[1][2] - A[0][2] * A[1][1]);

	Ainv[1][0] = inv_det * (A[1][2] * A[2][0] - A[1][0] * A[2][2]);
	Ainv[1][1] = inv_det * (A[0][0] * A[2][2] - A[0][2] * A[2][0]);
	Ainv[1][2] = inv_det * (A[0][2] * A[1][0] - A[0][0] * A[1][2]);

	Ainv[2][0] = inv_det * (A[1][0] * A[2][1] - A[1][1] * A[2][0]);
	Ainv[2][1] = inv_det * (A[0][1] * A[2][0] - A[0][0] * A[2][1]);
	Ainv[2][2] = inv_det * (A[0][0] * A[1][1] - A[0][1] * A[1][0]);
...
void radTRelaxationMethNo_2::DefineNewMagnetizations()
{
	...
	double BufMisfitM = 0.0;

	// Process each element sequentially (Gauss-Seidel)
	for(int i = 0; i < LocAmOfMainElem; i++)
	{
		// 1. Compute quasi-external field: sum(N[i][j] * M[j], j!=i) + H_ext[i]
		TVector3d QuasiExtField(0., 0., 0.);
		for(int j = 0; j < LocAmOfMainElem; j++)
		{
			if(j != i)
			{
				// N[i][j] * M[j]
		...
		QuasiExtField.x += ExternFieldAr[i].x;
		QuasiExtField.y += ExternFieldAr[i].y;
		QuasiExtField.z += ExternFieldAr[i].z;

		// 2. Get material and compute chi*N[i][i] matrix
		radTg3dRelax* g3dRelaxPtr = IntrctPtr->g3dRelaxPtrVect[i];
		radTMaterial* MaterPtr = (radTMaterial*)(g3dRelaxPtr->MaterHandle.rep);

		// Get instantaneous susceptibility tensor chi(H) and remanent magnetization Mr
		TMatrix3d KsiTensor;
		TVector3d MrVect;
		MaterPtr->DefineInstantKsiTensor(NewFieldAr[i], KsiTensor, MrVect);

		// Build matrix: BufMatr = I - chi * N[i][i]
		// N[i][i] is the self-interaction matrix (stored in InteractMatrix)
		TMatrix3df& Nii = IntrcMat[i][i];

		// chi * N[i][i] (3x3 matrix multiplication)
		double chi_N[3][3];
		chi_N[0][0] = KsiTensor.Str0.x * Nii.Str0.x + KsiTensor.Str0.y * Nii.Str1.x + KsiTensor.Str0.z * Nii.Str2.x;
		chi_N[0][1] = KsiTensor.Str0.x * Nii.Str0.y + KsiTensor.Str0.y * Nii.Str1.y + KsiTensor.Str0.z * Nii.Str2.y;
		chi_N[0][2] = KsiTensor.Str0.x * Nii.Str0.z + KsiTensor.Str0.y * Nii.Str1.z + KsiTensor.Str0.z * Nii.Str2.z;

		chi_N[1][0] = KsiTensor.Str1.x * Nii.Str0.x + KsiTensor.Str1.y * Nii.Str1.x + KsiTensor.Str1.z * Nii.Str2.x;
		chi_N[1][1] = KsiTensor.Str1.x * Nii.Str0.y + KsiTensor.Str1.y * Nii.Str1.y + KsiTensor.Str1.z * Nii.Str2.y;
		chi_N[1][2] = KsiTensor.Str1.x * Nii.Str0.z + KsiTensor.Str1.y * Nii.Str1.z + KsiTensor.Str1.z * Nii.Str2.z;

		chi_N[2][0] = KsiTensor.Str2.x * Nii.Str0.x + KsiTensor.Str2.y * Nii.Str1.x + KsiTensor.Str2.z * Nii.Str2.x;
		chi_N[2][1] = KsiTensor.Str2.x * Nii.Str0.y + KsiTensor.Str2.y * Nii.Str1.y + KsiTensor.Str2.z * Nii.Str2.y;
		chi_N[2][2] = KsiTensor.Str2.x * Nii.Str0.z + KsiTensor.Str2.y * Nii.Str1.z + KsiTensor.Str2.z * Nii.Str2.z;

		// BufMatr = I - chi*N[i][i]
		double BufMatr[3][3];
		BufMatr[0][0] = 1.0 - chi_N[0][0]; BufMatr[0][1] = -chi_N[0][1]; BufMatr[0][2] = -chi_N[0][2];
		BufMatr[1][0] = -chi_N[1][0]; BufMatr[1][1] = 1.0 - chi_N[1][1]; BufMatr[1][2] = -chi_N[1][2];
		BufMatr[2][0] = -chi_N[2][0]; BufMatr[2][1] = -chi_N[2][1]; BufMatr[2][2] = 1.0 - chi_N[2][2];

		// 3. Invert BufMatr
		double InvBufMatr[3][3];
		Matrix3d_inv(BufMatr, InvBufMatr);

		// 4. Compute RHS: chi*Mr + chi*QuasiExtField
		// But original Radia uses: RHS = MultByInstMr + QuasiExtField
		// where MultByInstMr is computed by MultMatrByInstKsiAndMr
		// For simplicity, use: RHS = chi*Mr + QuasiExtField
		// Then H_new = InvBufMatr * RHS
		TVector3d RHS;
		RHS.x = KsiTensor.Str0.x * MrVect.x + KsiTensor.Str0.y * MrVect.y + KsiTensor.Str0.z * MrVect.z + QuasiExtField.x;
		RHS.y = KsiTensor.Str1.x * MrVect.x + KsiTensor.Str1.y * MrVect.y + KsiTensor.Str1.z * MrVect.z + QuasiExtField.y;
		RHS.z = KsiTensor.Str2.x * MrVect.x + KsiTensor.Str2.y * MrVect.y + KsiTensor.Str2.z * MrVect.z + QuasiExtField.z;

		// H_new = InvBufMatr * RHS
		TVector3d H_new;
		H_new.x = InvBufMatr[0][0] * RHS.x + InvBufMatr[0][1] * RHS.y + InvBufMatr[0][2] * RHS.z;
		H_new.y = InvBufMatr[1][0] * RHS.x + InvBufMatr[1][1] * RHS.y + InvBufMatr[1][2] * RHS.z;
		H_new.z = InvBufMatr[2][0] * RHS.x + InvBufMatr[2][1] * RHS.y + InvBufMatr[2][2] * RHS.z;

		// Update H field
		NewFieldAr[i] = H_new;

		// 5. Compute M from material constitutive relation M = M(H)
		TVector3d M_new = MaterPtr->M(H_new);

		// 6. Apply under-relaxation: M = omega * M_new + (1-omega) * M_old
		TVector3d M_old = MagnAr[i];
		TVector3d M_relaxed;
		M_relaxed.x = Omega * M_new.x + (1.0 - Omega) * M_old.x;
		M_relaxed.y = Omega * M_new.y + (1.0 - Omega) * M_old.y;
		M_relaxed.z = Omega * M_new.z + (1.0 - Omega) * M_old.z;

		// 7. Update misfit tracking
		TVector3d Mdiff = M_relaxed - g3dRelaxPtr->Magn;
		BufMisfitM += Mdiff.x * Mdiff.x + Mdiff.y * Mdiff.y + Mdiff.z * Mdiff.z;

		// 8. Update magnetization (Gauss-Seidel: immediate update)
		MagnAr[i] = M_relaxed;
		g3dRelaxPtr->Magn = M_relaxed;
	}

	// Compute RMS misfit
	InstMisfitM = std::sqrt(BufMisfitM / LocAmOfMainElem);
}
...
int radTRelaxationMethNo_2::AutoRelax(double PrecOnMagnetiz, int MaxIterNumber, char MagnResetIsNotNeeded)
{
	...
	// Initialize misfit to large value
	InstMisfitM = 1.0e30;
...
{
	// For now, fall back to standard solver
	// Variable DOF version not yet implemented for implicit SS
	if(IntrctPtr == nullptr) return 0;
	...
	// TODO: Implement variable DOF version of implicit SS
