...
int radTApplication::MakeManualRelax(int InteractElemKey, int MethNo, int IterNumber, double RelaxParam)
{
	try
	{
		...
		// Legacy non-symmetric Krylov relaxation methods are retired.
		{
			bool validMethod = (MethNo == RadSolverMethod::LU);
...
int radTApplication::MakeAutoRelax(int InteractElemKey, double PrecOnMagnetiz, int MaxIterNumber, int MethNo, const char** arOptionNames, const char** arOptionValues, int numOptions)
{
	...
	try
	{
		...
		if(m_rankMPI <= 0) //OC02012020
		{
			...
			// Surface-charge soft iron (DOF >= 4) is no longer a supported C++ relaxation path.
			// Mesh-backed soft iron belongs to FEEC HDiv-VIM before reaching this function.
			{
				int nElemChk = InteractPtr->GetAmOfMainElem();
				for(int i = 0; i < nElemChk; i++)
				{
					int dd = InteractPtr->GetElementDOF(i);
					if(dd >= 4) { Send.ErrorMessage("Radia::Error203"); return 0; }
				}
			}
...
int radTApplication::SolveGen(int ObjKey, double PrecOnMagnetiz, int MaxIterNumber, int MethNo, const char* image)
{
	...
	try
	{
		...
		// Legacy non-symmetric H-matrix relaxation is retired; the legacy path always
		// builds the dense interaction object. Production magnetic-material solves
		// belong to HDiv-VIM.
...
void radTApplication::ComputeFieldBatch(double* B_out, double* H_out, int n_points,
                                        double* points, int container_handle, bool parallel)
{
	try
	{
		...
		// Per-point evaluation: thread-safe (thread-local radTField, disjoint
		// output indices, B_genComp reads only).
		...
		if(parallel)
		{
			...
			// parallel=true is only valid when NO TaskManager job is running: ngcore
			// CreateJob keeps static job state, so callers inside a job (NGSolve
			// assembly workers) must use parallel=false.
			ngcore::RegionTaskManager rtm(radia::GetMaxThreads());
			ngcore::ParallelFor(ngcore::IntRange(n_points), eval_one);
		}
		else
		{
			for(int i = 0; i < n_points; i++) eval_one((size_t)i);
		}
                                                  ...
                                                  double* points, int container_handle, bool parallel)
{
	try
	{
		...
		if(parallel)
		{
			// Self-wrap so a bare rad.Fld runs parallel without a caller `with TaskManager()`;
			// nested inside one = no-op.  Callers inside a running job use parallel=false
			// (see ComputeFieldBatch).
			ngcore::RegionTaskManager rtm(radia::GetMaxThreads());
			ngcore::ParallelFor(ngcore::IntRange(n_points), eval_one);
		}
		else
