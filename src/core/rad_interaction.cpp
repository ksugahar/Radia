...
void radTInteraction::PrecomputeWedgeGeometry()
{
	...
	for(int w = 0; w < nWedge; w++)
	{
		...
		for(int f = 0; f < 5; f++)
		{
			...
			// Get face vertices and split into triangles.  Warped (non-planar) quad face ->
			// use the REAL vertices (exact 2-triangle geometry); planar faces and the 2 tri
			// faces (never flagged) use the unchanged flattened-polygon reconstruction
			// (=> bit-identical for planar wedge meshes).
			TVector3d V[4];
			TVector3d RV[4]; int rnv = 0;
			if(poly->GetRealFaceVertsIfNonPlanar(f, RV, rnv) && rnv >= 4)
			{
