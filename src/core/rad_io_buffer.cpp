...
string radTIOBuffer::err_ar[] = {
	...
	"Radia::Error041::::Incorrect input: Method number should be 0 (LU), 1 (BiCGSTAB), or 2 (BiCGSTAB+HACApK).\0",
	...
	"Radia::Error052::::Incorrect input: Improper definition of the subdivision parameters.\0",
	...
	"Radia::Error203::::The retired EIEM2 collocation surface-charge path is not available. Current hexahedron / wedge / pyramid soft iron uses the parameter-free multipole-moment MMM solver; FEEC HDiv-VIM remains available through radia.vim.soft_iron_from_mesh(...) and radia.vim.hdiv_demag_solve(...).\0",
	"Radia::Error204::::Incorrect input: a single demag Solve mixes the 3-DOF MMM dipole element (RecMag) with MSC face-charge elements (tetrahedron / wedge / pyramid / hexahedron, 4-6 DOF). These use different formulations and must be solved as separate containers. Permanent magnets and pure-RecMag / pure-tet / pure-wedge / pure-hex solves are unaffected.\0",
	"Radia::Error205::::Incorrect input: B-input hysteresis (b_input_newton / b_input_hantila) requires EVERY element to be a hysteresis material (MatPlayHysteresis / MatEnergyHysteresis). B-input is supported for the 3-DOF MMM dipole element (RecMag, dense Newton) AND, when ALL elements are moment/MSC face-charge (tetrahedron / wedge / pyramid / hexahedron, 4-6 DOF), through the MMMM moment Picard. Mixing a moment/MSC element with a non-hysteresis (linear / nonlinear-BH) material under B-input is not supported; apply a hysteresis material to all elements or solve them as separate containers.\0",
