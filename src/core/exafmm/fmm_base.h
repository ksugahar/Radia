...
namespace exafmm_t {
  ...
  public:
    ...
    void kernel_matrix(RealVec& src_coord, RealVec& trg_coord, std::vector<T>& matrix) {
      ...
      ngcore::ParallelFor(ngcore::IntRange(nsrcs), [&](size_t i) {
    ...
    void P2P(NodePtrs<T>& leafs) {
      ...
      ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(targets.size())), [&](size_t i) {
    ...
    RealVec verify(NodePtrs<T>& leafs, bool sample=false) {
      ...
      ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(targets2.size())), [&](size_t i) {
