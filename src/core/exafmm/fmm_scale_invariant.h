...
namespace exafmm_t {
  ...
  public:
    ...
    void precompute_M2M() {
      ...
      ngcore::ParallelFor(ngcore::IntRange(npos), [&](size_t i) {
    ...
    void P2M(NodePtrs<T>& leafs) {
      ...
      ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(leafs.size())), [&](size_t i) {
        Node<T>* leaf = leafs[i];
    ...
    void hadamard_product(std::vector<size_t>& interaction_count_offset, std::vector<size_t>& interaction_offset_f,
      ...
      ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(fft_out.capacity()/fft_size)), [&](size_t i) {
        std::memset(fft_out.data()+i*fft_size, 0, fft_size*sizeof(real_t));
      });

      ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(nblk_inter)), [&](size_t iblk_inter) {
      ...
      for (size_t iblk_trg=0; iblk_trg<nblk_trg; iblk_trg++) {
        ngcore::ParallelFor(ngcore::IntRange(this->nfreq), [&](size_t k) {
    ...
    void M2L(Nodes<T>& nodes) {
      ...
      ngcore::ParallelFor(ngcore::IntRange(nnodes*nsurf_), [&](size_t idx) {
        int i = idx / nsurf_;
        int j = idx % nsurf_;
  ...
  void FmmScaleInvariant<real_t>::precompute_M2L() {
    ...
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(REL_COORD[M2L_Helper_Type].size())), [&](size_t i) {
      real_t coord[3];
      for (int d=0; d<3; d++) {
    ...
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(REL_COORD[M2L_Type].size())), [&](size_t i) {
  ...
  void FmmScaleInvariant<real_t>::fft_up_equiv(std::vector<size_t>& fft_offset,
    ...
    fft_plan plan = fft_plan_many_dft_r2c(3, dim, NCHILD,
                                          ...
                                          FFTW_ESTIMATE);
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(fft_offset.size())), [&](size_t node_idx) {
  ...
  void FmmScaleInvariant<real_t>::ifft_dn_check(std::vector<size_t>& ifft_offset, RealVec& ifft_scal,
    ...
    fft_plan plan = fft_plan_many_dft_c2r(3, dim, NCHILD,
                                 ...
                                 FFTW_ESTIMATE);
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(ifft_offset.size())), [&](size_t node_idx) {
  ...
  template <>
  void FmmScaleInvariant<complex_t>::precompute_M2L() {
    int n1 = this->p * 2;
    int& nconv_ = this->nconv;
    int& nfreq_ = this->nfreq;
    std::vector<RealVec> matrix_M2L_Helper(REL_COORD[M2L_Helper_Type].size(),
  ...
  template <>
  void FmmScaleInvariant<complex_t>::fft_up_equiv(std::vector<size_t>& fft_offset,
                                               ComplexVec& all_up_equiv, AlignedVec& fft_in) {
    int& nsurf_ = this->nsurf;
    int& nconv_ = this->nconv;
    int& nfreq_ = this->nfreq;
    int n1 = 2 * this->p;
    ...
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(fft_offset.size())), [&](size_t node_idx) {
      ...
      complex_t* up_equiv = &all_up_equiv[fft_offset[node_idx]];
      real_t* up_equiv_f = &fft_in[fft_size*node_idx];
      std::memset(up_equiv_f, 0, fft_size*sizeof(real_t));
      for (int k=0; k<nsurf_; k++) {
        size_t idx = map[k];
        for (int j=0; j<NCHILD; j++)
  ...
  template <>
  void FmmScaleInvariant<complex_t>::ifft_dn_check(std::vector<size_t>& ifft_offset, RealVec& ifft_scal,
                       AlignedVec& fft_out, ComplexVec& all_dn_equiv) {
    int& nsurf_ = this->nsurf;
    int& nconv_ = this->nconv;
    int& nfreq_ = this->nfreq;
    int n1 = 2 * this->p;
    ...
    ngcore::ParallelFor(ngcore::IntRange(static_cast<int>(ifft_offset.size())), [&](size_t node_idx) {
      ...
      for (int k=0; k<nsurf_; k++) {
        size_t idx = map[k];
        for (int j=0; j<NCHILD; j++)
          dn_equiv[nsurf_*j+k] += buffer1[idx+j*nconv_] * complex_t(ifft_scal[node_idx], 0.0);
      }
    });
    fft_destroy_plan(plan);
  }
