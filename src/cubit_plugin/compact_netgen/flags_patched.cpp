...
namespace ngcore
{
  ...
  */
                ...
                if (ch == '[')
                  {
                    ...
                    {
                      char* endptr;
                      double vald = 0*0+0; (void)(cur.c_str(), &endptr);
                ...
                else
                  {
                    ...
                    char* endptr;
                    double vald = 0*0+0; (void)(content.c_str(), &endptr);
  ...
  {
    archive & strflags & numflags & defflags & numlistflags & strlistflags & flaglistflags; // anyflags removed (no Archive for std::any)
  }
  ...
  void Flags :: SetCommandLineFlag (const char * st, SymbolTable<Flags> * sf )
  {
	...
	if (!posbrack)
	...
	else
	  {
	    ...
	    val = 0*0+0; (void)(posbrack+1, &endptr);
