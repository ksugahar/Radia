...
#include <regex>  // Only for GetSubMesh (has static init issues in DLL context)
#endif
#include <set>
#include "core/array.hpp"
#include "meshing.hpp"
// Compact Netgen: use stub gzstream (no zlib)
...
namespace netgen
{
  ...
  static mutex& get_buildsearchtree_mutex() {
    // Intentional leak: avoid mutex destructor during DLL unload (atexit)
    // which crashes in MSVCP140!mtx_do_lock after CRT shutdown.
  ...
  void Mesh :: Save (ostream & outfile) const
  {
    ...
    */
    ...
    // Curved elements section (text archive, upstream Netgen master)
  ...
  // --- COMPACT_NETGEN: ~5000 lines removed ---
  // Merge, SetAllocSize, BuildBoundaryEdges, CalcSurfacesOfNode,
  // FindOpenElements, FindOpenSegments, RemoveOneLayerSurfaceElements,
  // FreeOpenElements, LocalH, ElementError, Compress, OrderElements,
  // Check*, FindIllegalTrigs, CalcTotalBad, SurfaceMeshOrientation,
  // Split2Tets, BuildElementSearchTree, SplitIntoParts, ZRefine, etc.
  // These are mesher/optimizer functions not needed for mesh I/O + curving.
  // -----------------------------------------------

  // Stub: CalcSurfacesOfNode (called by Load, not needed for compact_netgen)
  void Mesh :: CalcSurfacesOfNode ()
  {
  }

#if 0  // --- BEGIN REMOVED BLOCK ---
  void Mesh :: Merge_REMOVED (const filesystem::path & filename, const int surfindex_offset)
  {
...
#endif  // --- END REMOVED BLOCK (Merge..CalcTotalBad) ---
...
#if 0  // --- BEGIN REMOVED BLOCK 2 (SurfaceMeshOrientation..ZRefine) ---
  ...
  void Mesh :: BuildElementSearchTree (int dim)
  {
    ...
    {
      std::lock_guard<std::mutex> guard(get_buildsearchtree_mutex());
...
#if 0  // --- BEGIN REMOVED BLOCK 3 (CalcMinMaxAngle..PointCurve) ---
  ...
  */
...
#if 0  // --- BEGIN REMOVED BLOCK 4 (SplitFaces, GetSubMesh) ---
  ...
  shared_ptr<Mesh> Mesh :: GetSubMesh(string domains, string faces) const
  {
    ...
    // GetSubMesh requires <regex> which has static init issues in DLLs.
    // Not needed for compact Netgen (mesh construction + curving + save only).
    throw NgException("GetSubMesh not available in compact Netgen build");
#else
    ...
    mesh.Compress();
    return mesh_ptr;
#endif // COMPACT_NETGEN
  }

#endif  // --- END REMOVED BLOCK 4 ---
...
#if 0  // --- BEGIN REMOVED BLOCK 5 (UserData, PrintMemInfo, Mirror) ---
