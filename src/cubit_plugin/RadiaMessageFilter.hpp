...
// the filter is active only during export command execution.
//
// Preserves:
...
class LearnEditionFilter : public CubitMessageHandler {
 public:
  ...
  void print_error(const char* message) override {
    ...
    // (2) "Unable to find entities to write" — spurious during export.
    ...
    // when the 50k-cap trap fires.  export does NOT use that dispatch
    ...
    // This filter is RAII-scoped to export execute() only, so builtin
...
// Use at the top of each export command's execute().
