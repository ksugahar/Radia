...
**2-Path .vol Generation** (either path produces the same `.vol`):
1. **Path A** (C++): Cubit -> `export netgen "model.vol" order N` -> `.vol` with labels + curving
2. **Path B** (Python): Cubit -> `cub5_to_vol.py --cub5 model.cub5 --order N` -> `.vol`

Path A and Path B must produce **identical** .vol for tet meshes.
Path B is maintained as: (a) debugging/verification reference, (b) Joachim demo, (c) Cubit-free .vol generation from .cub5.
...
- Used for: Path A/B comparison tests, Joachim demo, standalone .vol generation
- NOT called by Cubit panels (panels use `export netgen` APREPRO command)
