...
S:\Radia\01_GitHub\
  ...
  validation_test/         # heavier validation; hibino first, mdx only behind idle CI
  docs/
...
- `radia-optuna` is the explicit exception: Optuna is a generic, independently
  ...
  separate wheel stages that tree plus the 21-command `optuna_mex`. The audited
...
| **DtN / FEM-Kelvin operator** (compute core) | the FEM-Kelvin sparse generator of the layered (Sommerfeld-type) Green's operator -- a **core** capability, NOT an application. The historical research acts are summarized with embedded results in `docs/kelvin/kelvin_dtn_spectrum_archive.ipynb`; checked evidence lives in `validation_test/kelvin_dtn_spectrum/`, and reusable pieces promote into `src/radia/open_boundary` (like `hdiv_vim` did). **Two write-up tracks (2026-06-15):** Track A = this DtN+Kelvin core (the SA/Hachinohe paper: DtN-spectrum datasheet, sparse Kelvin open boundary, Sommerfeld isomorphism/surrogate, the directly-assembled material-aware DtN matrix, FEM-condensed≠BEM); Track B = its use as the **stream-function coil-design** kernel with iron (SEPARATE paper, see the Stream-function domain row + `HANDOFF_sommerfeld_dtn_kelvin_streamfunction.md`). | `docs/kelvin/kelvin_dtn_spectrum_archive.ipynb`, `validation_test/kelvin_dtn_spectrum/`, `packages/radia-mcp/tests/test_dtn_*` |
...
coefficients are not nodal values), and report `T_max`/`T_min` from
...
archived `bench_fmm_vs_aca_biotsavart.py` (summary in the result-bearing
`docs/kelvin/kelvin_dtn_spectrum_archive.ipynb`; maintained evidence under
`validation_test/kelvin_dtn_spectrum/`)
