...
- Mixed Galerkin admittance in its projected form: `radia.maglev.mixed_galerkin.
  ...
  block system per frequency, so the crossover between the Foster ladder and
  the sqrt(s) tail is decided by the Schur complement instead of being added.
  The coupling integrals are taken on a tensor quadrature grid graded to the
  skin depth on which the finite-element modes are sampled once.  On the
  copper cube it is exact at DC, agrees with the closed-form analytic-sine
  mixed Galerkin of `validation_test/mixed_galerkin/cube3d` to 0.3 % from
  1 Hz to 1 GHz, and sits 0.35 % above the Aitken Foster reference at 10 kHz
  where the additive `Y_mixed` is 53 % off.  `Y_mixed` / `Y_matrix_mixed`
  stay as the additive form behind the LTI export and say so.
- Fixed the drive projection of the bulk Foster spectrum
  (`bulk_foster_via_eigen`, `bulk_foster_matrix_via_eigen`): the projection
  used the free-dof block of the mass matrix, which drops the boundary
  hat-function part of a drive that does not vanish on the surface (the
  constant 1, the centred coordinates).  On the cube at maxh = L/6 the first
  overlap b_1 was 18 % low and the first residue g_1 34 % low; the bias
  shrinks like the mesh size.  The projection now uses the full mass matrix
  and equals `Integrate(phi_n * f)` to round-off.  Committed demo results
  under `docs/maglev/demos` are regenerated.
...
- Added `radia-optuna` as a separately installable Windows x64 wheel while
  ...
  20-command `optuna_mex`, and the generated Optuna 4.9.0 compatibility
  contracts without Radia solver, NGSolve, or MKL dependencies. The package is
  independently versioned from Radia; `pip install "radia[optuna]"` pins the
  standalone release that passed Radia's integration gate, while
  `radia[optuna-upstream]` adds the heavy pinned Python oracle stack.
  Generic `SimulinkRunner` operation is part of the standalone contract;
  Radia-specific application models remain in Radia.
  Wheel verification rejects missing files, wrong platform tags, extra MEX
  gateways, and leaked solver/runtime binaries. An isolated installation passed
  `radia-optuna-doctor` and a 20-trial seeded MATLAB TPE study after adding only
  the wheel's MATLAB directory.
- Split native optimization from the solver gateway. The 20-command,
  151552-byte `optuna_mex` contains the checked MT19937, Pareto, Parzen, and
  fused TPE kernels without NGSolve, oneMKL, Radia-core, or Python dependencies;
  `radia_mex` now contains 363 non-Optuna commands. MATLAB loads the optimizer
  gateway only when optimization is used, and a missing or non-20-command
  `optuna_mex` fails loudly instead of redirecting through `radia_mex` or a
...
- Brought warmed sequential MATLAB TPE throughput past upstream Optuna 4.9.0
  ...
  0.198069 s versus 0.327160 s. Public tables are now lazy views over typed
  study columns, hot-path timestamps avoid `datetime` construction, and four
  fused native TPE commands reuse the checked NumPy-compatible random-state
  handle. Both workloads preserve the exact upstream seeded checksum.
- Added incremental completed-trial history to the checked TPE random-state
  handle and one-call generation of all grouped conditional search spaces.
  The native fused path is restricted to the exact supported sequential
  in-memory contract; storage, pruning, constraints, constant liar, custom
  weighting, and categorical-distance callbacks use the high-level MATLAB
  orchestration path above the required native gateway. This removes repeated
  full-history projection without changing the upstream seed sequence.
- Made Optuna 4.9.0 complete-compatibility work finite and machine-auditable:
  sampler defaults now implement upstream `seed=None` private-entropy semantics,
  while explicit seeds retain exact proposal parity. Added a byte-stable public
  API/member inventory and generated MATLAB coverage ledger whose missing,
  partial, and unmapped counts block any complete-compatibility claim.
- Aligned the remaining core study-object lifecycle: imported WAITING and
  RUNNING trials are accepted, WAITING trials are claimed by `ask` without
  losing their number, parameters, or attributes, empty persisted studies are
  visible immediately, and study listings now return `StudySummary` objects.
  Added the upstream BaseTrial, BaseSampler, and BasePruner type hierarchy and
  the checked BasePruner `prune` interface.
- Matched QMC's public warning controls: unseeded scrambled sequences report
  the same asynchronous-worker risk, dynamic/categorical fallback reports
  independent sampling only after prior trials exist, and both warnings can be
  disabled exactly as in Optuna 4.9.0.
- Matched TPE's conditional-space controls: `Group=true` now performs the
  upstream co-occurrence decomposition and uses the global good/bad trial split
  before projecting each group, independent fallback obeys the upstream
  warning control, and `CategoricalDistanceFcn` applies Optuna 4.9.0's
  distance-weighted categorical Parzen kernels. Both proposal sequences are
  generated by the pinned upstream oracle.
- Moved the NumPy-compatible MT19937 stream behind a checked native MEX handle
  with exact snapshot/restore. Reduced in-memory TPE overhead by skipping non-persistable sampler
  snapshots and private relative-search-space attributes, avoiding empty table
  deletions, batching trial completion updates, and constructing `FrozenTrial`
  only when `tell` has an output. All 95 Optuna MATLAB tests still pass; seeded
  TPE checksums remain identical to upstream 4.9.0.
- Replaced three Optuna-shaped MATLAB-only contracts with upstream behavior:
  `Study.optimize` now catches `TrialPruned`, `tell` accepts a trial number,
  and `get_trials` / `best_trials` return `FrozenTrial` objects rather than
  tables. `FrozenTrial` now exposes the checked suggestion, report,
  should-prune, and attribute surface, and `create_trial` complements the
  camel-case factory. Public documentation now calls this a differentially
  verified Optuna 4.9.0 subset, not a drop-in implementation.
- Replaced MATLAB's sampler-local random streams with a NumPy legacy
  `RandomState`-compatible MT19937 implementation and locked explicit-seed
  proposal sequences to a regenerable upstream Optuna 4.9.0 oracle. Coverage
  includes Random, scalar/mixed/multivariate/multi-objective TPE, Grid,
  scrambled and unscrambled Sobol/Halton QMC, PartialFixed, fixed and
  conditional BruteForce, CMA-ES, NSGA-II, NSGA-III, all six built-in
  crossovers, GP acquisition after startup, pruning, distribution warnings,
  and constrained Pareto behavior. GP now defaults to a pinned Optuna 4.9.0
  Python backend so its PyTorch/SciPy LogEI proposals are exact; the previous
  toolbox-free GP remains an explicit MATLAB-integration backend. Multivariate
  TPE now uses Optuna's shared-component product
  mixture for both candidate generation and joint log-density acquisition.
  Callable TPE gamma/weights and CMA-ES independent-sampler controls now share
  the same seeded differential oracle, including the multi-objective path.
- Added `FixedTrial`, MAT-backed load/copy/delete/list study management,
  upstream fANOVA/MDI/PED-ANOVA parameter importance, maximum-trial and
  stagnation termination callbacks, constraint-aware exact GP acquisition,
  and deterministic GP history replay after MAT-file reload.
- Aligned Study defaults and ask/tell behavior with Optuna 4.9.0, including
  anonymous names, default TPE/NSGA-II selection, fixed distributions,
  PRUNED/FAIL snapshots, and infinite COMPLETE objectives. Scrambled QMC now
  uses `scipy.stats.qmc`, and evolutionary samplers persist their independent
  RandomSampler state separately from crossover/optimizer state. Enqueued
  out-of-range fixed values now warn but remain fixed, as upstream Optuna does.
  Same-kind numeric distribution changes now warn and reuse the first value,
  stepped highs are normalized in public metadata, and parameter names that
  collide after MATLAB field sanitization remain distinct public parameters.
- Added a second differential oracle generated through the official
  `optuna/optuna-mcp` 0.2.0 server over stdio. It locks the public
  Study/Trial MCP contract for fixed-distribution `ask`, `tell`, trial
  attributes, imported trials, metric names, best-trial selection, and
  multi-objective Pareto fronts while the direct Optuna oracle retains the
  seeded sampler sequences that optuna-mcp does not expose.
- Reclassified every `test_optuna*.m` function in a checked manifest as an
  upstream Python differential, official MCP differential, or MATLAB-only
  integration test. Removed handwritten sampler sequences, quality bands,
  dominance rules, pruning decisions, and private-state goldens as parity
  authorities; MATLAB storage, parallel, native, and Simulink checks remain
  explicitly non-parity integration coverage.
- Added radia-mcp compatibility/audit tools and version-3 MATLAB optimization
  code generation for every supported sampler, structured options, TPE
