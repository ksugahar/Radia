...
[Eqnedit64 GitHub Release](https://github.com/ksugahar/Radia/releases/tag/eqnedit64-v3.0.12)
...
同じ数式行区切り`\\`を作り、単一行なら`aligned`で包みます。構造を変えないraw改行は
`Shift+Enter`です。保存ファイルではこの`aligned`を外側の`equation`に収めます。
