...
if ($LASTEXITCODE -ne 0) {{ exit $LASTEXITCODE }}
  ...
  try {{
    ...
    if (-not (Test-Path -LiteralPath $python)) {{
      ...
      if ($LASTEXITCODE -ne 0) {{ exit $LASTEXITCODE }}
      & $python -m pip install --disable-pip-version-check --quiet numpy scipy pytest pyyaml setuptools 'mcp>=1.0,<2'
      if ($LASTEXITCODE -ne 0) {{ exit $LASTEXITCODE }}
    }}
    & $python -c "import mcp, numpy, pytest, scipy, setuptools, yaml"
