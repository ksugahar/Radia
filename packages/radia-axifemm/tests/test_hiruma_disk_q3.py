"""Phase A3: Hiruma 3-term on Cu disk with Q3 quad mesh.

Goal: push BEM gap below 0.1% (Q1 gives 0.55%, Q2 gives 0.27%).
"""
import json
import os
from math import pi
...
def solve_disk_q3(NR_disk, Nz_disk, NR_air, Nz_air, R_air, Z_air, N_stages=6,
                  label=""):
    print(f"\n=== Q3 quad: NR_d={NR_disk} Nz_d={Nz_disk} NR_a={NR_air} "
          f"Nz_a={Nz_air} R_air={R_air*1e3}mm  {label} ===")
    ngsglobals.msg_level = 0
    mesh = make_structured_disk_quad_mesh(NR_disk, Nz_disk, NR_air, Nz_air,
                                          R_air, Z_air)
    fes = H1Henrotte(mesh, order=3, dirichlet="axis|right|top|bot")
...
def main():
    # Q3 has 16 DOFs/quad. Coarser mesh than Q2 yields similar DOF budget.
    cases = [
        (8,  4,  6,  6, 100e-3, 100e-3, "coarse"),
        (16, 8,  10, 10, 200e-3, 200e-3, "medium"),
        (32, 16, 12, 12, 500e-3, 500e-3, "fine"),
    ]
    all_res = {}
    for NR_d, Nz_d, NR_a, Nz_a, R_a, Z_a, lbl in cases:
        all_res[lbl] = solve_disk_q3(NR_d, Nz_d, NR_a, Nz_a, R_a, Z_a,
                                     N_stages=6, label=lbl)

    print("\n" + "="*78)
    print("Summary: Q3 quad mesh vs BEM-Foster (Mathematica reference 224.307 us)")
    print("="*78)
    print(f"{'case':<10} {'ne':>8} {'free':>6}  "
          f"{'tau_1 eigsh':>11}  {'/BEM':>7}  {'gap%':>7}")
    for lbl, r in all_res.items():
        t1 = r["eigsh_tau_us"][0]
        ratio_bem = t1 / BEM_TAU[0]
        gap_pct = (1.0 - ratio_bem) * 100
        print(f"{lbl:<10} {r['mesh']['ne']:>8} {r['mesh']['free']:>6}  "
              f"{t1:>11.4f}  {ratio_bem:>7.4f}  {gap_pct:>+7.3f}")

    out_path = os.path.join(os.path.dirname(__file__),
                            "test_hiruma_disk_q3_results.json")
    with open(out_path, "w") as fp:
        json.dump({"results": all_res, "bem_tau_us": BEM_TAU}, fp, indent=2)
    print(f"\nSaved: {out_path}")
