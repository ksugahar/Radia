...
add_block("simulink/Signal Routing/Bus Selector", ...
    blockModelName + "/Select Monitor", ...
    OutputSignals="attempted_trials,failed_trials,pruned_trials", ...
    Position=[430 225 445 315]);
add_block("simulink/Signal Routing/Mux", ...
    blockModelName + "/Monitor Vector", Inputs="3", ...
    Position=[500 225 510 315]);
add_block("simulink/Sinks/To Workspace", ...
    blockModelName + "/Monitor", ...
    VariableName="wheel_monitor", SaveFormat="Array", ...
    Position=[550 255 650 285]);
...
add_line(blockModelName, "Optuna Study/5", "Select Monitor/1");
for monitorIndex = 1:3
    add_line(blockModelName, "Select Monitor/" + monitorIndex, ...
        "Monitor Vector/" + monitorIndex);
end
add_line(blockModelName, "Monitor Vector/1", "Monitor/1");
...
resumedMonitor = resumedOutput.get("wheel_monitor");
assert(resumedAttempted(end) == 6);
assert(resumedMonitor(end,1) == 6);
...
result = struct( ...
    ...
    "simulink_monitor_bus", "OptunaMonitorBusV1", ...
