...
def test_matlab_path_names_the_layout_it_resolved():
    ...
    # setup.py stages <repo>/matlab into the package only during a wheel
    # build, so an editable install has to resolve the checkout instead of
    # silently pointing MATLAB at a directory that does not exist.
    ...
    if resolved == "checkout":
        assert "editable install" in payload["hint"]
    else:
        assert payload["hint"] == ""
