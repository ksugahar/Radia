...
_LAYOUT_HINTS = {
    "wheel": "",
    "checkout": (
        "Resolved the monorepo checkout: an editable install has no staged "
        "MATLAB tree, so radia-optuna is reading <repo>/matlab directly."
    ),
    "missing": (
        "No MATLAB tree found. setup.py stages <repo>/matlab into the "
        "package only during a wheel build (python -m build --wheel), so an "
        "editable install outside the monorepo has nothing to point MATLAB "
        "at."
    ),
}
