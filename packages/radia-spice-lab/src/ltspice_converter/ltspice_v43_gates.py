...
def include_library_identity_ok(run: Mapping[str, object]) -> bool:
    """Check deterministic include search, model shadowing, and netlist owner."""

    fields = (
        "include_path", "library_section", "search_path", "selected_model",
        "shadowed_model", "parameter_scope", "expanded_netlist_owner",
    )
    return (
        bool(str(run.get("include_generation_id") or ""))
        and bool(str(run.get("include_path") or ""))
        and bool(str(run.get("library_section") or ""))
        and isinstance(run.get("search_path"), Sequence)
        and len(run.get("search_path") or []) >= 2
        and bool(str(run.get("selected_model") or ""))
        and bool(str(run.get("shadowed_model") or ""))
        and run.get("selected_model") != run.get("shadowed_model")
        and bool(str(run.get("parameter_scope") or ""))
        and bool(str(run.get("expanded_netlist_owner") or ""))
        and all(run.get(f"parsed_{field}") == run.get(field) for field in fields)
        and _sha256(run.get("netlist_sha256"))
        and run.get("parsed_netlist_sha256") == run.get("netlist_sha256")
        and _sha256(run.get("result_sha256"))
        and run.get("parsed_result_sha256") == run.get("result_sha256")
    )


def raw_binary_identity_ok(run: Mapping[str, object]) -> bool:
    """Check RAW byte order, variable table, complex layout, and step owner."""

    return (
        str(run.get("byte_order") or "") == "little"
        and isinstance(run.get("variable_table"), Sequence)
        and len(run.get("variable_table") or []) >= 2
        and len(set(run.get("variable_table") or [])) == len(run.get("variable_table") or [])
        and str(run.get("complex_trace_layout") or "") == "real_imag_interleaved"
        and int(run.get("stepped_run_index") or -1) >= 0
        and int(run.get("sample_count") or 0) > 0
        and bool(str(run.get("result_owner") or ""))
        and bool(str(run.get("netlist_owner") or ""))
        and all(run.get(f"parsed_{field}") == run.get(field) for field in (
            "byte_order", "variable_table", "complex_trace_layout",
            "stepped_run_index", "sample_count", "result_owner", "netlist_owner",
        ))
        and _sha256(run.get("raw_sha256"))
        and run.get("parsed_raw_sha256") == run.get("raw_sha256")
        and _sha256(run.get("netlist_sha256"))
        and run.get("parsed_netlist_sha256") == run.get("netlist_sha256")
    )
