...
# ----------------------------------------------------------------------
# Netgen Vol runner — exports + runs standalone check-vol subprocess and
...
def _run_netgen_export(cubit_mod, parent):
    ...
    if python == "py":
        args.append("-3")
    report_path = vol_path + ".check.json"
    if os.path.isfile(report_path):
        os.remove(report_path)
    args += ["-m", "cubit_mesh_export.check", vol_path,
             "--format", "json", "--report-json", report_path]
...
def install_menu():
    ...
    # Claro owns the native actions; release them before Python/SWIG teardown.
    # Leaving them registered caused Windows fast-fail 0xC0000409 on GUI exit.
    app = QApplication.instance()
