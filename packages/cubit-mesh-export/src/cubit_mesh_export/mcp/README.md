# `cubit_mesh_export.mcp` — Cubit mesh scripting MCP server
...
`cubit_topics` for discovery; no Radia catalog or installation is required. For an
...
a GUI. Radia topology workflows may consume this package in the other direction.
...
Claude Code (MCP client)
    → mcp-server-cubit (system Python 3.12)
        → batch: daemon.py (Cubit bundled Python, stdio JSON-RPC)
...
- `src/cubit_mesh_export/mcp/server.py` — tool registration, instructions,
  annotation presets
- `src/cubit_mesh_export/mcp/session.py` + `daemon.py` (batch stdio) — persistent session
- `src/cubit_mesh_export/mcp/probe_ops.py` — SHARED probe queries for both
  runners (summary/quality/per_volume/entities/labels)
- `src/cubit_mesh_export/mcp/label_audit.py` — cubit-free block/sideset
  convention audit (shared by `probe("labels")` and tests)
- `src/cubit_mesh_export/mcp/knowledge/` — sub-modules (export,
  netgen_workflow, scripting, cpp_sdk, custom_toolbar, ...)
- `src/cubit_mesh_export/mcp/vol_inventory.py`, `gmsh_v41.py`, `*_gate.py`
  — export gates
