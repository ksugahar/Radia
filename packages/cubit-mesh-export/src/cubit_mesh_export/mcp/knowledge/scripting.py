...
| `export nastran_bdf "f.bdf" order N` | NetgenCurver + ACIS | 1-2 |
...
## 9. HIGH: Mixed Hex-Tet Mesh for Consumers Without Pyramid Support

```python
# WRONG for consumers that cannot read CPYRAM elements
cubit.cmd("block 1 add hex all")
cubit.cmd("block 1 add tet all")
cubit.cmd("block 1 add pyramid all")
cubit.cmd('export nastran_bdf "mesh.bdf" overwrite')  # Pyramids as CPYRAM!

# RIGHT: use nopyramid when the consumer lacks CPYRAM support
...
elements at hex-tet interfaces. Some consumers cannot read
standard pyramid elements — use `PYRAM=False` for Nastran export.

## Consumers Without CPYRAM Support

For those consumers, write degenerate hex pyramids instead of CPYRAM elements:

```python
# For consumers without CPYRAM: use nopyramid
cubit.cmd('export nastran_bdf "mesh.bdf" nopyramid overwrite')
```

With `nopyramid`, pyramid elements are written as degenerate CHEXA
(8-node hex with repeated nodes) instead of CPYRAM (5-node), which such consumers
can read.
...
- `cubit.cmd('export nastran_bdf "file.bdf" dimension 2 overwrite')`
...
## Problem: A Consumer Cannot Read Nastran Pyramid Elements

**Cause**: The receiving solver does not support standard CPYRAM elements.
...
**Symptom**: Exported .bdf uses Cubit's built-in format instead of the exporter
format with order 2 / nopyramid support.

**Cause**: Cubit has a built-in `export nastran` command. The package command
is `export nastran_bdf` to avoid the naming conflict.

**Fix**: Always use `export nastran_bdf`:
```python
...
The production exporters are native `.ccm` commands. They read Cubit's mesh and
write each interchange format directly; the Python/MCP layer invokes those
commands and validates their artifacts rather than maintaining duplicate writers.

## How It Works

```
Cubit mesh database                 Output file
  blocks / sidesets / nodes ──> native exporter ──> .vol/.msh/.vtk/.bdf/...
```

Each command reads the current model, applies its checked order/label contract,
and writes the requested format. Python does not provide a second exporter.
...
```python
# RIGHT: cubit-mesh-export APREPRO commands
cubit.cmd('export gmsh "mesh.msh" overwrite')
cubit.cmd('export nastran_bdf "mesh.bdf" dimension 3 overwrite')
cubit.cmd('export vtk "mesh.vtk" overwrite')
cubit.cmd('export netgen "mesh.vol" order 3 overwrite')

# WRONG CONTRACT: Cubit's built-in writer is not this BDF interchange route
...
for n in [2, 4, 8, 16]:  # mesh refinement levels
    ...
    cubit.cmd(f'export nastran_bdf "{folder}/sphere.nas" dimension 3 overwrite large')
```
...
for mesh_size in [5.0, 2.0, 1.0, 0.5]:
    ...
    cubit.cmd(f'export nastran_bdf "{folder}/mesh.bdf" dimension 2 overwrite large')
    cubit.cmd('exit')
```
...
cubit.cmd('export nastran_bdf "mesh.nas" dimension 3 overwrite everything')
...
The `export nastran_bdf` command handles this with the `nopyramid` option:

```
export nastran_bdf "output.bdf" nopyramid   # writes CHEXA instead of CPYRAM
```
...
UTF-8.  This is a Cubit core limitation; cubit-mesh-export cannot fix it.
...
- `cubit_docs topic=license_per_user` -- official activation boundary.
