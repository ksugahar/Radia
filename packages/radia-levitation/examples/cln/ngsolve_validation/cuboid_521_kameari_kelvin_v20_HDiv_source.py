...
print("Starting v20 (HDiv-projected J source for J·n=0 strict)...",
      flush=True)
...
def main():
    ...
    # === HDiv-projected J source for J·n=0 strict (v20) ===
    # HDiv basis on conductor with dirichlet="conductor_surface" gives
    # J·n = 0 STRICTLY as an essential BC (basis-level constraint).
    # The L^2 projection of sigma * A_s_phys onto this space yields
    # gfJ_imp satisfying J·n=0 at all 6 cuboid faces.
    print("  Building HDiv source space (J·n=0 strict)...", flush=True)
    t0 = time.time()
    fes_J = HDiv(mesh, order=ORDER,
                  dirichlet="conductor_surface",
                  definedon=mesh.Materials("conductor"))
    print(f"  HDiv ndof (conductor only, J·n=0 strict) = {fes_J.ndof}",
          flush=True)
    ...
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v20_HDiv_source.json"
