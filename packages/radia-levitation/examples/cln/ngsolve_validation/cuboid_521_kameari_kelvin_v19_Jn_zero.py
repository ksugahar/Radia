...
print("Starting v19 (A_s Coulomb correction for J·n=0)...", flush=True)
...
def main():
    ...
    # === Coulomb correction for J·n=0 at conductor surface (v19) ===
    # Solve a global Poisson with Neumann surface source -A_s·n on cond_surf.
    # The resulting phi makes (A_s + grad(phi))·n = 0 from the conductor side
    # at conductor surface (so J = sigma * (A_s + grad(phi)) satisfies J·n=0).
    print("  Coulomb correction for J·n=0...", flush=True)
    t0 = time.time()
    fes_phi_corr = Periodic(H1(mesh, order=ORDER, dirichlet_bbnd="GND"))
    phi_corr, psi_corr = fes_phi_corr.TnT()
    a_phi_corr = BilinearForm(fes_phi_corr)
    a_phi_corr += grad(phi_corr) * grad(psi_corr) * dx(bonus_intorder=BONUS_INT)
    a_phi_corr += PHI_SHIFT * phi_corr * psi_corr * dx(bonus_intorder=BONUS_INT)
    n_normal = specialcf.normal(3)
    # Use analytical A_s_phys_cf at boundary (A_s_cf is MaterialCF, VOL-only).
    # On conductor surface A_s_cf == A_s_phys_cf since the surface is in
    # the inner physical region (not Kelvin). A_s_phys is a global CF.
    A_s_dot_n_bnd = A_s_phys_cf * n_normal
    f_phi_corr = LinearForm(fes_phi_corr)
    f_phi_corr += -A_s_dot_n_bnd * psi_corr.Trace() * ds("conductor_surface",
                                                          bonus_intorder=BONUS_INT)
    with TaskManager():
        a_phi_corr.Assemble()
        f_phi_corr.Assemble()
        try:
            inv_phi_corr = a_phi_corr.mat.Inverse(fes_phi_corr.FreeDofs(),
                                                    inverse="pardiso")
        except Exception:
            inv_phi_corr = a_phi_corr.mat.Inverse(fes_phi_corr.FreeDofs(),
                                                    inverse="sparsecholesky")
        gf_phi_corr = GridFunction(fes_phi_corr)
        gf_phi_corr.vec.data = inv_phi_corr * f_phi_corr.vec
    print(f"    {time.time()-t0:.1f}s", flush=True)

    # Build corrected A_s as an HCurl GridFunction
    gfA_s_corrected = GridFunction(fes, name="A_s_corrected")
    with TaskManager():
        gfA_s_corrected.Set(A_s_cf + grad(gf_phi_corr),
                            bonus_intorder=BONUS_INT)

    # Diagnostic: verify (A_s_corrected · n) at conductor surface is small
    A_s_corr_n_int = float(Integrate(
        (gfA_s_corrected * n_normal) ** 2
        * ds("conductor_surface", bonus_intorder=BONUS_INT), mesh))
    A_s_n_orig_int = float(Integrate(
        (A_s_phys_cf * n_normal) ** 2
        * ds("conductor_surface", bonus_intorder=BONUS_INT), mesh))
    print(f"  ∫_∂C |A_s_corrected·n|² dS = {A_s_corr_n_int:.4e} "
          f"(was {A_s_n_orig_int:.4e}, "
          f"ratio {A_s_corr_n_int/A_s_n_orig_int if A_s_n_orig_int else 0:.4e})",
          flush=True)

    # Also check curl(A_s_corrected) ≈ B_0 ẑ in conductor (= curl(A_s_phys))
    curl_diff = float(Integrate(
        ((curl(gfA_s_corrected) - CoefficientFunction((0, 0, 1))) ** 2)
        * dx("conductor", bonus_intorder=BONUS_INT), mesh))
    print(f"  ∫_C |curl(A_s_corr) - B_0 ẑ|² dV = {curl_diff:.4e}",
          flush=True)

    # Replace gfA_s with corrected version for the iteration
    gfA_s = gfA_s_corrected
    ...
    out = {"method": "HCurl + Kelvin Kameari iteration (v19: Coulomb correction for J·n=0)",
           "L_n_formula": "L_n = R_n² × ∫_full ν |curl(gfA_r)|² dV (canonical B² form)",
           "gauge_recipe": "nograds=True + tree-cotree + Helmholtz-Hodge + A_s Coulomb correction",
           "R_K_mm": R_K*1000, "h_cond_mm": H_COND*1000,
           "h_air_mm": H_AIR*1000, "order": ORDER, "ne": mesh.ne,
           "rel_err_gfA_s_cond": rel_err,
           "A_s_corr_dot_n_int": A_s_corr_n_int,
           "A_s_orig_dot_n_int": A_s_n_orig_int,
           "curl_diff_in_cond": curl_diff,
           "stages": diag, "R0_analytic": R0_anal,
           "sign_flip_first_n": sign_flip_first_n}
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v19_Jn_zero.json"
