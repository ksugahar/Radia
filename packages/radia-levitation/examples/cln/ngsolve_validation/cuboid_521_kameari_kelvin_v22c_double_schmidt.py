...
print("Starting v22c (double Schmidt: J in σ-norm + A in (1/μ)-norm) (refined mesh + Order=3)...", flush=True)
...
def main():
    ...
    u_history = []  # store gfu_n for A-side Schmidt orthogonalization
    ...
    for n in range(N_STAGES):
        ...
        # === A-side Schmidt orthogonalization in (1/μ)-norm ===
        # Make gfu_n orthogonal to all previous gfu_k in a_T(u, v) = ∫(ν/r)∇u·∇v
        # inner product (= magnetic energy norm).
        # This ensures L_n_dot = L_n_B2 (cross terms cancel).
        a_drift_pre = 0.0
        for gfu_k in u_history:
            ak_uk = float(Integrate(nu_cf / r_weight
                                      * grad(gfu_k) * grad(gfu_k) * dx, mesh))
            ak_un = float(Integrate(nu_cf / r_weight
                                      * grad(gfu_k) * grad(gfu_n) * dx, mesh))
            if abs(ak_uk) > 1e-40:
                coeff = ak_un / ak_uk
                gfu_n.vec.data -= coeff * gfu_k.vec
                a_drift_pre = max(a_drift_pre, abs(ak_un / (ak_uk if ak_uk > 0 else 1.0)))
        ...
        # Save the orthogonalized A_n for later A-Schmidt
        gfu_save = GridFunction(fes_u)
        gfu_save.vec.data = gfu_n.vec.data
        u_history.append(gfu_save)
        ...
        # τ_n via B² form (per-stage manifestly positive):
        # ∫_3D ν |curl(R_n × A_n)|² dV where A_n = u_n / r
        # = R_n² × 2π × ∫_2D (ν/r) × |∇u_n|² dr dz
        ...
        print(f"  [{n}] drift={rel_drift:.3e}  En={R_inv:.3e}  "
              ...
              f"τ_dot={tau_dot_str}μs  τ_B2={tau_B2_str}μs  "
    ...
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v22c_double_schmidt.json"
