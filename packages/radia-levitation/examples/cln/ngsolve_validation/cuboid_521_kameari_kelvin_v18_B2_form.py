...
print("Starting v18 (Helmholtz-Hodge + L_n via B² integral)...", flush=True)
...
def main():
    ...
    out = {"method": "HCurl + Kelvin Kameari iteration (v18: H-Hodge + L_n via B² integral)",
           "L_n_formula": "L_n = R_n² × ∫_full ν |curl(gfA_r)|² dV (canonical B² form)",
           "gauge_recipe": "nograds=True + tree-cotree + Helmholtz-Hodge projection (no penalty)",
    ...
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v18_B2_form.json"
