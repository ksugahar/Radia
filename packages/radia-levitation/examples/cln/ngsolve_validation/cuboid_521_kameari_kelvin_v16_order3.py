"""Kameari + Kelvin v16: v15 with Order = 3 (2026-05-05).

Hypothesis: v15 Order=2 sign-flips at stage 1 because the position-dependent
ν' = (ρ'/R)² ν₀ in the Kelvin region is not adequately resolved. The closed-
PEC working baseline (uniform ν₀) needs Order=3 + bonus_intorder=8 for
machine-precision orthogonality through N=11. v16 tests Order=3 for Kelvin.

If v16 still sign-flips at stage 1, the problem is structural (Periodic
boundary, gauge handling, Convention B sign), not discretization order.

(original v15 docstring follows for reference)
...
print("Starting v16 (v15 with Order=3)...", flush=True)
...
def main():
    ...
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v16_order3.json"
