...
`cubit_mesh_export.mcp.rules` (cubit) and `radia_mcp.build123d.rules`
...
exposing one, stays **non-public**. Converters live in independent repositories
separate from the tool's knowledge MCP server and use the neutral IR. This code is
...
`tools/policy_lint.py` Policy 10 checks public catalog entries; the package
policy guard checks imports and distribution contents.
