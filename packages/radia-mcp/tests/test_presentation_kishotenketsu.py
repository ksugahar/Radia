...
# --------------------------------------------------------------------------
# 2026-09-10、IGTE'26 から一枚を控えへ移したときに出た二つ。どちらも検査側の
# 欠陥で、症状は同じ「転がまとめの枚に落ちる」だった。

def _deck_with_furniture(tmp_path, rows, name="furniture.pptx"):
    """研究室の版面どおり、結論帯の下に出典行・所属・頁番号を置いたデッキ。"""
    prs = pptx.Presentation()
    prs.slide_width, prs.slide_height = Inches(13.333), Inches(7.5)
    for n, (title, body, takeaway) in enumerate(
        ...
        for line in body:
            box = slide.shapes.add_textbox(Inches(0.6), Inches(1.5),
                                           Inches(12.0), Inches(3.0))
            box.text_frame.text = line
        if takeaway:                                    # 結論帯 (top 0.85)
            band = slide.shapes.add_textbox(Inches(0.4), Inches(6.4),
                                            Inches(12.5), Inches(0.6))
            band.text_frame.paragraphs[0].add_run().text = takeaway
        # 飾り: 出典行 (0.94)、所属と頁番号 (0.96) ―― 帯より下に置く
...
def test_page_furniture_is_not_read_as_the_takeaway(tmp_path):
    # 結論帯を持たない枚（区切りの枚がまさにそれ）で、最後の文字ブロックが
    # 頁番号になり、下端文が「4」になっていた。
    rows = list(ARC)
    rows.insert(2, ("Outline", ["So far the problem.", "From here, the method."], ""))
    r = presentation_kishotenketsu_check(str(_deck_with_furniture(tmp_path, rows)))
    outline = next(s for s in r["outline"] if 4 in s["slides"])
    assert "4" not in outline["takeaways"]
    assert "Kindai University" not in outline["takeaways"]
    # 帯を持つ枚は、飾りに邪魔されず帯を拾う
    assert any("A circuit, exact at DC." in t
               for phase in r["outline"] for t in phase["takeaways"])


def test_no_x_does_y_is_a_complaint(tmp_path):
    # 研究室が実際に書く言い方。cannot 系だけを見ていると承が見つからず、
    # 転が最後のまとめ枚まで滑り落ちる。
    rows = [("The ladder", ["Kameari 2018 builds it."],
             "A circuit, DC exact: no rational model makes slope minus one half."),
            ("One space", ["Inspired by XFEM [10]: one matrix."],
             "Keep the ladder, add one surface mode."),
            ("A cylinder", ["Copper, 5 mm."], "Two unknowns hold it to 0.06 %."),
            ("Summary", ["Bulk plus surface."], "Keep the ladder, add the mode."),
            ]
    r = presentation_kishotenketsu_check(str(_deck(tmp_path, rows)))
    assert r["arc"]["sho"] == [2]          # 不足は 2 枚目で述べられている
    assert r["turn_slide"] == 3            # まとめ (5 枚目) ではない
