"""Rotor-sweep (cogging / reluctance) TORQUE method for motors -- the workflow
learned from the FEMM community (groups.io/g/femm): rotate the rotor and read the
air-gap Maxwell-stress torque at each angle. This locks in that radia-ngsolve can
reproduce the FEMM motor cogging/reluctance-torque sweep with EXISTING capability
...
Two self-consistency cases (no external analytic number needed):
...
gap layers, weighted-stress-tensor torque) are recorded in the lab memory note
reference_femm_motor_fea.md.
"""
import math
import os
import sys
...
def main():
    ...
    print("\n[OK] rotor-sweep cogging/reluctance torque validated: concentric ~0 "
          ...
          "zeros at 0/90/180, antisymmetric extrema).")
