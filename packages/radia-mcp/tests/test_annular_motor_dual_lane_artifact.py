...
def test_annular_motor_artifacts_keep_one_physical_identity():
...
def test_native_motor_angle_family_artifact_records_live_matlab_evidence():
    ...
    assert native["matlab_release"] == "(R2026a)"
    assert native["test_count"] == native["passed_count"] == 74
    assert native["failed_count"] == native["incomplete_count"] == 0
    assert native["foreign_openmp_runtime_dirs_excluded_count"] >= 1
