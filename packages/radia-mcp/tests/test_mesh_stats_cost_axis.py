...
@pytest.mark.skipif(shutil.which("python") is None, reason="no python")
def test_dof_estimate_matches_ngsolve_fes(cube_msh, tmp_path):
    """Cross-check the gmsh-side dof counts against NGSolve's own FES.

    This is the claim that makes dof_estimate worth reporting, so it is
    verified against an independent implementation rather than asserted.
    """
