...
def test_health_report_preserves_unscored_question_candidates():
    ...
    assert "central_question_singularity" not in t.grant_writing_health_report(
        text, skip="singularity"
    )["tools_run"]
...
@pytest.mark.parametrize("count,status", [(99, "within_estimate"), (100, "within_estimate"), (101, "exceeds_estimate")])
...
@pytest.mark.parametrize("kwargs", [
    {"page_limit": -1}, {"page_limit": math.nan}, {"reserved_pages": math.inf},
    {"chars_per_page": 0}, {"chars_per_page": 1.5}, {"figure_count": -1},
    {"figure_count": 1.5}, {"figure_page_cost": -1}, {"page_limit": True},
])
...
@pytest.mark.parametrize("kwargs", [{"preset": "unknown"}, {"fields": ", ,"}, {"preset": "tateisi_research", "fields": "目的"}])
...
def test_new_tools_are_registered_with_real_mcp_schemas():
    ...
    for suffix in ("central_question_singularity_check", "draft_length_budget_check", "form_field_coverage_check"):
