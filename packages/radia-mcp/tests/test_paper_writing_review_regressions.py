...
def test_overfull_hbox_count_not_limited_to_detail_cap(tmp_path):
    ...
    log.write_text("Overfull \\hbox (1.0pt too wide) detected at line 5\n" * 23
                   ...
                   encoding="utf-8")
    result = pw.paper_writing_check_overfull_hbox(str(log))
    # 23 hbox plus the \output-active vbox: an overflowing frame or float
    # is a defect too, and this test's subject is the 20-detail cap.
    assert result["overfull_count"] == 24
...
def test_overfull_scan_counts_a_wrapped_source_line():
    """TeX wraps the line number onto the next log line; that box still counts.

    Every alternative in the scanner demanded the number on the same line, so a
    wrapped report matched none of them and was silently dropped. For a check
    whose target is zero overfull boxes, undercounting reads exactly like a pass.
    """
    from radia_mcp._shared.latex_log import scan_overfull, summarize_overfull

    wrapped = (
        "Overfull \\hbox (12.34pt too wide) in paragraph at lines\n"
        "118--120\n"
    )
    records = scan_overfull(wrapped)
    assert [r["lines"] for r in records] == ["118-120"]
    assert records[0]["site"] == "paragraph"

    same_line = "Overfull \\hbox (12.34pt too wide) in paragraph at lines 118--120\n"
    assert scan_overfull(same_line) == records
    assert summarize_overfull(wrapped)["overfull_count"] == 1
