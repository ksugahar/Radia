...
def build123d_jointed_assembly_source_replay_gate(summary_json: str) -> str:
    ...
    try:
        ...
        v47_identity = _validate_build123d_v47_source_identity(payload)
        if v47_identity:
            result["build123d_v47_source_identity"] = v47_identity
            checks = result.setdefault("checks", {})
            if isinstance(checks, dict):
                checks.update(v47_identity.get("checks", {}))
            if v47_identity.get("status") != "ok":
                result["status"] = "needs_attention"
        v48_identity = _validate_build123d_v48_source_identity(payload)
        if v48_identity:
            result["build123d_v48_source_identity"] = v48_identity
            checks = result.setdefault("checks", {})
            if isinstance(checks, dict):
        ...
        if v50_identity:
            ...
            if v50_identity.get("status") != "ok":
                result["status"] = "needs_attention"
        v51_identity = _validate_build123d_v51_source_identity(payload)
        if v51_identity:
            result["build123d_v51_source_identity"] = v51_identity
            checks = result.setdefault("checks", {})
            if isinstance(checks, dict):
                checks.update(v51_identity.get("checks", {}))
...
def build123d_mass_property_crosscheck(
    ...
    v47_identity = _validate_build123d_v47_public_identity({
        "reference": reference_rows,
        "measured": measured_sets,
    })
    if v47_identity:
        summary["build123d_v47_public_identity"] = v47_identity
        checks = summary.setdefault("checks", {})
        if isinstance(checks, dict):
            checks.update(v47_identity.get("checks", {}))
        if v47_identity.get("status") != "ok":
            summary["status"] = "needs_attention"
    v48_identity = _validate_build123d_v48_public_identity({
        "reference": reference_rows,
        "measured": measured_sets,
    })
    if v48_identity:
        summary["build123d_v48_public_identity"] = v48_identity
        checks = summary.setdefault("checks", {})
        if isinstance(checks, dict):
            checks.update(v48_identity.get("checks", {}))
        if v48_identity.get("status") != "ok":
    ...
    if v50_identity:
        ...
        if v50_identity.get("status") != "ok":
            summary["status"] = "needs_attention"
    v51_identity = _validate_build123d_v51_public_identity({
        "reference": reference_rows,
        "measured": measured_sets,
    })
    if v51_identity:
        summary["build123d_v51_public_identity"] = v51_identity
        checks = summary.setdefault("checks", {})
        if isinstance(checks, dict):
            checks.update(v51_identity.get("checks", {}))
        if v51_identity.get("status") != "ok":
