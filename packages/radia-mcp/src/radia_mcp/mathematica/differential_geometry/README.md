...
The executable distilled knowledge is `curve_surface_basics.wls`.
...
| `curve_surface_basics.wls` | curve/surface DG fundamentals distilled from the local Mathematica curve/surface background into Radia-owned assertions: curve speed/curvature/torsion; surface first and second fundamental forms; sphere/cylinder area, normal, Gaussian and mean curvature; `g = J^T J`; surface gradient; stream-function current `K = n x grad_Gamma psi` is tangential and surface-divergence-free on sphere/plane; Laplace-Beltrami = `div_Gamma grad_Gamma` with the sphere eigenvalue check | 23 assertions, ALL PASS |
