...
def matlab_optuna_simulink_contract():
    ...
    return {
        ...
        "simulink_blocks": [
            "Radia/Applications/Induction Heating uses separate radia_ih_eddy_sfun and radia_ih_thermal_sfun native distributed-field kernels; its inputs are current, workpiece angle, and ambient temperature.",
            ...
            "IH Thermal publishes its previous accepted field in mdlOutputs, feeds it directly to Eddy, and advances only in mdlUpdate. Linear current changes use current-squared heat scaling; temperature-dependent operator data trigger a new field solve. Nonlinear BH fails fast in the preview. LUT and lumped IH builders are removed.",
