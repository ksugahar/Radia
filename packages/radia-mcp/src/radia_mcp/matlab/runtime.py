...
def matlab_radia_mex_contract(topic="all"):
    ...
    sections = {
        ...
        "optimization": {"toolbox_interface": {"entry_point": "radia.optuna.optimize(fun, parameters, options)", "signature": "[x, fval, exitflag, output] -- the Global Optimization Toolbox return contract, so a ga/particleswarm/sdo.optimize user reads an Optuna run without a second vocabulary", "options": "radia.optuna.optimoptions(Name=Value) -> radia.optuna.OptimizeOptions; accepts a leading \"optuna\" solver name or an existing options object to copy, and rejects an unknown option by name", "toolbox_option_names": ["MaxTime", "FunctionTolerance", "Display", "OutputFcn", "PlotFcn", "UseParallel"], "renamed_options": {"MaxTrials": "ga MaxGenerations / fmincon MaxIterations, counted in trials", "MaxStallTrials": "ga MaxStallGenerations, counted in trials"}, "optuna_only_options": ["Sampler", "Pruner", "Seed", "StoragePath", "Directions", "ConstraintFcn"], "exitflag": {"1": "FunctionTolerance satisfied over MaxStallTrials", "0": "MaxTrials reached", "-1": "stopped by an OutputFcn or PlotFcn", "-2": "no feasible point (no trial completed)", "-5": "MaxTime exceeded"}, "output_fields": ["funccount", "trials", "message", "rngstate", "study", "bestTrialNumber", "prunedcount", "failedcount", "elapsedtime"], "hook_protocol": "stop = fcn(x, optimValues, state) with state \"init\"/\"iter\"/\"done\"; a true stop ends the run", "parameters": "radia.optuna.OptimizationParameter (Name, Value, Minimum, Maximum, Free, Type, Transform, Step, Choices) or OptimizationParameter.fromStruct(struct(name, [lo hi]))", "from_model": "radia.optuna.getParameterFromModel(model, names) reads the model/base workspace the way sdo.getParameterFromModel does and leaves Minimum/Maximum infinite; optimize then refuses to search an unbounded parameter rather than inventing a range", "objective_contract": "fun(x) receives a struct of values, never a Trial -- that is what makes UseParallel honest, because suggesting mutates the study and stays serial while the batch is evaluated on the pool", "use_parallel": "asks a batch, evaluates it with parfor, then tells; errors when no pool is open instead of silently running serially", "sampler_factory": "radia.optuna.internal.samplerFromName is the single short-name mapping, shared with the Simulink block; it refuses \"auto\", which AutoSamplerPolicy.choose resolves", "test_gate": "runtests('tests/matlab/test_optuna_got_interface.m')", "upstream_parity": "none -- upstream Optuna has no optimoptions and no exitflag, so this surface is a MATLAB extension and is never evidence of Optuna compatibility"}, "package": "radia.optuna", "native_gateway": "optuna_mex", "native_gateway_required": True, "native_command_count": len(optuna_commands), "classes": optuna_classes, "factory_functions": optuna_functions, "storage": "MAT-file containing readable MATLAB tables and CAE trial/failure artifacts", "native_kernels": _optuna_native_kernels(optuna_commands), "mcp_tool": "matlab_optuna_simulink_contract"},
