"""Solver-neutral regularized linear inversion (TSVD + Tikhonov).
...
def tsvd_solve(A, b, k):
    ...
    if np.any(s[:k] == 0):
        raise ValueError("selected TSVD modes include zero singular values; reduce k")
    coeffs = (U[:, :k].T @ b) / s[:k]
...
def filter_factors(s, lam):
    ...
    Zero singular modes are omitted, including when lam=0.
    """
    s = np.asarray(s, dtype=float)
    if not np.isscalar(lam) or not np.isfinite(lam) or lam < 0:
    ...
    scale = np.hypot(s, lam)
    ratio = np.divide(s, scale, out=np.zeros_like(s), where=scale != 0)
    return ratio * ratio
...
def tikhonov_solve(A, b, lam):
    ...
    recovers the minimum-norm least-squares solution. At lam=0 use NumPy's
    least-squares rank convention (rcond=None); positive lam retains all modes.
    """
    U, s, Vt = _svd(A)
    b = np.asarray(b, dtype=float)
    filter_factors(s, lam)  # Validate the spectral/regularization contract.
    if lam == 0:
        return np.linalg.lstsq(np.asarray(A, dtype=float), b, rcond=None)[0]
    # s/(s^2+lam^2), avoiding phi/s = 0/0 on exact null modes.
    scale = np.hypot(s, lam)
    weights = (s / scale) / scale
    coeffs = weights * (U.T @ b)
    return Vt.T @ coeffs
