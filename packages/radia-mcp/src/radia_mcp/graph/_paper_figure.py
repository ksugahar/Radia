"""radia_mcp.graph.paper_figure — paper-grade figure scaffolds + gates.
...
# defer the import to the first call so `mcp-server-graph --selftest`
...
def _check_no_in_figure_title(fig) -> list[str]:
    ...
    on `on_fail='raise'` since shipping with a title is a peer-review
    visible style break.
    """
    bad = []
    # Per-axes titles
