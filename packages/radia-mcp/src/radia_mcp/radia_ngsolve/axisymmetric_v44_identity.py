...
from .axisymmetric_v45_identity import validate_public_v45_identity
...
def validate_public_identity(identity: object) -> dict[str, bool]:
    ...
    if isinstance(actuator, dict):
        ...
        try:
            ...
            return {"magnetostatic_2d_v44_axisymmetric_actuator_identity": False}
        mirrored = all(
        ...
        checks.update(
            {
                "magnetostatic_2d_v44_axisymmetric_actuator_generation_closure": bool(generation) and mirrored,
                "magnetostatic_2d_v44_axisymmetric_actuator_flux_force_coenergy_mirror": (
    ...
    if isinstance(fringe, dict):
        ...
        try:
            ...
            return {"magnetostatic_2d_v44_electrostatic_fringe_identity": False}
        mirrored = all(
        ...
        checks.update(
            {
                "magnetostatic_2d_v44_electrostatic_fringe_generation_closure": bool(generation) and mirrored,
                "magnetostatic_2d_v44_electrostatic_fringe_charge_energy_closure": (
                ...
                "magnetostatic_2d_v44_electrostatic_fringe_axis_factor": (
                    math.isclose(factor, 2.0 * math.pi, rel_tol=1.0e-12, abs_tol=1.0e-15)
                    and result_factor == factor
                ),
                "magnetostatic_2d_v44_electrostatic_fringe_mesh_and_result_owner": (
                    str(fringe.get("mesh_owner", "")).startswith("mesh:")
                    and fringe.get("result_mesh_owner") == fringe.get("mesh_owner")
                ),
                "magnetostatic_2d_v44_electrostatic_fringe_result_digest": (
