...
All `radia_export` commands use MeshExportInterface (C++ plugin). Block element
type settings are ignored — high-order nodes are generated by NetgenCurver.

| Export Command | HO Method | Max Order |
|----------------|-----------|-----------|
| `radia_export netgen "f.vol" order N` | NetgenCurver + ACIS | 1-5 |
| `radia_export gmsh "f.msh" order N` | NetgenCurver + ACIS | 1-3 |
| `radia_export nastran "f.bdf" order N` | NetgenCurver + ACIS | 1-2 |
| `radia_export vtk "f.vtk" order N` | NetgenCurver + ACIS | 1-2 |
| `radia_export meg "f.meg"` | — | 1 |
| `radia_export femeem "dir"` | — | 1 (tet only) |

## Design: Why radia_export Uses 1st Order Internally
...
for high-order curving with `radia_export netgen`, STEP files are NOT needed
(radia_export netgen uses Cubit's ACIS kernel directly via CallbackGeometry).
...
`radia_export netgen` uses Cubit's ACIS kernel directly, so no STEP exchange
or OCC topology matching is required.

STEP reimport is only relevant if you need to match Cubit topology to
an external OCC-based tool (not for radia_export netgen).
"""
...
and NGSolve/Netgen, enabling the `radia_export netgen` high-order curving workflow.
...
In radia_export netgen, the last block wins for bcname priority.
...
**These APIs have been deleted.** Use `radia_export netgen` instead.
...
## Problem: radia_export netgen Returns Mesh with 0 Boundary Elements
...
Exodus II uses Cubit's built-in `export mesh` command (not a radia_export plugin):
```python
...
In radia_export netgen, last block wins for bcname. Source/sink
