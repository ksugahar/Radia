...
def _notebook_citation_summary(path: pathlib.Path, repo_root: pathlib.Path) -> dict:
    ...
    if not isinstance(declared, list):
        declared = []
    declared_keys = {
        str(key).strip() for key in declared if str(key).strip()
    }
    exemption_reason = str(
        radia_metadata.get("citation_audit_exempt_reason", "")
    ).strip()
    ...
    inline_keys.update(re.findall(
        r"(?<!\w)@([A-Za-z0-9_:.+\-/]+)", citation_text
    ))
    ...
    has_reference_heading = bool(re.search(
        r"(?im)^#{1,6}\s+(?:references|bibliography|参考文献)\s*$",
        citation_text,
    ))
    return {
        "path": _rel(path, repo_root),
...
def document_meta_notebook_citation_audit(
        ...
        tracked_only: bool = True,
        max_items: int = 50) -> dict:
    """Audit notebook citations against Radia's canonical ``references.bib``.

    Citation keys may be written inline as LaTeX/Pandoc citations or declared in
    ``metadata.radia.citation_keys``. DOI and arXiv links are also resolved to
    canonical entries. A scholarly identifier without a declared/inline key is
    reported so notebook references remain machine-auditable instead of only
    ...
    return {
        ...
        "summary": {
            ...
            "ok_reference_section_exempted": sum(
                1 for row in rows
                if row["status"] == "ok_reference_section_exempted"
            ),
            "gaps": gap_count,
        },
        "rows": rows,
