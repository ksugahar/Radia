"""Japanese paper checks and compatibility entry points for grant writing.

Shared algorithms live in radia_mcp._shared.japanese_checks. This module owns
paper-specific rules; grant wrappers separately handle application-form prose.
"""
...
from radia_mcp._shared import japanese_checks as _checks
...
_MISUSE = [
    ...
    (r"新規性を[^、。]{0,20}置く", "不自然なメタ表現。先行研究との差を具体的に直接述べる"),
]
...
_NOTATION_PAIRS = [
    ...
    (r"(?<=[てでにを])頂く", "いただく"),
    (r"(?<=て)下さい", "ください"),
    # 人数の助数詞 (編集者・圭子 2026-09-03): 佐藤・比留間のように定まった人物を
    # 数えるなら「4人」。「N名」は名簿上の不特定の人数に使う。申請書・論文の
    # 本文で数えるのはほぼ特定の人物なので「人」を提案する。
    (r"(?<=[一二三四五六七八九十0-9０-９])名(?=[のはがでをへと、。])", "人"),
    # 送り仮名の揺れ
    (r"行なう", "行う"),   # 行う に統一
    (r"表わす", "表す"),   # 表す に統一
    (r"現わす", "現す"),
...
def grant_writing_lint_bedrock(text: str) -> dict:
    ...
    """
    return _checks._lint_bedrock(text, _scan_hedges)
...
def grant_writing_suggest_redundancy_fixes(text: str) -> dict:
    ...
    の定番 lint。
    """
    return _checks._suggest_redundancy_fixes(text, _REDUNDANCY)
...
def grant_writing_check_misuse_japanese(text: str) -> dict:
    ...
    """
    return _checks._check_misuse_japanese(text, _MISUSE)
