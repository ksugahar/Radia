...
# references.bib policy: one canonical database, manuscript-specific .bbl

## The three rules (Sugahara Lab)

1. **ONE canonical `references.bib`** bundled under
   `radia_mcp/bibliography/data/` serves the digest, full paper, poster,
   and slides. Do NOT keep a separate `.bib` in each manuscript folder.
   Manuscripts cite canonical keys; the publisher receives the
   manuscript-specific `.bbl` generated by `bibliography_make_bbl`.
...
| Promotion path | a digest grows into a full paper using the same stable keys |
| Audit | one canonical file to lint and verify |
| Submission | generated `.bbl` is self-contained and includes only cited works |

Layout:
```
radia_mcp/bibliography/data/references.bib  <- THE canonical source
project/
  digest/igte_digest.tex
  digest/igte_digest.bbl                  <- generated for submission
  full/ieee_tmag.tex
  full/ieee_tmag.bbl                      <- generated for submission
```
...
The `paper_writing_verify_citation` tool **refuses to fabricate**. With no
explicit `bib_path`, it checks the bundled canonical file; if an external
source cannot be verified it returns an error or no-candidate verdict rather
than inventing an entry.

## Enforcement in the submission gate

`paper_writing_em_submission_gate` uses the bundled canonical bibliography by
default. An explicit `bib_path` remains available when reviewing an external
project, but a supplied missing or unreadable path is a critical failure.

## How to apply

  1. Keep one canonical `references.bib`; manuscript folders hold no `.bib` copy.
  ...
  4. Gate digest AND full paper against the canonical bibliography.
  5. Run `bibliography_make_bbl(tex)` for each deliverable. It refuses to
     write a partial `.bbl` if any `\cite{}` key is absent from the canonical
     source.
...
- `paper_writing_check_citation_keys_exist` — every \cite resolves in
  the canonical bib.
- `bibliography_make_bbl` — generate the only bibliography artifact shipped
  beside a manuscript.
...
etc.) are journal-agnostic.  THIS module is EM-paper-specific:
  ...
  reference_bib / bib / bib_policy / shared_bib / citation_verification
        ONE canonical references.bib for all manuscripts; EVERY entry
        web-verified before insertion; ship a generated per-manuscript .bbl.
...
def paper_writing_em_submission_gate(
    ...
    if not bib_path:
        ...
        bib_source = "canonical_default"

    tex_valid = bool(tex_path and pathlib.Path(tex_path).is_file())
    bib_valid = bool(bib_path and pathlib.Path(bib_path).is_file())
    pdf_valid = bool(pdf_path and pathlib.Path(pdf_path).is_file())
    for name, supplied, valid in (
        ("tex_input", tex_path, tex_valid),
        ("bib_input", bib_path, bib_valid),
        ("pdf_input", pdf_path, pdf_valid),
    ):
            ...
            if r.get("ok") and r.get("files_resolved"):
                ...
                if n_inputs > 0:
                    merged_temp_dir = tempfile.TemporaryDirectory(
                        prefix="radia_mcp_paper_gate_"
                    )
                    merged = pathlib.Path(merged_temp_dir.name) / "merged.tex"
                    merged.write_text(r["merged_tex"], encoding="utf-8")
                    merged_tex_path = str(merged)
                    _add(
                        "multifile_resolved",
                        "pass",
                        (f"resolved {n_inputs} \\input subfiles -> "
                         "temporary merged source"),
                ...
                _add(
                    "multifile_resolved",
                    "fail",
                    f"resolver failed: {r.get('error', 'input chain is incomplete')}",
                    r,
                )
        except Exception as e:  # noqa: BLE001
            _add("multifile_resolved", "fail",
                 f"resolver error: {e}")

    # v0.93.0: abstract auto-extraction
    if tex_valid and not abstract_text and auto_extract_abstract:
        try:
    ...
    if tex_valid:
        ...
        try:
            ...
            n_fwd = r.get("dangling_ref_count", len(r.get("dangling_refs", [])))
            n_orphan = r.get("orphan_label_count", len(r.get("orphan_labels", [])))
            status = "fail" if n_fwd > 0 else "pass"
            if status == "pass" and n_orphan > 0:
                status = "warn"
            _add("figure_forward_reference",
                 status,
                 f"{n_fwd} dangling references, {n_orphan} orphan labels",
                 r)
        except Exception as e:  # noqa: BLE001
        ...
        try:
            ...
            n_dangling = len(r.get("dangling_refs", []))
            n_unused = len(r.get("unused_labels", []))
            status = "fail" if n_dangling else "warn" if n_unused else "pass"
            _add(
                "equation_numbering",
                status,
                f"{n_dangling} dangling equation refs, {n_unused} unused labels",
                r,
            )
        except Exception as e:  # noqa: BLE001
        ...
        try:
            ...
            n_under = r.get("total_underlines", 0)
        ...
        try:
            if target_category == "ieej":
            ...
            elif target_category in {"accelerator", "electromagnetics"}:
                _add(
                    "target_keywords",
                    "skip",
                    "venue keyword rules are template-specific; verify the official venue template",
                    target_profile,
                )
            elif target_category == "ieee":
                ...
                _add(
                    "target_keywords",
                    "skip",
                    "select a supported venue before applying keyword rules",
                    target_profile,
                )
        except Exception as e:  # noqa: BLE001
        ...
        _add(
            "tex_checks",
            "skip",
            "no tex_path supplied" if not tex_path else "invalid tex_path supplied",
        )

    # ------ BIB POLICY GATE (v0.91.0) ------
    # The bundled references.bib is the default single source of truth.
    # External projects may supply an explicit override, but manuscript-local
    # copies are not created by this workflow.
    _add(
        "bib_policy",
        "pass" if bib_valid else "fail",
        (
            f"using {bib_source} references.bib: {bib_path}; "
            "ship a manuscript-specific .bbl generated from this source"
        ),
        {"bib_path": bib_path, "source": bib_source},
    )
    ...
    if bib_valid:
        if tex_valid:
            try:
                ...
                cited_keys = set(
                    _collect_cited_keys(
                        pathlib.Path(_scan_tex).read_text(encoding="utf-8")
                    )
                )
                cited_problems = [
                    problem for problem in r.get("problems", [])
                    if problem.get("key") in cited_keys
                ]
                critical_fields = [
                    field
                    for problem in cited_problems
                    for field in problem.get("missing_fields", [])
                    if "recommended" not in field and "optional" not in field
                ]
                recommended_fields = [
                    field
                    for problem in cited_problems
                    for field in problem.get("missing_fields", [])
                    if "recommended" in field or "optional" in field
                ]
                detail = {
                    **r,
                    "cited_entries_checked": len(cited_keys),
                    "cited_problems": cited_problems,
                    "critical_missing_field_count": len(critical_fields),
                    "recommended_missing_field_count": len(recommended_fields),
                }
                status = (
                    "fail" if critical_fields
                    else "warn" if recommended_fields
                    else "pass"
                )
                _add(
                    "lint_reference_format",
                    status,
                    (
                        f"cited entries: {len(critical_fields)} required and "
                        f"{len(recommended_fields)} recommended fields missing"
                    ),
                    detail,
                )
            except Exception as e:  # noqa: BLE001
            ...
            _add(
                "lint_reference_format",
                "skip",
                "no tex_path supplied; no cited bibliography subset to lint",
            )

        if tex_valid:
            try:
                r = _t.paper_writing_check_citation_usage(_scan_tex, bib_path)
                n_missing = r.get("missing_in_bib_count", 0)
                n_unused = r.get("unused_in_tex_count", 0)
                status = "fail" if n_missing else "warn" if n_unused else "pass"
                _add(
                    "check_citation_usage",
                    status,
                    f"{n_missing} missing cite keys, {n_unused} unused bib entries",
                    r,
                )
            except Exception as e:  # noqa: BLE001
            ...
            if author_last_names:
                try:
                    ...
                    _add(
                        "self_citation_ratio",
                        "warn" if verdict in {"WARN", "HIGH"} else "pass",
                        f"self-citation ratio {r.get('self_citation_ratio', 0):.1%}",
                        r,
                    )
                except Exception as e:  # noqa: BLE001
        ...
        _add(
            "bib_checks",
            "skip",
            "no bib_path supplied" if not bib_path else "invalid bib_path supplied",
        )
    ...
    if abstract_text:
        try:
            ...
            _add(
                ...
                "pass" if r.get("within_limit") else "fail",
                f"abstract {r.get('count', 0)} {r.get('unit', '')}; limit {r.get('limit', 0)}",
                r,
            )
        except Exception as e:  # noqa: BLE001
        ...
        try:
            r = _t.paper_writing_check_abstract_background_ratio(
                abstract_text)
            ratio = r.get("background_ratio", 0.0)
            status = "fail" if ratio > 0.40 else "warn" if ratio > 0.25 else "pass"
            _add(
                "abstract_background_ratio",
                status,
                f"abstract background ratio {ratio:.1%}",
                r,
            )
        except Exception as e:  # noqa: BLE001
    ...
    if pdf_valid:
        ...
        if tex_valid:
            try:
                r = paper_writing_check_floats_far_from_reference(
                    ...
                    aux_path=str(pathlib.Path(tex_path).with_suffix(".aux")),
                )
                n_flag = r.get("flagged_count", 0)
                status = (
                    "skip" if not r.get("applicable", True)
                    else "warn" if n_flag > 0
                    else "pass"
                )
                _add("floats_far_from_reference", status,
                     r.get("reason") if status == "skip"
                     else f"{n_flag} floats drift > {layout_max_pages_apart}"
        ...
        _add(
            "pdf_checks",
            "skip",
            "no pdf_path supplied" if not pdf_path else "invalid pdf_path supplied",
        )
    ...
    if merged_temp_dir is not None:
