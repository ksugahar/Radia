...
小数点と既知の `Fig.` / `Eq.` 等は文境界にしない。文末12文字を分類し、`示した` / `考えられる` 等は短い末尾パターンより優先する。`start_sentence_index` は0始まり。未分類文は連続区間を切るが、それ自体をcriticalにしない。英語ピリオド後の空白がない文など、任意の文境界を認識する自然言語パーサではない。
