    ...
    for slide_no, name, shape, asset in found:
        if "unreadable" in asset:
            rows.append({
                ...
                "displayed_cm": float(Emu(shape.width).cm),
                "displayed_pt": float(Emu(shape.width).pt),
                "displayed_cm_height": float(Emu(shape.height).cm),
                "scale": float("nan"),
...
def figure_audit_pptx_figures(pptx_path: str,
    ...
    for r in rep["pictures"]:
        ...
        if r.get("source_type") == "svg":
            resolution = "vector"
        elif r.get("effective_dpi") is None:
            resolution = "resolution unknown"
        else:
            resolution = f"{r['effective_dpi']:.0f} dpi"
        lines.append(
