...
（圭子）が読み、意味の通らない文を指摘した。

> 四名の手法を誘導加熱で結合し、その判定則を加速器電磁石設計へ発展させる。

指摘は二つ。「四名」は不特定の人数の数え方で、佐藤・比留間・長嶺・菅原と
定まった人物なら「4人」。そして、この文では誘導加熱と加速器電磁石が同じ
研究に見える。編集者の案は次のとおり。

> 4人の手法を誘導加熱を例題として結合させ、そこで培ったノウハウを
> 加速器電磁石設計に展開させる。

三案（09-01 版、圧縮版、編集者案）を同じ検査に掛けると、欄全体の読みやすさ点は
85.7、85.7、85.8 で区別できなかった。点数を作る軸は文長、助詞、漢字率、主述距離
という表面統計であり、二つの対象の**関係**（例題として結合し、そこで得たものを
展開する）が書かれているかは、どの検査も読まない。短く漢語で詰めるほど表面統計は
良くなるので、圧縮して意味を落とした文は同点か高得点になる。

圧縮が起きた原因は頁である。欄の充填率が 0.99 のとき、修正者は「行数を
増やさない」を制約にし、関係語（例題として、そこで得た）を足す方向ではなく
削る方向に書いた。編集者案は 11 字長く、1 行段落が 2 行になる。

研究代表者の方針: **どこか内容を削り、分かりやすい文を採る。** 頁が満杯で
あることは、文を圧縮する理由にならない。関係語を落とすくらいなら、再掲に
なっている一文（例: 計算資源の目的を言い直した文）を欄から外して行を作る。
そして、**無理に詰め込んで意味の分からない文になるなら、その一文は無いほうが
よい。** 圧縮して意味が壊れた文は、情報を足しているのではなく、審査者の
読みを止めている。収まらない文は、短くするのではなく消す。

規則は次のとおり。

- 二つの対象課題が一文にあるなら、両者の関係を語で書く（例題として、で得た、
  培った、移す、適用する、比較する）。「A で結合し、B へ発展させる」は
  関係を書いていない。`grant_writing_paired_object_relation_check` が、関係語の
  ない文を列挙する（question）。
- 特定の人物の人数は「N人」。「N名」は名簿上の不特定の人数に使う。表記ゆれ
  検査が「N名」に「N人」を提案する。
- 読みやすさ点で文案を選ばない。点は文単位の意味を区別できない。文案の比較は
  人が読む。
