...
def _expand_sibling_inputs(
    ...
    for line in text.split("\n"):
        ...
        suffix = line[comment.start():] if comment else ""
        ...
        if code and _INPUT_COMMAND.search(code):
            line = _INPUT_COMMAND.sub(replace, code) + suffix
        lines.append(line)
    return "\n".join(lines)
...
def grant_writing_adjacent_reviewer_readability_check(text: str) -> dict:
    ...
    abstract_feasibility_evidence_patterns = (
        (
            "generic_public_change",
            re.compile(r"(?:公開|外部)(?:変更|対応|活動)"),
        ),
        (
...
def grant_writing_japanese_readability_score(
    ...
    axes = {
        ...
        "notation_and_usage_consistency": {
            ...
            "score_max": 10,
            "evidence": {"misuse": misuse, "notation": notation},
        },
...
def _budget_ledger_rows(path: pathlib.Path, sheet_name: str) -> list[dict]:
    ...
    for source_row, row in enumerate(matrix[header_index + 1 :], header_index + 2):
        ...
        # e-Rad source workbooks commonly keep cached SUMIF/SUM totals below
        # the ledger after a blank separator. Their cached value sits in the
        # amount column while category, year, and item are blank. It is a
        # summary formula, not a malformed expenditure row. Keep an
        # amount-only row inside the ledger strict; only ignore this trailing
        # summary shape after a separator and after at least one valid row.
        if (
            rows
