...
def validate_matlab_ml_rl_artifact(value: Mapping[str, object]) -> dict[str, object]:
    ...
    checks: dict[str, bool] = {
        ...
        "seed_is_recorded": isinstance(value.get("random_seed"), int) and value.get("random_seed") >= 0,
    ...
    if task in {"regression", "classification"}:
        ...
        checks["cross_validation_is_explicit"] = isinstance(folds, int) and folds >= 2
        ...
        checks["training_episodes_are_positive"] = isinstance(value.get("training_episodes"), int) and value.get("training_episodes") > 0
        checks["evaluation_episodes_are_positive"] = isinstance(value.get("evaluation_episodes"), int) and value.get("evaluation_episodes") > 0
