...
# Does the audience SEE where the background ends and the proposal begins?
# (Matsuo, 2026-09-10: 「途中に目次/アウトラインを挟むと分かりやすい」)
from ._outline import (  # noqa: F401
...
def presentation_check_quoted_figure_credit(
    ...
    """論文から引用した図が、同じスライドで出典を名乗っているかを検査する。

    ビルドが書く図の由来 (``figure_text_source_evidence.json``) で
    ``source_evidence.quoted_from`` を持つ picture は他者の図である。
    そのスライドの本文に出典表記がなければ違反として報告する。逆に、
    出典表記があるのに ``quoted_from`` の証拠が無い picture は
    ``unverified_credit`` として報告する（自作図に他者の図と読める表記を
    付けている、または証拠の記録漏れ）。

    既定の出典表記の目印: ``©``, ``(c)``, ``Fig.``, ``Figure:``,
    ``reproduced``, ``courtesy``, ``adapted from``, ``出典``, ``引用``。

    Args:
        pptx_path: .pptx
        evidence_path: 由来 JSON。空なら .pptx と同じ場所の
            ``figure_text_source_evidence.json`` を探す。
        credit_markers: 出典表記の目印を差し替える。
    """
    try:
        import pptx  # type: ignore
        from pptx.enum.shapes import MSO_SHAPE_TYPE  # type: ignore
    ...
    markers = credit_markers or ["©", "(c)", "Fig.", "Figure:", "reproduced",
                                 "courtesy", "adapted from", "出典", "引用"]
    ...
    for index, slide in enumerate(prs.slides, start=1):
        ...
        has_credit = any(marker.casefold() in text.casefold() for marker in markers)
...
def presentation_check_qa_backup_slides(pptx_path: str,
    ...
    """pptx に Q&A backup slide が N 枚以上あり、かつ**非表示**かを確認。

    研究室の規約 (菅原 2026-09-10): **本編から落とした話は捨てず、非表示の
    質問スライドとして同じファイルに残す**。問われたその場で出せて、図も
    出典もセリフも一緒に残るからである。別ファイルに逃がすと本番で開けず、
    消せば二度と戻らない。

    だから題が `Backup` / `Q&A` / `補足` でも **表示のままなら控えではない**
    ―― 本番でそのまま映る。名前で数えるだけの検査はその取りこぼしを
    「控えが足りている」と報告してしまうので、`named_but_visible` に分けて
    返す。
    """
    try:
        import pptx as _pptx
    ...
    named_but_visible = [b for b in backup_slides
                         if b["reason"] == "named but VISIBLE"]
    ...
    return {
        ...
        "warnings": ([f"スライド {b['slide']}「{b['title']}」は控えの名前だが"
                      "非表示になっていない ―― 本番でそのまま映る。"
                      for b in named_but_visible]),
        "rule": "本編から落とした話は捨てず、非表示の質問スライドとして"
                "同じファイルに残す（菅原 2026-09-10）。問われたその場で"
                "出せて、図も出典もセリフも一緒に残る。別ファイルに逃がすと"
                "本番で開けず、消せば二度と戻らない。",
