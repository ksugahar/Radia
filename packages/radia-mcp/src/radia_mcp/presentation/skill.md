...
> **2026-07-17: paper-writing スキル群に統合。** presentation は独立サーバを
> やめ、`mcp-server-paper-writing` が全 `presentation_*` ツールを配信する。
> 上流の作文原則は `paper_writing/skill.md` と共有。
>
> **2026-08-19 更新（菅原方針）: セリフを先に書き、それに対応するスライドを
> 作る。** 台本が先で、スライドは「言っている間に見せるもの」。§Phase 2 の
> script-first がこの運用の中心で、下記の **script-first Markdown** がその
> 入稿形式（`>` 引用 = セリフ → ノート、それ以外 = スライドに映すもの）。
> `radia.equation.markdown_to_pptx` がその 1 ファイルから、数式を Microsoft
> 数式 (OMML) のまま持つ .pptx とスピーカーノートを組む。したがって
> 「AI はスライド本体を作れない」という旧前提はもう成り立たない。AI の
...
筋が転じていることと、聴衆にその転じ目が見えていることは別である。
背景・従来法が続いたあと、提案へ移る直前に区切りの枚を置き、どこまでが
問題でどこからが解決法かを明示する。冒頭に一度だけ置く目次は、中盤で
現在地を見失う問題には答えない。

`presentation_check_outline_slide` は次を確認する。

1. 目次または区切りの枚がある
2. その枚が `presentation_kishotenketsu_check` の転の直前にある
3. 問題側と解決法側を両方名指ししている

境目はタイトルの章名ではなく下端の主張文から求めるため、「タイトルは
名詞句・主張は下端帯」という研究室の様式を保てる。不合格時は
`suggested_outline` が挿入位置と両側の項目を枚番号付きで返す。枚を増やせない
場合は、境目の枚の下端文で章が変わったことを明言する。
...
#### 入稿形式: script-first Markdown (2026-08-19)

**セリフとスライドを 1 つの .md に、セリフを先に書く。**

| 記法 | 意味 | 出力先 |
|---|---|---|
| `# 見出し` | スライドが 1 枚始まる。見出しがタイトル | スライドタイトル |
| `> 行` | **セリフ**（言うこと） | スピーカーノート |
| それ以外 | **映すもの**（箇条書き・数式・短いラベル） | スライド本文 |

```markdown
# Kelvin 変換の目的

> 外部領域を有限領域に写して解く、という話をします。
> 無限遠を切らずに済むのが利点です。

- 外部 $\Omega^{E}$ を $\Omega'$ へ写す
- $\nu' = \nu \dfrac{r'^{2}}{R^{2}}$
```
...
数式は `$...$` / `$$...$$` のまま書けばよい。**画像ではなく Microsoft 数式
(OMML)** として入るので、PowerPoint の数式エディタでそのまま直せ、テーマの
フォントと文字サイズに追従する。セリフ側に書いた `$...$` も同じ扱い。

なぜこの向きか:

- **セリフが先**という Phase 2 の原則が、ファイルの書き順そのものになる。
  見出しの直後に書くのはセリフで、スライドはそのあと。
- **1 ファイルに両方ある**ので、`presentation_script_vs_slide_coverage` /
  `presentation_estimate_per_slide_time` / `presentation_speaker_note_ratio`
  が同じ入力で回る。台本ファイルと .pptx が別々だと、この照合は必ず腐る。
- セリフをスライドに書いてしまう事故（聴衆が読み上げを追ってしまう）が、
  記法のレベルで起きない。

セリフの無いスライドはノートが空ではなく**ノートごと存在しない**ので、
「まだ台本を書いていないスライド」は照合ツールから見て区別がつく。
