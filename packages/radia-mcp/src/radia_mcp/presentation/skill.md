...
### 転じ目を聴衆に見せる ― 途中の目次 (2026-09-10)

筋が転じていることと、**聴衆にその転じ目が見えている**ことは別である。
松尾先生の査読（IGTE'26、2026-09-10）:

> よくあるように途中に目次/アウトラインを挟むと分かりやすいと思いました。
> 例えば、どこまでのスライドが現状の問題(あるいは背景)で、どこからが今回の
> 解決法の話になるのか、明示する方が聴衆には分かりやすいと思いました。

要は**位置**である。冒頭に一度だけ読み上げる目次は、この指摘に答えていない。
聞き手が現在地を見失うのは冒頭ではなく中盤 ―― 従来法の話が三枚続いたあと、
いま聞いているのが「まだ問題」なのか「もう解決法」なのか分からなくなる場面
である。だから置くなら**転の直前**、あるいは同じ枚を現在地を変えて二度出す。

`presentation_check_outline_slide` がこれを検査する。見るのは三つ:

1. 目次・区切りの枚があるか
2. **それが転の直前にあるか**（冒頭だけの目次は不合格）
3. 問題の側と解決法の側を**両方名指し**しているか（章名を並べただけは不合格）

境目は `presentation_kishotenketsu_check` の**転**から取る。転はタイトルの語
ではなく下端の主張文で決まるので、「タイトルは名詞句・主張は下端帯」という
研究室の様式のままで使える ―― 「Method」「Results」という章題を前提にした
検査なら、この様式のデッキは常に落ちてしまう。目次の枚は題の語
（Outline / 目次 / 本日の流れ …）だけでなく、**本文がデッキ自身の題を並べて
いるか**でも見つける。区切りに固有の題を付けることがあるからである。

不合格のときは `suggested_outline` が、入れる位置（`insert_before_slide`）と
両側に並べる項目を枚番号つきで返す。IGTE'26 に当てると転は 5 枚目、問題側は
2–4 枚目、解決法側は 5–10 枚目だった。

枚を一枚使う余裕が無いときの答えは「無視する」ではない。**境目の枚の下端文で
章が変わったと言い切る**のでもよい。発話時間は
`presentation_estimate_per_slide_time` で測れる（区切りの枚は十数秒で済む）。
