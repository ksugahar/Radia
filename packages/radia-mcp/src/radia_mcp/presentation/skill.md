...
その章名を大きく示す疎な区切りスライドを置く。たとえば手法提案の口頭発表なら:
...
- 章名と一行だけの section card（例: `3 Results`）
- 同じ agenda を各章の先頭で再掲し、現在の章だけを色で強調する progress divider

一枚の agenda を冒頭に置くだけでは、後半で現在地を知らせられないので不十分。
...
章扉または progress divider があるかを検査する。不合格時は
