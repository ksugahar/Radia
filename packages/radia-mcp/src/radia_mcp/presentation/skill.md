...
- **赤字は各行に配らず、1 枚の核心語句 1〜2 箇所を選ぶ。** 箇条書きの前半をすべて赤くすると、赤が背景になり重要語が浮かばない。まず全文を黒で組み、聴衆に覚えてほしい語句だけを赤くする。赤字だけを斜め読みして主張の筋が追えるか、投影サイズで確認する。数値カードや図が主役の枚では、赤字を無理に追加しない。
...
- **まとめは本文の再掲にしない。** 「今回できたこと」「次に検証すること」「到達目標」など、発表の出口に必要な項目だけを各 1 行で示す。数値があるなら本文を増やさず図や短い数値カードへ置く。主張が 3 行に収まらない場合は言い換えより削除を優先し、最後に声に出して一息で説明できるか確かめる。
