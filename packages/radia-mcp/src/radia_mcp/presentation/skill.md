...
まずデッキ全体を読み、実際の内容をまとまりのある章へ分類する。各主要章の先頭で、
**全章を同じ順序で再掲し、今から始まる章だけを赤字などで強調する。**

たとえば手法提案の発表なら、各回とも次の三項目をすべて出す。

1. `Motivation` — 背景・従来法・問題
2. `Proposed method` — ゴール・着想・定式化
3. `Results` — 例題・比較・検証

Motivation の開始では 1、提案法の開始では 2、結果の開始では 3 だけを赤字、太字、
...
Discussion が複数枚あるなら、その開始にも同じ進行表示を置く。一枚だけの Summary や
...
Theory／Implementation など独自の四章以上を反復する場合は、各 Outline の章名列を直接
