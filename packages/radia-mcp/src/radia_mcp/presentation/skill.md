...
### 実測で足りなかった二つ (v1.4.x, 2026-09-06 IGTE'26 制作から)

- ✅ `presentation_check_text_box_overflow` — **本文が枠から溢れていないか**。
  生成器は題の帯と結論帯を幅に合わせて縮めるが、本文は縮めない。だから本文が
  一行増えただけで結論帯の下へ潜り込み、**描画するまで気づけない**。IGTE'26 の
  制作ではこれが五回起きた（3・5・6・2 枚目）。各 text frame の幅と run の
  font size から折り返し行数を見積もり、必要高さと枠の高さを比べる。
  一行しか入らない帯（題・結論帯・脚注・ページ番号）と `word_wrap = False` の
  枠は対象外。許容は既定 0.5 行分（切り上げの累積誤差ぶん）なので、
  「ちょうど 1 行溢れ」は取りこぼしうるが 2 行以上は確実に捕まえる。
  日本語主体なら `char_width_em=1.0`。
  **数式も文字として数える（v1.4.x で追加）**。python-pptx が読む run は
  `a:r` だけなので、OMML の数式は素通りすると文字数 0 になる。本文の半分が
  数式の 2 枚（IGTE'26 の 2・7 枚目）が「溢れなし」と報告され、描画して初めて
  帯の下に潜っていると分かった。いまは `m:t` の葉を数え、`math_char_em`
  （既定 0.6 em）で幅に換算し、**別行立ての**総和・分数・根号だけ行高を
  `tall_math_scale` 倍にする。本文に混ざった `√s` や添字で段落全体を
  高くしてはいけない（それをやると数式を含む本文が軒並み偽陽性になる）。
  `char_width_em` の既定は 0.48。太字 24 pt では 0.505、並体 20 pt では
  0.494 以下と二点で測れて、溢れるのは題ではなく本文なので並体側に寄せた。
  **残る過小評価は箇条書きの下げ幅**（プレースホルダが版下から継ぐ `marL`
  は読めない）。境界の枚は最後に必ず描画して見ること。
- ✅ `presentation_check_quoted_figure_credit` — **他者の図の出典がそのスライドに
  あるか**。ビルドが書く `figure_text_source_evidence.json` で
  `source_evidence.quoted_from` を持つ picture は引用図なので、同じスライドの
  本文に出典表記（`©` / `Fig.` / `出典` など）を要求する。逆に、出典表記が
  あるのに引用の証拠が無い picture は `unverified_credit` として報告する。
  引用図は図中文字の証拠も別扱いにすること（自作 24 pt/15 cm を一律に記録すると
  嘘になる。IGTE'26 は栗山 2019 Fig. 9(a) が 7 pt/4.44 cm = 1.58 pt/cm で、
  自作 24 pt/15 cm = 1.6 pt/cm とほぼ同比だった）。

### 転は「どこから来たか」を名乗る — inspired ロジック (2026-09-06, 菅原)

**提案は、借りた外の考えを credit つきで名指しすると初聴で通る。** 聞き手は
既に信じている考えに相乗りできるので、新奇さを一から売り込まずに済む。
IGTE'26 の転はこの形にした:

> 承（松尾先生 [4]）: 有限の梯子は有理関数だから、しっぽは整数の傾き。
> 終端はそれを振るだけで、指数は変えられない。
> 転（比留間先生の XFEM [10], [20]）: 通常の FEM の基底と enrich 関数を
> **一つの行列で**解く。借りたのは要素ではなく、その組み方。だから CLN の
> 関数と SIBC の基底を一つの Galerkin 行列に混ぜる。

書き方の型:

1. **承で不足を名指しし、誰が示したかを言う**（「松尾先生 [4] が…」）。
   出所のある不足は、出所のある解決を呼ぶ。
2. **転の頭に「どこから来たか」を置く**。要素技術ではなく**構成**を借りた、
   と言うと、自分の寄与との境界が同時に立つ。
3. **借り物には必ず出典 `[n]` を同じ枚に置く**。頭字語（XFEM, SIBC）は
   出典ではない。可能なら**その論文の図をそのまま引用**する（引用図の規則は
   上の `presentation_check_quoted_figure_credit` に従う）。研究室内で同じ図を
   別の発表も引くなら、**同じレンダリングを使う**（IGTE'26 では久保田君の
   ポスターと同じ比留間 Fig. 4 のファイルを共有した）。

`presentation_kishotenketsu_check` がこれを検査する:
`転 names where the idea came from (inspired-by, credited)`。転の**枚全体の
文字**（下端文ではない。出所は本文に書く）に inspired by / 着想 / 倣 などの語
と `[n]`・`et al.`・`Name 2024`・「先生」のいずれかが揃っているかを見て、
見つかった語と出典を `turn_inspiration` に返す。

同じ検査で 2026-09-06 に直した二つ:
- **題のプレースホルダが無いデッキ**（研究室のビルダは題の帯を自分で描くので
  `shapes.title` は `None`）では全枚が無題に見え、`Backup` の切り出しが
  効かず**隠しスライド 9 枚が筋に混ざって**いた。いまは上端の一行ボックスを
  題とみなし、**`show="0"` の隠しスライド**からを裏として切る。
- 転の語彙に研究室が実際に書く命令形（`keep` / `mix` / `join` / `enrich` …）が
  無く、転が最後のまとめ枚に見えていた。

### 内容が切り替わるたびに章扉を置く ― recurring outline (2026-09-11 修正)

ここでいう Outline は、冒頭に章名を一度だけ並べる「総目次」でも、問題と解決法の
境界を後から説明する一枚でもない。**これから何を話すかを、内容が切り替わるその場で
知らせる章扉**である。松尾先生の査読（IGTE'26、2026-09-10）を菅原が
2026-09-11 に明確化した運用:

> スライド全体から適切な分類をして、そのスライドを内容が切り替わるたびに挿入する。
> 「これから Results が始まる」「これから Motivation が始まる」と示すスライドである。

**Motivation / Proposed method / Results は例であり、固定の三分類ではない。**
まずデッキ全体を読み、実際の内容をまとまりのある章へ分類する。各主要章の先頭に、
**全章を同じ順序で再掲し、今から始まる章だけを赤字などで強調した** Outline を置く。
たとえば手法提案の口頭発表なら:

1. `Motivation` の章扉 → 背景・従来法・問題
2. `Proposed method` の章扉 → ゴール・着想・定式化
3. `Results` の章扉 → 例題・比較・検証

Discussion が複数枚あるなら、その開始にも章扉を置く。一枚だけの Summary や謝辞の前に
機械的に章扉を増やす必要はない。分類は枚数合わせでなく、**話の責任が変わる地点**から
決める。

各章の開始時に**全項目をもう一度見せる**ことが要点である。現在章だけを赤字、太字、
大きな文字などで一意に強調し、他の章は同じ弱い書式にそろえる。現在章だけを載せた
疎な section card は、全体のどこにいるかを示さないので不合格とする。

一枚の agenda を冒頭に置くだけでも、後半で現在地を知らせられないので不十分。
逆に「ここまでが問題、ここからが解決法」とだけ書く境界スライドも、次に始まる章を
直接名乗らない限り章扉ではない。

`presentation_check_outline_slide` は、起承転結から共通の
Motivation／Proposed method／Results の切り替わりを推定し、それぞれの開始位置に
全項目を再掲し現在章だけを強調した progress divider があるかを検査する。不合格時は
`suggested_outline` に欠けている章と挿入位置を返す。ツールの分類は補助であり、
最終的な章分けはスライド全体を読んだ人間またはエージェントが内容から決める。

### 落とした話は捨てない ― 非表示の質問スライドにする (2026-09-10)

菅原の規約:

> 展開点の話は質問スライドにしましょう。質問スライドは非表示スライドとして
> 残すとして、radia-mcp に残しましょう。

本編を削るとき、その枚を**消さない**。`show="0"` の非表示スライドとして同じ
ファイルの後ろへ移す。理由は三つある。問われたその場で出せる。図・出典・
セリフが一緒に残るので、作り直しにならない。そして別ファイルへ逃がすと本番で
開けない。

IGTE'26 の「Moving the expansion point」（栗山ら 2019 の引用図つき、1:08）が
この扱いの最初の例である。松尾先生の「あらすじは無い方が分かりやすい」に
従って本編から外し、「では展開点を動かせばよいのでは」と問われたときの答え
としては残した。本編は 13:22 → 12:14（125 wpm）になり、初めて余裕ができた。

`presentation_check_qa_backup_slides` は控えの枚数だけでなく、**非表示か**を
見る。題が `Backup` / `Q&A` / `補足` でも表示のままなら本番で映るので、
`named_but_visible` に分けて `warnings` を出す ―― 名前で数えるだけの検査は、
消し忘れた枚を「控えが足りている」と報告してしまう。

落とした枚を控えへ移したら、**筋の検査をやり直すこと**。IGTE'26 では一枚
抜いた結果、承を述べていた枚が消えて `presentation_kishotenketsu_check` の
転がまとめの枚へ滑り落ちた（点数 9 点台 → 3.8）。検査側の欠陥二つが同時に
露見したので直してある（下端文を頁番号から拾っていた／`no rational model
makes ...` を不足と認めていなかった）が、**一枚動かしたら筋は測り直す**の
が規則である。

## 🖼️ 生成 AI のデッキを「見せ方の参照」にする (2026-09-02)

Gemini Notebook などが作るデッキは、それなりに整っているが **各枚が 1 枚の
ラスタ画像**で、文字も図も編集できない(IGTE'26 の例: 14 枚とも 1376×768 px、
45 cm 幅で約 78 dpi)。納品物にはならないが、**見せ方の参照としては有用**。
使い方は「写す」ではなく「測る」:

1. `presentation_check_raster_slides(pptx)` で一枚貼りかどうかと実効 dpi を確かめる
   (編集不能・投影で滲む、を先に言う)。
2. 文字寸法は画素で測る: 文字高さ px ÷ (画像幅 px / スライド幅 cm) → cm → pt。
   スライド幅が違えば**幅で比例換算**して自分のデッキと比べる。
   IGTE'26 の実測(当方 33.9 cm 幅換算): 見出し ≈37 pt、本文 ≈25 pt、結論 ≈29 pt。
   当方(36→28 / 24 / 24)と**寸法は同じ**だった。差は書体の重さ・語数・毎枚の図。
3. 採るもの: **曲線は凡例でなく矢印付きの名札で名指し**
   (`radia_mcp.figure.place_label_arrow`; 凡例は 4 項目になるとどの隅も空かず
   `place_legend_clear` が "outside" を返す = 固定キャンバスで切れて**消える**)、
   結論の帯は **28 pt 太字**(長文は 26/24 に落とす)、本文の要語だけ太字、
   文字だけの枚に概念図、方式比較は ✓✗ の表。
4. 採らないもの: ラスタ一枚貼り、図中の幻覚文(意味のない語列が小さな注記に出る)、
   "perfectly" "proof" "true accuracy" の言い過ぎ(結果の数字は良いが一般命題に
   すると過大主張)、捏造の QR コードや謝辞。

markdown から組むデッキで `**太字**` が文字のまま残るのは
`presentation_check_raw_math_markup` が `markdown_emphasis` として拾い、
`presentation_apply_bold_markers` が太字 run に直す(run を分割し、寸法・色・書体は
元の run から継ぐ)。

## 🧭 レビューメモをセリフ・ノート・スライド・図の順に反映する (2026-09-03)

IGTE'26 の改善メモ(菅原)から一般化した教訓。**セリフが正**で、反映は
セリフ → 英語ノート → スライド本文 → 図 の順に同期し、直感的な主メッセージは
文だけでなく図にする(役割分担なら周波数軸の帯図: 梯子の帯・受け渡し・表面
モードの帯を実曲線の上に塗る)。

1. **先行研究は否定ではなく拡張として位置づけ、冒頭で言う。**「欠点を暴く」
   口調は聴衆(先行研究の同僚が座っている)を敵に回す。「One of the
   successful approaches is X; we keep X for what it is good at and add
   one thing at the other end」で足りる。
2. **否定形の主張は、否定している前提を聴衆が知っているときだけ効く。**
   "nothing fitted" は当てはめで接続した未発表の先行構成を知る著者には要点
   でも、聴衆には「そもそも当てはめが要るとは思っていなかった」ので空振り。
   `presentation_check_negated_premise(pptx)` が、否定形("no X" "nothing
   X-ed" "without X" "X-free" 「X なし」「X 不要」)のうち前提 X が先行
   スライドに否定なしで現れないものを拾う。直し方は二つ: 前提を先に一行で
   立てる(「通常の補修は接続周波数を当てはめる」)か、否定をやめて代わりに
   何をするかを言う(「接続は射影が決める」)。技術的な補足なら "no
   empirical crossover parameter is introduced" 程度に留める。
3. **許容誤差に依存する最上級は言わない。**「最小のモード数」は遷移帯域を
   どこまで再現したいかで変わる。安全な主張は「比較的少ない基底関数で広い
   帯域を表現できる」。`presentation_check_quantitative_claim_context` の
   対象でもある。
4. **式を書いたとき現れる名前のある対象は「形」であって「手法」ではない。**
   Schur 補元・DtN 写像・Steklov–Poincaré は、ポート量を陽に書けばそうなる
   というだけで、計算は小さなブロック系を解いて終わり。スライドの題に
   しない("Schur complement, not a fit" → "One system, coupled by the
   projection")。導出は裏スライドへ。
5. **「なぜ段数を増やさないか」型の想定問答は 構造 → 数値 → 費用 の順。**
   有理関数の傾きは整数(構造)が主、桁落ち(16・8・4・2 桁)は従、メッシュ
   細分化の費用は別の問いへの答え。順を逆にすると「精度が足りないだけ」に
   聞こえる。
6. **系譜は「自然な前段階」として示す。**先行法が同じ発想の途中にあること
   (二つの領域を使う)を認めてから、越えた一点(第二の点を無限遠に)を言う。
   類似は位置の類似で種類の類似ではない、という限界も質疑用に持つ。
7. **文を足したら時間を測り直し、超えたら文を丸ごと落とす。**圧縮は禁止
   (文の圧縮は聞き取れない)。落とす候補は、別の枚に移った内容の重複、
   裏スライドが答える質問の先回り、限界の列挙のうち裏にある項目。
...
- **「112%」等の拡大率指定は、幅・高さへ同じ係数を掛ける。** 孤立図は中心、
  横並び図は中心線とラベル側の基準線など、意味のある整列基準を保って等方拡大する。
  左上固定で隣接間隔を崩さない。拡大後はcaption、ラベル、区切り線との重なりを再検査する。
...
- グラフ、模式図、線画は、利用環境が対応する限りPNGへラスタ化せずSVGを優先する。
  PowerPointの`asvg:svgBlip`だけを持つ図も正常なベクタ図として扱い、`viewBox`で
  縦横比、SVG実体のSHA-1で証跡を照合する。SVGの等方拡大は画質違反にしない。
  等方縮小後の図中文字が20 pt未満になる場合と、非等方変形は違反とする。
...
- `presentation_check_embedded_figure_text_size(path)` で画像内文字を検査する。
  ...
  実効フォントサイズを算定する。PowerPointや変換ツールがshape名を書き換えるため、
  manifestは可変なshape名だけに依存せず、画像blobのSHA-1である`asset_sha1`を併記する。
  20 pt未満と未確認画像はともに0件を納品条件とする。
- `presentation_speaking_pace_estimate`、タイトルoutline、takeaway検査では、
  `補足`、`質問対策`、`予備`、`付録`、`Appendix`、`Backup`で始まるスライドと
...
- **強調する百分率は「量・位置／集約・基準・条件」と一体で示す** — `0.28%`
  や `0.18%以内` だけでは、どこの何を何と比較した値か分からない。
  `ギャップ中央の Bz：FEM 基準値との差 0.28%`、
  `鉄心体積平均 Mz：Mesh A 基準の相対差 -0.17%` のように自己完結させる。
  `q6/q10/q20` のような実装内部の mesh ID は、物理的な分割条件を定義しない限り
  画面に出さない。非常に小さい値はデータ精度を超える桁を並べず、
  `5.6×10^-5%` のように科学表記と妥当な有効数字を用いる。
  `presentation_check_quantitative_claim_context` で補助点検する。
...
- 直線が図と重なる場合は 点線 / 曲線 に
- 矢印を縁取りしない
- **矢印と中立な区切り線・補助直線が交差する場合、矢印を最前面に置く。**
  直線は背景の構造、矢印は方向・因果・遷移という意味を担うため、主張の強い矢印を
  区切り線で欠けさせない。生成順は区切り線を先、矢印を後とし、
  `presentation_check_arrow_layering` で z-order 違反0件を確認する。
...
### 追-D2. 数値解析スライドの再現性と表示互換性

- **課題と原因を同じ階層で混同しない。** 下端や見出しには「歪んだ要素に弱い」のような観測された課題を書く。「選点法が評価点に敏感」は、その課題を生む原因として図中の因果線またはノートで一段下げて説明する。
- **短い定量ラベルは、24 pt 以上で収まるなら1行にする。** 「評価位置＋物理量＋比較対象」を途中で分断しない。たとえば「ギャップ中央 Bz の FEM 差」は一つの評価量なので、語句の途中で改行しない。
- **数式は記号定義と物理的な帰結を同じスライドかノートで結ぶ。** 式を置くだけで終えず、「分母に透磁率が入るため、高透磁率で対角優勢が弱まる」のように、式がそのスライドの課題へどう効くかを説明する。
- **PowerPointで数式を実表示して検査する。** Eqnedit64等のベクトル原本は保存するが、SVG-onlyの埋め込みで文字化け・欠落が起きる場合は、高解像度透過PNGまたはOffice互換フォールバックを併設する。生成元の正しさだけでなく、対象環境のPowerPoint表示を合否基準とする。
- **メッシュ依存の結果には要素種・次数・分割を明示する。** hex、tet、wedge、構造・非構造、要素次数、分割数のうち解釈に必要な条件を見える場所へ置く。「ひずみに強い」と述べる場合、要素数・要素種・ひずみが同時に変わる比較なら、その限定をノートで明示する。
- **null/loopモードは観測不能性だけで説明しない。** 外部場に現れるか、内部変数がどこまで増幅するか、発生位置が反復で移動するかを示し、非線形材料則・履歴則への影響までつなぐ。
- **日本語ノートの外部書き出しはUTF-8 BOM付きとする。** PPTX内のnotes XMLがUTF-8として復号でき、置換文字 `U+FFFD` を含まないことも確認する。BOMなしUTF-8を誤判定するWindows編集環境を前提に、文字コードを配布前に実検査する。
