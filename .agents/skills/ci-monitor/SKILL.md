...
- **Right after `git push` of a release** (Phase 7 of `release-triple`):
...
Explicit run IDs (release-triple Phase 7 idiom):
...
## Integration with release-triple

`release-triple` skill Phase 7 is the canonical caller.  The
release-triple flow ends with three CI runs (main + 2 tags); call
