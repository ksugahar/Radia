...
% Three startup repeats do not fully warm MATLAB's JIT for this workload.
% Prime the complete workload first, symmetrically with the Python runner.
[~, warmScalarChecksums] = measure(@runScalar, trials, repeats);
[~, warmGroupedChecksums] = measure(@runGrouped, trials, repeats);
measureTrialsDataframe(dataframeTrials, repeats);
verifyChecksums(warmScalarChecksums, expectedScalarChecksum, "scalar prewarm");
verifyChecksums(warmGroupedChecksums, expectedGroupedChecksum, "grouped prewarm");
...
[dataframeSeconds, dataframeRows, dataframeColumns] = ...
    measureTrialsDataframe(dataframeTrials, repeats);
...
result = struct( ...
    ...
    "grouped_conditional", summarize(groupedSeconds, groupedChecksums, ...
        trials, warmupRepeats), ...
    "trials_dataframe", struct( ...
        "all_seconds", dataframeSeconds, ...
        "median_warmed_seconds", ...
            median(dataframeSeconds(warmupRepeats + 1:end)), ...
        "rows_per_second", dataframeRows / ...
            median(dataframeSeconds(warmupRepeats + 1:end)), ...
        "rows", dataframeRows, ...
...
function [durations, rows, columns] = measureTrialsDataframe(trials, repeats)
modes = ["A", "B"];
...
for index = 1:trials
    study.add_trial(radia.optuna.create_trial( ...
        ...
        params=struct( ...
            "x", (index - 1) / trials, ...
            "mode", modes(mod(index - 1, 2) + 1)), ...
        distributions=distributions, ...
        user_attrs=struct("owner", "lab"), ...
        system_attrs=struct("origin", "benchmark")));
end
attrs = ["number", "value", "params", "user_attrs", ...
    "system_attrs", "state"];
expectedColumns = ["number", "value", "params_mode", "params_x", ...
    "user_attrs_owner", "system_attrs_origin", "state"];
...
columns = width(frame);
if rows ~= trials || ...
        ~isequal(string(frame.Properties.VariableNames), expectedColumns)
    error("radia:optuna:BenchmarkDataframe", ...
        "trials_dataframe benchmark contract changed.");
end
end
