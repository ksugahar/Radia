#!/usr/bin/env python3
"""
Benchmark script for PrepareCache() batch evaluation optimization.
Verifies that H-matrix acceleration works with GridFunction.Set()

Run directly: python test_batch_evaluation.py
Not a pytest test (uses module-level benchmark code).
"""
...
def main():
    import radia as rad
    import ngsolve
    from ngsolve import HDiv, GridFunction, Mesh
    ...
    B_cf_standard = rad.RadiaField(magnet, 'b')
    ...
    with TaskManager():
        ...
        B_cf_batch = rad.RadiaField(magnet, 'b')
