...
checkedSources = [ ...
    ...
    "src/core/rad_hacapk_hdiv.h", testFiles];
sourceRecords = repmat(struct("path", "", "sha256", ""), numel(checkedSources), 1);
for index = 1:numel(checkedSources)
    sourceRecords(index) = struct("path", checkedSources(index), ...
        "sha256", sha256TextFile(fullfile(root, checkedSources(index))));
end
...
provenancePath = mexPath + ".build.json";
if ~isfile(provenancePath)
    error("radia:validation:BuildProvenance", ...
        "Build with -MatlabMexOnly -RequireNativeProvenance before validation.");
end
buildProvenance = jsondecode(fileread(provenancePath));
if string(buildProvenance.schema) ~= "radia.native-build-provenance.v1" || ...
        buildProvenance.source_dirty || ...
        string(buildProvenance.binary_sha256) ~= sha256File(mexPath)
    error("radia:validation:BuildProvenance", ...
        "The MEX must match its clean-source build provenance.");
end
...
artifact = struct( ...
    ...
    "native_build_provenance", buildProvenance, ...
    "checked_source_hashes", sourceRecords, ...
    "execution_launcher", string(getenv("RADIA_VALIDATION_LAUNCHER")), ...
