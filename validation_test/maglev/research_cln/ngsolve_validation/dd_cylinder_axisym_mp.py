"""dd_cylinder_axisym_mp.py — DD axisym VIM cylinder, multiprocessing K assembly.

Same MP strategy as dd_sphere_axisym_mp.py. K row-wise partitioned over workers,
each accumulates local DD K, then main reduces via dd_add.
"""
...
def assemble_K_M_b_cylinder_dd_mp(Nr, Nz, n_quad=2, n_workers=None,
                                    dps=40, verbose=True):
    ...
    for i in range(Nr):
        for j in range(Nz):
            ...
            for qr in range(n_quad):
                r_q_mp = r_a_mp + dr_mp * nodes_01[qr]
                wr_mp = dr_mp * w_01[qr]
                for qz in range(n_quad):
                    z_q_mp = z_a_mp + dz_mp * nodes_01[qz]
                    wz_mp = dz_mp * w_01[qz]
...
def main():
    ...
    print("=" * 72)
    print("DD axisym VIM cylinder (multiprocessing)")
    print("=" * 72)
    print(f"\n  Nr={args.Nr}, Nz={args.Nz}, total {args.Nr * args.Nz} cells")
    print(f"  Workers: {args.workers or (multiprocessing.cpu_count() - 1)}, "
          f"n_quad={args.nquad}, eigh_dps={args.eigh_dps}")

    t0 = time.time()
    K_hi, K_lo, M, b = assemble_K_M_b_cylinder_dd_mp(
        args.Nr, args.Nz, n_quad=args.nquad,
    ...
    print(f"\n  Cauer rungs (DD cylinder axisym, MP):")
    ...
    out_path = (Path(__file__).parent
                / f"dd_cylinder_axisym_mp_Nr{args.Nr}_Nz{args.Nz}_"
                f"nq{args.nquad}_results.json")
    out_path.write_text(json.dumps({
        "shape": "cylinder", "Nr": args.Nr, "Nz": args.Nz,
