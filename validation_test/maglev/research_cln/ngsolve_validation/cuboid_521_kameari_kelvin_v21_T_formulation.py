"""Kameari + Kelvin v21: T-formulation source projection (2026-05-05).

Final best-effort attempt for FEM Kameari to reach BEM HDiv canonical
43.84 us. Sugahara verdict: if v21 still doesn't reach 43.84, that
confirms FEM Kameari for 3D isolated-conductor-in-vacuum is structurally
limited and BEM Foster-CLN is the practical Radia solution.

T-formulation source projection (de Rham complex):
    HCurl(C, t·T=0) -∇×→ HDiv(C, J·n=0 weak per face) -∇·→ L²(C)
By taking T in HCurl(C) with full Dirichlet, J = curl(T) is automatically:
    - div-free (∇·∇× ≡ 0)
    - J·n = 0 weakly on each face: ∫_face (∇×T)·n dS = ∮_∂face T·dl = 0

Step 1: project σA_phys onto curl(HCurl_0(C)) by solving:
    ∫_C ν₀ curl(T)·curl(s) dV = ∫_C σA_phys · curl(s) dV  ∀s ∈ HCurl_0(C)
Step 2: J_0 := curl(T) — div-free + J·n=0 weak source for Kameari iteration.
Step 3: Standard HCurl + Kelvin Kameari iteration as v17/v18 with J_0.

(original v20 docstring follows for reference)
...
print("Starting v21 (T-formulation source: J = curl(T) auto-div-free)...",
      flush=True)
...
def main():
    ...
    # === T-formulation source projection (v21) ===
    # Build T in HCurl(conductor) with t·T=0 on all 6 conductor faces.
    # Solve: ∫_C ν₀ curl(T)·curl(s) dV = ∫_C σA_phys · curl(s) dV  ∀s
    # Then J_0 = curl(T) is automatically div-free + J·n=0 weak per face.
    print("  Building T space (HCurl on conductor, full Dirichlet, "
          "tree-cotree gauge)...", flush=True)
    t0 = time.time()
    fes_T = HCurl(mesh, order=ORDER,
                   dirichlet="conductor_surface",
                   definedon=mesh.Materials("conductor"),
                   nograds=True)
    # Apply tree-cotree gauge mask on conductor mesh edges
    cond_tree_edges = build_spanning_tree(mesh)  # uses full mesh tree
    fd_T = fes_T.FreeDofs()
    n_masked_T = 0
    for edge_nr in cond_tree_edges:
        edge = mesh.edges[edge_nr]
        dofs = fes_T.GetDofNrs(edge)
        if dofs and dofs[0] >= 0 and fd_T[dofs[0]]:
            fd_T[dofs[0]] = False
            n_masked_T += 1
    print(f"  T HCurl ndof = {fes_T.ndof}, "
          f"FreeDofs (after tree-cotree) = {sum(fd_T)} "
          f"(masked {n_masked_T})", flush=True)

    T_trial, T_test = fes_T.TnT()
    a_T = BilinearForm(fes_T)
    # Pure L² projection: curl-curl on T with tree-cotree gauge (no mass).
    a_T += curl(T_trial) * curl(T_test) \
        * dx("conductor", bonus_intorder=BONUS_INT)

    f_T = LinearForm(fes_T)
    f_T += sigma_Cu * A_s_phys_cf * curl(T_test) \
        * dx("conductor", bonus_intorder=BONUS_INT)
    ...
    with TaskManager():
        ...
        f_T.Assemble()
        try:
            inv_T = a_T.mat.Inverse(fd_T, inverse="pardiso")
        except Exception:
            inv_T = a_T.mat.Inverse(fd_T, inverse="sparsecholesky")
        gfT = GridFunction(fes_T, name="T_source")
        gfT.vec.data = inv_T * f_T.vec
    print(f"    {time.time()-t0:.1f}s", flush=True)

    # J_0 := curl(T) automatically div-free + J·n=0 weak per face
    n_normal = specialcf.normal(3)

    # Diagnostic: weak J·n=0 — integrate (curl T)·n on each face
    Jn_int = float(Integrate(
        (curl(gfT) * n_normal) ** 2
        * ds("conductor_surface", bonus_intorder=BONUS_INT), mesh))
    print(f"  ∫_∂C |curl(T)·n|² dS = {Jn_int:.4e} "
          f"(weak J·n=0 from t·T=0 Dirichlet)", flush=True)

    # How well does curl(T) approximate σA_phys in conductor?
    proj_err_int = float(Integrate(
        (curl(gfT) - sigma_Cu * A_s_phys_cf) *
        (curl(gfT) - sigma_Cu * A_s_phys_cf)
        * dx("conductor", bonus_intorder=BONUS_INT), mesh))
    proj_norm_int = float(Integrate(
        (sigma_Cu * A_s_phys_cf) * (sigma_Cu * A_s_phys_cf)
        * dx("conductor", bonus_intorder=BONUS_INT), mesh))
    print(f"  ||curl(T) - σA_phys||/||σA_phys|| in conductor = "
          f"{(proj_err_int/proj_norm_int)**0.5:.4e}", flush=True)

    # Use curl(T) as the iteration source — it's a CF, not GridFunction,
    # but NGSolve handles this in LinearForm assembly.
    A_s_corr_n_int = Jn_int
    A_s_n_orig_int = float(Integrate(
        (sigma_Cu * A_s_phys_cf * n_normal) ** 2
        * ds("conductor_surface", bonus_intorder=BONUS_INT), mesh))
    curl_diff = proj_err_int

    # Build a HDiv-equivalent source: for the iteration we need a CF.
    # Use the symbolic curl(gfT) directly — it's div-free + J·n=0 weak.
    gfJ_imp = curl(gfT)  # symbolic CF
    print("  J_0 = curl(T) prepared (div-free + J·n=0 weak per face)\n",
          flush=True)
    ...
    # R_0 check via the T-projected J source: <J_imp, J_imp/sigma>_C
    # Note: gfJ_imp is symbolic curl(gfT), evaluated within Integrate.
    ...
    out = {"method": "HCurl + Kelvin Kameari iteration (v21: T-formulation source, J = curl(T))",
           "L_n_formula": "L_n = R_n² × ∫_full ν |curl(gfA_r)|² dV (canonical B² form)",
           "gauge_recipe": "nograds=True + tree-cotree + Helmholtz-Hodge + T-formulation source (no penalty)",
    ...
    out_path = Path(__file__).parent / "cuboid_521_kameari_kelvin_v21_T_formulation.json"
