...
opts=IHOperatorAssemblyOptions(axisymmetric_thermal_vol='fixture.vol',sample_time_s=.1)
...
cfg.update(n_heat=len(op.heat_dofs),heat_projection=op.unit_heat_density_W_per_m3.tolist(),heat_cell_weights=op.heat_cell_weights_m3.tolist(),temperature_cell_weights=op.temperature_cell_weights_J_per_K.tolist(),initial_temperature_K=[293.15]*mesh.nv)
with ng.TaskManager():
    fes=ng.H1(mesh,order=1);u,v=fes.TnT();w=2*np.pi*ng.x
...
out=Path('C:/temp/ih-axisym-native/axisymmetric');out.mkdir(parents=True,exist_ok=True)
