...
def run(step_path: Path, *, frequency: float, maxh: float,
    ...
    if experimental_peec:
        ...
        result["status"] = ("exploratory scalar comparison only; current/loss "
                            "maps and PEEC surface-element validation pending")
    return result
...
def main():
    ...
    print(json.dumps(run(args.step, frequency=args.frequency,
                         ...
                         experimental_peec=args.experimental_peec), indent=2))
