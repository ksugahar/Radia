...
repoRoot=fileparts(fileparts(fileparts(mfilename("fullpath"))));addpath(fullfile(repoRoot,"matlab"));
...
handoffs=counts-1;slope=handoffs(:)\errors(:);
gates=struct("finite",all(isfinite(errors)),"bounded",all(errors<1e-2), ...
 "approximately_linear",corr(handoffs(:),errors(:))>0.98,"nondecreasing",all(diff(errors)>0));
result=struct("schema","radia.validation.ltspice.interval_handoff.v1", ...
 ...
 "relative_error_per_handoff_fit",slope,"gates",gates);
