...
Also guards the 2026-06-17 coil-bug fix: this solver used to rad.UtiDelAll() AFTER building the coil
...
def coil_script(tmp_path_factory):
    ...
    p = tmp_path_factory.mktemp("accel_hdiv") / "loop_coil.py"
    p.write_text(
        "import sys, os\n"
...
def test_hdiv_rejects_multimaterial(tmp_path, coil_script):
    ...
    r = solve_hdiv(coil_script=coil_script, vol_file=vol, mat=_linear_mat(),
...
def test_hdiv_ima_rejected(tmp_path, coil_script):
    """demag_backend='hdiv' with an IMA image string returns a clean error in this TET-only panel."""
    from calc_accel_hdiv import solve_hdiv
    p = _iron_only_yoke_vol(tmp_path / "half_yoke.vol")
    r = solve_hdiv(coil_script=coil_script, vol_file=str(p), mat=_linear_mat(),
