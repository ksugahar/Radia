...
| `test_hex_native_design_lane.py` | the complementary Sculpt + HDiv-MMM half: sector domain STL -> `cubit_stl_to_vol` hex (size 0.012, 360 hexes, min quality 0.540) -> the SLP design loop and the hex path of `iron_only_mesh` / `verify_design_iron_only` | all mesh gates incl. `boundary_faces_ok`; accepted iterates monotone with gain in [0.5 %, 15 %] (+3.3 % measured on the 2026-08-09 tree; the iterate count tracks the in-development SLP driver, so the gates are deliberately tree-robust); iron extraction stays hex (170 elements); matched-0/1 ersatz bands abs < 20 %. Requires a Cubit license and runs headless (~3-4 min). |
