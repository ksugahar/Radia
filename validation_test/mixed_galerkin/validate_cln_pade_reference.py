...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path(__file__).parent / "results/cln_pade_reference.json")
    ...
    with mp.workdps(100):
        ...
        moments = [mp.mpf(1)] + [
            (-1)**k * mp.fsum(4 / x**(2*k + 2) for x in zeros)
            for k in range(1, 21)]
        for order in (1, 4, 10):
            for kind in ("L", "R"):
                ...
                for frequency in (1e-3, 1.0, 1e3, 1e6):
                    ...
                    expected = complex(mp.polyval(list(reversed(p)), u) /
                                       mp.polyval(list(reversed(q)), u))
                    actual = complex(module.Y_cln_pade(complex(u), order, 1.0, 1.0, 1.0, kind=kind) / math.pi)
                    error = abs(actual - expected) / abs(expected)
                    rows.append({"N": order, "kind": kind, "u_imag": frequency,
                                 "reference_ratio": [expected.real, expected.imag],
                                 "actual_ratio": [actual.real, actual.imag],
                                 "relative_complex_error": error, "passed": error < 1e-8})
    git = subprocess.run(["git", "-c", f"safe.directory={ROOT.as_posix()}", "rev-parse", "HEAD"],
                         cwd=ROOT, capture_output=True, text=True, check=True)
    import hashlib
    result = {"schema": "radia.cln-pade-reference.v1", "passed": all(row["passed"] for row in rows),
              "utc": datetime.now(timezone.utc).isoformat(), "host": platform.node(),
              "source_base_commit": git.stdout.strip(), "source_sha256": hashlib.sha256(source.read_bytes()).hexdigest(),
              "elapsed_seconds": time.perf_counter() - started,
              "python": platform.python_version(), "numpy": np.__version__, "scipy": scipy.__version__,
              "mpmath": mp.__version__, "precision_digits": 100, "n_modes": 200,
              ...
              "relative_tolerance": 1e-8, "cases": rows}
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    print(f"{sum(row['passed'] for row in rows)}/{len(rows)} passed; max error={max(row['relative_complex_error'] for row in rows):.3g}")
