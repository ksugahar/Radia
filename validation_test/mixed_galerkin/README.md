...
| `time_domain/` | Time-domain realization in the settled time-domain-SIBC form: passive real-pole Foster fit of Y_mixed(s) (dominant poles from AAA, log-spaced diffusive filler, NNLS residues) → **18** real poles (cube), DC exact, step response within 0.10 % of the exact inverse Laplace transform (Talbot) on 1e-8..1e-1 s. The one-sided AAA poles used directly (22 poles, 21 complex, non-conjugate) are kept as a recorded negative result: their step response strayed 10.7 % below 1 µs (fixed 2026-09-04). No `d` tuning. Re-run 2026-09-02 after fixing a hardcoded path to the retired `examples/` tree that had left the script un-runnable; earlier text said 21. The script exposes `summary()`, so `emit_results.py` records it as `cube3d_time_domain_aaa` with the step-response curves; the IGTE talk draws its time-domain figure from that record. |
...
| `cube3d/09_wedge_basis.py` | a non-separable basis with genuine 2D structure across a z-edge, plus 11 symmetry images | RUN 2026-09-02: DIVERGES. One z-edge wedge, no symmetry images, against the Mellin asymptote: 21.8 % at 10 kHz, 292 % at 1 MHz, 2974 % at 100 MHz, where the tensor baseline gives 0.55 %, 0.045 %, 0.0044 %. The error grows with frequency instead of shrinking, so the function as written is not asymptotically admissible either. The edge remains open |
| `_references/box_heat_content.py` | EXACT admittance of slab / square / cube by the heat-content integral `<v> = -sMS int exp(-sMS tau) m(tau)^D dtau` along the pi/4 ray | slab closed form reproduced to 1e-13; cube |Y|(10 kHz) = 3.431902 (Aitken Foster N=799: 3.431919). Replaces the pending NGSolve ground truth for boxes. 2026-09-04 |
| `cube3d/10_edge_corner_dofs.py` | CONFORMING localised bumps `d = g(t) - g(2t)` give the face (`d F F`), edge (`d d F`) and corner (`d d d`) regions their own amplitude beside the tensor envelope; exact reference | DONE 2026-09-04 (artifact case `box_edge_corner_dofs`): cube envelope-only floor 0.71 % (2 unknowns) .. 0.21 % (126); + corner 0.16 %; + face 0.20 %; **+ EDGE 0.014 %** (15x, at every frequency); + edge + corner 0.0125 %, 0.030 % with 11 unknowns; square 0.34 % -> 0.006 %. The single coefficient gets the EDGE amplitude wrong; the rest is the transition-band coupling with the bulk, which falls with the bulk rank (0.108 % at 4 unknowns -> 0.0125 % at 128). Frozen-ratio test: the free amplitude ratios converge to xi_E/xi_1 = 0.800 and xi_C/xi_1 = 0.457 (real) above 1 MHz but are complex and swing through the transition band; ONE surface function with those ratios frozen at their 1 GHz values equals the free 3-DOF result above 1 MHz and fails in the transition band unless the bulk is rich (n<=3: 0.21 % vs 0.030 %; n<=1: 0.74 % vs 0.11 %; n<=9: 0.020 % vs 0.0125 %). So a better fixed envelope suffices asymptotically; in the transition band the edge and corner amplitudes must be independent DOFs because they couple to the interior modes. |
...
| 3D cube     | 0.33% vs the exact heat-content admittance (rank 20, closed K_ss; also the NGSolve FEM) -> 0.0125% with an independent EDGE amplitude (`cube3d/10`) | — | — | |
