...
monitor = output{3}.Values;
requiredMonitorFields = ["header","sample_index","total_heat_W", ...
    "temperature_mean_K"];
if ~isstruct(monitor) || ...
        ~all(isfield(monitor, cellstr(requiredMonitorFields))) || ...
        ~isstruct(monitor.header) || ...
        ~isfield(monitor.header, "schema_version") || ...
        monitor.header.schema_version.Data(end) ~= 1 || ...
        ~isfinite(monitor.total_heat_W.Data(end)) || ...
        ~isfinite(monitor.temperature_mean_K.Data(end))
    error("radia:simulink:IHReleaseMonitor", ...
        "The native IH smoke simulation did not publish valid IHMonitorBusV1 telemetry.");
end
