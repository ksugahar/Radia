...
arguments
 ...
 options.RootDirectory (1,1) string="C:\temp\radia_ltspice_block"
end
...
parameters=sprintf('''%s'',%s,%s,%.17g,''%s'',%.17g,%.17g,''%s''',options.Netlist,cellLiteral(options.InputNames),cellLiteral(options.OutputTraces),options.SampleTime_s,replace(options.RootDirectory,"'","''"),options.MaxStep_s,options.Timeout_s,options.Executable);
