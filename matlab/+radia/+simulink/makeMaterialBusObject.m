function busObject = makeMaterialBusObject(runtime, options)
%MAKEMATERIALBUSOBJECT Create the fixed-width Radia material Simulink.Bus.

arguments
    runtime (1,1) struct
    options.Name (1,1) string = "RadiaMaterialBus"
    options.AssignToBase (1,1) logical = true
end

names = string(fieldnames(runtime));
elements = repmat(Simulink.BusElement,numel(names),1);
for k = 1:numel(names)
    value = runtime.(names(k));
    if ~(isnumeric(value) || islogical(value)) || ~isreal(value)
        error("radia:simulink:MaterialBusField", ...
            "Runtime field '%s' must be real numeric or logical.",names(k));
    end
    element = Simulink.BusElement;
    element.Name = char(names(k));
...
busObject.Description = "Fixed-width Radia material and .vol region contract";
busObject.Elements = elements;
if options.AssignToBase
    assignin("base",char(options.Name),busObject);
end
end
