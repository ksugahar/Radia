...
if isfield(config, "temperature_constant_coefficients")
    if ~isfield(config,"temperature_representation") || ...
            string(config.temperature_representation) ~= "ngsolve-h1-coefficients" || ...
            string(config.rotation_mode) ~= "none"
        error("radia:simulink:IHConfigTemperatureRepresentation", ...
            "FE coefficients require an explicit ngsolve-h1-coefficients representation and rotation_mode=none.");
    end
    config.temperature_constant_coefficients = finiteVector( ...
        config.temperature_constant_coefficients,nTemperature,"temperature_constant_coefficients");
    config.temperature_cell_weights = finiteVector(config.temperature_cell_weights,nTemperature,"temperature_cell_weights");
    config.initial_temperature_K = finiteVector(config.initial_temperature_K,nTemperature,"initial_temperature_K");
    stats = radia.simulink.ihTemperatureStatistics(config,config.initial_temperature_K);
    if stats(1) <= 0
        error("radia:simulink:IHConfigTemperatureRepresentation", "Initial evaluated temperatures must be positive kelvin.");
    end
else
    if isfield(config,"temperature_representation") && string(config.temperature_representation) ~= "nodal-kelvin"
        error("radia:simulink:IHConfigTemperatureRepresentation", "Missing temperature_constant_coefficients.");
    end
    config.temperature_cell_weights = positiveVector(config.temperature_cell_weights,nTemperature,"temperature_cell_weights");
    config.initial_temperature_K = positiveVector(config.initial_temperature_K,nTemperature,"initial_temperature_K");
end
