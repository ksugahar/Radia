function configureStreamFunctionOptimizationSubsystem(blockPath)
%CONFIGURESTREAMFUNCTIONOPTIMIZATIONSUBSYSTEM Apply mask values internally.
arguments
    blockPath (1,1) string
end
if getSimulinkBlockHandle(blockPath) < 0
    error("radia:simulink:StreamFunctionTopologyBlock", ...
...
runnerExpression = string(get_param( ...
    blockPath,"radia_streamfunction_adjoint_runner"));
sampleTime = string(get_param(blockPath,"sample_time_s"));
historyCapacity = string(get_param(blockPath,"history_capacity"));
set_param(blockPath + "/Stream Function Design","Parameters", ...
    "'streamfunction'," + configFile + "," + runRoot + "," + ...
    timeout + "," + python);
set_param(blockPath + "/Analytic Topology Optimization","Parameters", ...
    runnerExpression + "," + sampleTime + "," + ...
    historyCapacity);

runner = radia.simulink.resolveStreamFunctionTopologyRunner( ...
