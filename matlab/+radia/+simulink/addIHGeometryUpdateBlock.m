...
%   rebuild regardless of the fingerprint.
%
...
mask.Description = "Watches the workpiece/coil geometry files and " + ...
    "rebuilds the IH operators when their content changes.  The " + ...
    "assemble command must write config_file (MAT/JSON, native IH " + ...
    "configuration); it runs only at diagram update / simulation " + ...
    "start or through the Rebuild now button, never per time step.  " + ...
    "Use absolute file paths.  Leave every field empty to keep the " + ...
    "block inert.";
mask.addParameter(Type="edit", Name="wp_vol", ...
    Prompt="Workpiece mesh (.vol)", Value="", Evaluate="off");
mask.addParameter(Type="edit", Name="coil_file", ...
    Prompt="Coil geometry (.step PEEC / .vol BEM-A)", Value="", ...
    Evaluate="off");
mask.addParameter(Type="edit", Name="assemble_command", ...
    Prompt="Assemble command (writes config_file)", Value="", ...
    Evaluate="off");
