...
arguments
 ...
 options.HysteresisKind (1,1) string {mustBeMember(options.HysteresisKind,"play")}="play";options.Epsilon (1,1) double {mustBePositive}=1e-8
 options.CommandName (1,1) string="command";options.BackEmfName (1,1) string="back_emf";options.CurrentTrace (1,1) string
 options.Turns (1,1) double {mustBePositive};options.CoreArea_m2 (1,1) double {mustBePositive};options.MagneticPath_m (1,1) double {mustBePositive};options.CoreVolume_m3 (1,1) double=NaN
 options.GapPathFactor (1,1) double {mustBeNonnegative}=2;options.SampleTime_s (1,1) double {mustBePositive}
 options.MaxIterations (1,1) double {mustBeInteger,mustBePositive}=12;options.RelativeTolerance (1,1) double {mustBePositive}=1e-4;options.Relaxation (1,1) double {mustBeGreaterThan(options.Relaxation,0),mustBeLessThanOrEqual(options.Relaxation,1)}=0.5
