function ihEddySFunction(block)
%IHEDDYSFUNCTION Map Simulink signals to a persistent native Eddy object.
setup(block);
end
...
block.InputPort(1).Dimensions = 1; % current_A
block.InputPort(2).Dimensions = 1; % angle_rad
block.InputPort(3).Dimensions = config.n_temperature; % temperature_K
block.OutputPort(1).Dimensions = config.n_heat; % heat_density_W_per_m3
...
h = getHandle(block);
if h == 0, error("radia:simulink:IHEddyHandle", "IH Eddy handle is not initialized."); end
end
