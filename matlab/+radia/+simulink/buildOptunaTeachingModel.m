...
set_param(modelName, "Solver", "FixedStepDiscrete", ...
    ...
    "PreLoadFcn", "radia.simulink.makeOptunaMonitorBusObject();");
...
inputNames = ["Best","Status","Progress","Best Trial","Monitor"];
...
for index = 1:5
    ...
    if index <= 4
        ...
        add_block("simulink/Signal Routing/Bus Selector", ...
            dashboard + "/Select Monitor Telemetry", ...
            OutputSignals=strjoin(["attempted_trials", "completed_trials", ...
                "pruned_trials", "failed_trials", "pareto_count", ...
                "last_value", "current_trial"], ","), ...
            Position=[125 360 140 500]);
        add_block("simulink/Signal Routing/Mux", ...
            dashboard + "/Monitor Telemetry", Inputs="7", ...
            Position=[210 360 220 500]);
        add_block("simulink/Sinks/To Workspace", ...
            dashboard + "/Log Monitor Telemetry", ...
            VariableName="teaching_monitor", SaveFormat="Array", ...
            Position=[285 410 425 440]);
        add_line(dashboard, "Monitor/1", ...
            "Select Monitor Telemetry/1");
        for monitorIndex = 1:7
            add_line(dashboard, ...
                "Select Monitor Telemetry/" + monitorIndex, ...
                "Monitor Telemetry/" + monitorIndex);
        end
        add_line(dashboard, "Monitor Telemetry/1", ...
            "Log Monitor Telemetry/1");
    end
end
...
mask.Display = "disp('Study Dashboard');" + ...
    ...
    "port_label('input',5,'monitor');";
set_param(dashboard, UserData=struct( ...
    "role", "student-progress-dashboard", ...
    "full_history", "normalized-mat-study", ...
    "top_level_output_lines", 5, ...
    "monitor_bus", "OptunaMonitorBusV1"), UserDataPersistent="on");
end
