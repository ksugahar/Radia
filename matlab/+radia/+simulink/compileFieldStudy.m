...
if double(boundaries.boundary_count)>options.MaxBoundaries
    error("radia:simulink:FieldStudyBoundaryCapacity", ...
        "Boundary count exceeds MaxBoundaries=%d.",options.MaxBoundaries);
end
...
unknown=setdiff([dirichletNames;robinNames],boundaries.boundary_names,"stable");
if ~isempty(unknown)
...
if physics~="steady_heat" && ~isempty(robinNames)
    error("radia:simulink:FieldStudyRobin", ...
        "Robin boundaries are available only for steady_heat.");
end
...
runtime.physics_code=uint16(find(["electrostatic","current_flow", ...
    "steady_heat","harmonic_eddy"]==physics,1)-1);
...
runtime.boundary_count=boundaries.boundary_count;
for k=1:double(boundaries.boundary_count)
    runtime.boundary_id(k)=boundaries.boundary_ids(k);
    name=boundaries.boundary_names(k);
    if isfield(dirichlet,char(name))
        ...
        runtime.boundary_kind(k)=uint16(1);
        runtime.boundary_value_real(k)=real(value);
        runtime.boundary_value_imag(k)=imag(value);
    elseif isfield(robin,char(name))
        ...
        runtime.boundary_kind(k)=uint16(2);
        runtime.robin_transfer_W_per_m2K(k)=row.transfer_w_per_m2_k;
        runtime.robin_ambient_K(k)=row.ambient_k;
    end
end
...
for k=1:numel(contract.region_names)
    ...
    if physics=="harmonic_eddy"
        if runtime.bh_count(index)>0 || runtime.hysteresis_model_code(index)>0
            error("radia:simulink:FieldStudyLinearHarmonic", ...
                "harmonic_eddy currently requires linear non-hysteretic materials.");
        end
        row=struct("permeability_h_per_m",mu0*runtime.mu_r(index), ...
            "conductivity_s_per_m",runtime.conductivity_S_per_m(index));
    elseif physics=="electrostatic"
    ...
    elseif physics=="current_flow"
        ...
        if conductivity<=0
            error("radia:simulink:FieldStudyThermalMaterial", ...
                "steady_heat requires positive thermal conductivity in region '%s'.",region);
        end
