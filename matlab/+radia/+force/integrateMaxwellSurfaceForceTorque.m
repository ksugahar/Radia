function [force_N, torque_Nm] = integrateMaxwellSurfaceForceTorque( ...
    magneticFluxDensity_T, outwardNormal, areaWeights_m2, samplePoints_m, ...
    pivot_m, permeability_H_per_m)
%INTEGRATEMAXWELLSURFACEFORCETORQUE Integrate static air stress resultants.

if nargin < 5 || isempty(pivot_m)
    pivot_m = [0, 0, 0];
end
if nargin < 6 || isempty(permeability_H_per_m)
...
if ~isreal(weightsValue) || any(~isfinite(weights), "all") || any(weights < 0)
