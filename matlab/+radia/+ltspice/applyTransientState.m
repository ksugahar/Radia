...
endPattern='(?im)^\s*\.end\s*$';
if isempty(regexp(text,endPattern,'once')),error("radia:ltspice:MissingEnd","Cannot inject transient state because the netlist has no .end directive.");end
