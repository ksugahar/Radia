...
endPattern='(?im)^[\t ]*\.end[\t ]*\r?$';
if numel(regexp(text,endPattern))~=1
 error("radia:ltspice:StateNetlistEnd","State injection requires exactly one standalone .end directive.");
end
...
text=regexprep(text,'(?im)^[\t ]*\.tran(?=\s|$)[^\r\n]*',tran,'once'); if ~hadTran,text=regexprep(text,endPattern,tran+newline+".end",'once');end
text=regexprep(text,'(?im)^[\t ]*\.ic(?=\s|$)[^\r\n]*\r?\n?',"");
if isempty(directives),ending=".end";else,ending=join(directives,newline)+newline+".end";end
text=regexprep(text,endPattern,ending,'once');
