...
%NETLISTTOSCHEMATIC Wrap Radia's canonical Python CIR->ASC converter.
arguments
...
parts=[quote(options.PythonExecutable),"-m","radia.ltspice.cli",quote(netlistFile),"-o",quote(outputFile),"--to","asc"];
...
if status~=0||~isfile(outputFile)
 error("radia:ltspice:ConverterFailed","radia.ltspice failed (status %d).\n%s",status,commandOutput);
end
...
if options.ValidateRoundTrip
 ...
 pythonCode="from pathlib import Path; from radia.ltspice import topology_equivalent; import json,sys; ok,details=topology_equivalent(Path(sys.argv[1]).read_text(encoding='utf-8',errors='replace'),Path(sys.argv[2]).read_text(encoding='utf-8',errors='replace')); print(json.dumps(dict(equivalent=ok,details=details)))";
...
result=struct("schema","radia.ltspice.netlist_to_schematic.v1","source_netlist",netlistFile, ...
 "schematic_file",string(outputFile),"converter","radia.ltspice", ...
