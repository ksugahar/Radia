classdef SchematicEditor < handle
    ...
    properties(SetAccess=private), SourcePath (1,1) string; Text (1,1) string; LibraryPaths (1,:) string=strings(1,0); LineEnding (1,1) string=newline; end
    methods
        function obj=SchematicEditor(path)
            ...
            obj.SourcePath=path;raw=fileread(path);obj.Text=string(raw);obj.LineEnding=localLineEnding(raw);
        end
        function setComponentValue(obj,reference,value)
            ...
            target=[];
            matches=names(strcmpi(strtrim(extractAfter(strtrim(lines(names)),"SYMATTR InstName ")),reference));if numel(matches)>1,error("radia:ltspice:AmbiguousComponent","Component reference occurs more than once: %s",reference);end,if ~isempty(matches),target=matches(1);end
            ...
            obj.Text=join(lines,obj.LineEnding);
        end
        ...
        function setComponentAttribute(obj,reference,attribute,value)
            ...
            if isempty(relative),lines=[lines(1:target);needle+string(value);lines(target+1:end)];else,lines(target+relative)=needle+string(value);end,obj.Text=join(lines,obj.LineEnding);
        end
        ...
        function setComponentPosition(obj,reference,position,rotation)
            if nargin<4,rotation="R0";end,[lines,target]=obj.componentBlock(reference);symbol=find(startsWith(strtrim(lines(1:target)),"SYMBOL "),1,'last');parts=split(strtrim(lines(symbol)));parts(3)=string(round(position(1)));parts(4)=string(round(position(2)));parts(5)=string(rotation);lines(symbol)=join(parts," ");obj.Text=join(lines,obj.LineEnding);
        end
        function references=getComponents(obj,prefixes)
            if nargin<2,prefixes="*";end,lines=splitlines(obj.Text);rows=startsWith(strtrim(lines),"SYMATTR InstName ");references=strtrim(extractAfter(strtrim(lines(rows)),"SYMATTR InstName "));if prefixes~="*",designators=upper(extractBetween(references,1,1));references=references(arrayfun(@(x)any(startsWith(upper(string(prefixes)),x)),designators));end
        end
        ...
        function removeComponent(obj,reference),[lines,target,stop]=obj.componentBlock(reference);symbol=find(startsWith(strtrim(lines(1:target)),"SYMBOL "),1,'last');lines(symbol:stop)=[];obj.Text=join(lines,obj.LineEnding);end
        function addComponent(obj,symbol,reference,position,rotation,value)
            ...
            if any(strcmpi(obj.getComponents(),reference)),error("radia:ltspice:DuplicateComponent","Component already exists: %s",reference);end
            addition="SYMBOL "+symbol+" "+round(position(1))+" "+round(position(2))+" "+rotation+obj.LineEnding+"SYMATTR InstName "+reference;if strlength(value)>0,addition=addition+obj.LineEnding+"SYMATTR Value "+value;end,obj.Text=obj.Text+obj.LineEnding+addition;
        end
        ...
        function addDirective(obj,directive,x,y)
            ...
            obj.Text=obj.Text+obj.LineEnding+sprintf("TEXT %d %d Left 2 !%s",round(x),round(y),directive);
        end
        ...
        function value=getParameter(obj,name),p="(?im)^TEXT\s+\d+\s+\d+\s+\S+\s+\d+\s+!\.param\s+"+regexptranslate('escape',char(name))+"\s*=\s*([^\s;]+)";t=regexp(obj.Text,p,'tokens');if isempty(t),error("radia:ltspice:ParameterNotFound","Parameter not found: %s",name);end,if numel(t)>1,error("radia:ltspice:AmbiguousParameter","Parameter is defined more than once: %s",name);end,value=string(t{1}{1});end
        function setParameter(obj,name,value)
            ...
            matches=regexp(obj.Text,p,'match');if isempty(matches),obj.addDirective(".param "+name+"="+text);elseif numel(matches)>1,error("radia:ltspice:AmbiguousParameter","Parameter is defined more than once: %s",name);else,obj.Text=regexprep(obj.Text,p,"$1"+text,'once');end
        end
        ...
        function x=reset_netlist(obj),raw=fileread(obj.SourcePath);obj.Text=string(raw);obj.LineEnding=localLineEnding(raw);x=true;end
    end
    methods (Access=private)
        function [lines,target,stop]=componentBlock(obj,reference)
            lines=splitlines(obj.Text);names=find(startsWith(strtrim(lines),"SYMATTR InstName "));target=[];matches=names(strcmpi(strtrim(extractAfter(strtrim(lines(names)),"SYMATTR InstName ")),reference));if numel(matches)>1,error("radia:ltspice:AmbiguousComponent","Component reference occurs more than once: %s",reference);end,if ~isempty(matches),target=matches(1);end
...
function ending=localLineEnding(raw),if contains(raw,sprintf('\r\n')),ending=string(sprintf('\r\n'));elseif contains(raw,sprintf('\r')),ending=string(sprintf('\r'));else,ending=string(newline);end,end
