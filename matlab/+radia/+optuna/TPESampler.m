classdef TPESampler < radia.optuna.BaseSampler
    ...
    methods
        ...
        function value = sampleFloat(obj, study, trial, name, low, high, options)
            ...
            % A PRUNED trial without a reported intermediate value has no
            % objective, but Optuna still ranks it (last among pruned).
    ...
    methods (Access=private)
        ...
        function [good, bad] = splitObservations(obj, values, objectives, ...
            ...
            % represented by its prior component only.  gamma() sees the
            % study-level COMPLETE+PRUNED count, exactly like upstream's
            % n = sum(state != RUNNING for trial in trials); the clamp only
            % bites when a dynamic search space leaves this parameter with
            % fewer observations than the study has finished trials.
            if nargin < 8
                states = strings(0, 1);
            end
            nGood = obj.goodTrialCount(study.nonRunningTrialCount());
        ...
        function [indices, trialRows, states, pending] = ...
            ...
            % Optuna's TPE reads history from
            % study._get_trials(states=(COMPLETE, PRUNED)), adding RUNNING
            % only under constant_liar. Dropping PRUNED trials keeps the
            % sampler in its random startup phase far longer than upstream
            % on any pruner-enabled study and then fits on a truncated
            % history.
        ...
        function order = rankObservations(obj, objectives, direction, ...
            ...
            % Optuna's _split_trials fills the below set one class at a
            % time: feasible COMPLETE (best objective first), then feasible
            % PRUNED (deepest reported step first, then best intermediate
            % value), then infeasible (smallest total positive violation
            % first). Missing or nonfinite constraint data is deliberately
            % worst (Inf), and RUNNING constant-liar observations can shape
            % only the above density.
            ...
            if ~isempty(prunedRows)
                ...
                % A pruned trial that never reported ranks last, exactly as
                % _get_pruned_trial_score's (1, 0.0) sentinel does.
