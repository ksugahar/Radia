...
%   radia.optuna.study-export.v2 document. EXPORT_STUDY(...,Path=FILE)
...
for index=1:height(trials)
    ...
    [~,constraintValues,constraintNames]=study.constraintRecord(number);
    records{index}=struct( ...
        ...
        "constraints",{constraintRecords( ...
            constraintNames,constraintValues)}, ...
...
function records=constraintRecords(names,values)
records=cell(1,numel(names));
for constraintIndex=1:numel(names)
    records{constraintIndex}=struct( ...
        "name",char(names(constraintIndex)), ...
        "value",double(values(constraintIndex)));
end
end
end
