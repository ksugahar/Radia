classdef MOTPESampler < handle
    %MOTPESAMPLER Optuna-style multi-objective TPE sampler.
    ...
    methods
        function obj=MOTPESampler(options)
            arguments
                ...
                options.MaxGoodTrials (1,1) double {mustBeInteger,mustBePositive}=25
                options.NumberOfEIChoices (1,1) double {mustBeInteger,mustBePositive}=24
                options.PriorWeight (1,1) double {mustBePositive}=1
                options.ConstraintsFcn=[]
            end
            if options.Gamma<=0 || options.Gamma>1, error("radia:optuna:MOTPEGamma","Gamma must be greater than zero and at most one."); end
            if ~isempty(options.ConstraintsFcn) && ~isa(options.ConstraintsFcn,"function_handle")
                error("radia:optuna:ConstraintsFcn","ConstraintsFcn must be a function handle.");
            end
            obj.Stream=RandStream("mt19937ar","Seed",options.Seed); obj.NStartupTrials=options.NStartupTrials;
            obj.Gamma=options.Gamma; obj.MaxGoodTrials=options.MaxGoodTrials;
            obj.NumberOfEIChoices=options.NumberOfEIChoices; obj.PriorWeight=options.PriorWeight;
            obj.ConstraintsFcn=options.ConstraintsFcn;
        end
        function value=sampleFloat(obj,study,trial,name,low,high,options) %#ok<INUSD>
            obj.validate(low,high,options); [x,objectives,trialNumbers]=radia.optuna.internal.ParetoSupport.numericObservations(study,name);
            ...
            [goodMask,goodWeights]=radia.optuna.internal.ParetoSupport.splitMOTPE( ...
                study,trialNumbers,objectives,nGood);
            good=x(goodMask); bad=x(~goodMask);
            ...
            badWeights=ones(numel(bad),1);
            for k=1:numel(candidates), score(k)=obj.logKde(candidates(k),good,bwGood,goodWeights)-obj.logKde(candidates(k),bad,bwBad,badWeights); end
        ...
        function value=sampleCategorical(obj,study,trial,name,choices) %#ok<INUSD>
            ...
            [tokens,objectives,trialNumbers]=radia.optuna.internal.ParetoSupport.categoricalObservations(study,name); count=numel(choices);
            if numel(tokens)<max(2,obj.NStartupTrials), value=obj.choiceAt(choices,1+floor(rand(obj.Stream)*count)); return, end
            nGood=min(obj.MaxGoodTrials,ceil(obj.Gamma*numel(tokens)));
            [goodMask,goodWeights]=radia.optuna.internal.ParetoSupport.splitMOTPE( ...
                study,trialNumbers,objectives,nGood);
            good=tokens(goodMask); bad=tokens(~goodMask); scores=zeros(count,1);
            for k=1:count
                token=obj.token(obj.choiceAt(choices,k)); pg=(sum(goodWeights(good==token))+obj.PriorWeight)/(sum(goodWeights)+obj.PriorWeight*count);
    ...
    methods (Access=private)
        ...
        function value=logKde(~,point,values,width,weights)
            if isempty(values), value=-log(max(width,eps))-0.5*log(2*pi); return, end
            weights=reshape(double(weights),[],1); weights=weights/sum(weights);
            z=(point-values(:))/width;
            terms=-0.5*z.^2+log(max(weights,realmin)); peak=max(terms);
            value=peak+log(sum(exp(terms-peak)))-log(width)-0.5*log(2*pi);
        end
