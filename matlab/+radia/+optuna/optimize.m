...
while ~stopRequested && trial < options.MaxTrials
    ...
    for index = 1:numel(trials)
        ...
        if outcome.failed
            ...
            study.tell(trials(index), State="FAIL");
            fvalThisTrial = NaN;
...
if exitflag == 0 && trial >= options.MaxTrials
    exitflag = 0;
end
if isempty(best.fval)
    exitflag = -2;
...
if options.Pruner ~= "none"
    settings = [settings, {"Pruner", prunerFromName(options.Pruner)}];
end
...
switch name
    ...
    case "patient"
        pruner = radia.optuna.PatientPruner(radia.optuna.MedianPruner(), 1);
    case "successivehalving"
        pruner = radia.optuna.SuccessiveHalvingPruner();
    case "threshold"
        pruner = radia.optuna.ThresholdPruner();
    otherwise
        error("radia:optuna:PrunerName", "Unknown pruner '%s'.", name);
end
end
...
for index = 1:count
    ...
    for k = 1:numel(names)
        entry.(matlab.lang.makeValidName(names(k))) = ...
            suggestFrom(trial, names(k), distributions{k});
    end
    for k = 1:numel(fixed)
...
function value = suggestFrom(trial, name, distribution)
switch distribution.kind
    case "categorical"
        value = trial.suggest_categorical(name, distribution.choices);
    case "integer"
        value = trial.suggest_int(name, distribution.low, distribution.high, ...
            log=distribution.log, step=distribution.step);
    otherwise
        value = trial.suggest_float(name, distribution.low, distribution.high, ...
            log=distribution.log, step=distribution.step);
end
end
...
try
    if ~isempty(options.ConstraintFcn)
        violation = options.ConstraintFcn(value);
        if any(violation > 0)
            score.failed = true;
            score.state = "FAIL";
            return
...
if options.UseParallel
    ...
    count = min(pool.NumWorkers, remaining);
else
    count = 1;
end
count = max(1, count);
