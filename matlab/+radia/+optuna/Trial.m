classdef Trial < radia.optuna.BaseTrial
    ...
    methods (Static, Hidden=true)
        ...
        function [key, registry] = claimKey(registry, name)
            %CLAIMKEY Canonical original-name to MATLAB field-key mapping.
            %   REGISTRY maps every already-claimed key to the original
            %   parameter name it represents. Study.freezeTrial replays this
            %   over the stored parameter rows so a FrozenTrial's
            %   Distributions keys agree with its Params keys even when two
            %   distinct names share one makeValidName result.
        ...
        function keys = claimKeys(names)
            %CLAIMKEYS Batch form of claimKey over ordered parameter names.
            names = reshape(string(names), 1, []);
            keys = strings(1, numel(names));
            registry = struct();
            for index = 1:numel(names)
                [key, registry] = ...
                    radia.optuna.Trial.claimKey(registry, names(index));
    ...
    methods (Access=private)
        ...
        function key = claimParameterName(obj, name)
            [key, obj.ParameterNames] = ...
                radia.optuna.Trial.claimKey(obj.ParameterNames, name);
        end

        function value = singleDistributionValue(~, spec)
            % Optuna's Trial._suggest returns _get_single_value(distribution)
            % without calling the sampler, so a single-valued distribution
            % consumes no random number and no sampler ever sees a
            % zero-width range.
