# axifemm — NMR axisymmetric stored-reference validation
...
The example bundles its own copy of `axifemm_core.py` (the pure-Python
...
```bash
cd examples/axifemm/nmr_validation
python nmr_validate.py
```

Expected runtime: ~30–60 s on a typical workstation.  Outputs:

- `nmr_axifemm_p1a.mat` — axifemm B_z, B_r, A_phi, r·A_phi sampled
- `nmr_axifemm_p1a.png` — three-method overlay plot:
  FEMM (black), NGSolve mixed (red dashed), axifemm Phase 1a (blue)
...
```
    r (mm)       B_z FEMM      B_z mixed      B_z axifemm      ratio
...
`B_z stored ≈ B_z mixed ≈ B_z axifemm` within ~0.3 % across r ∈ [0, 70 mm]
**except at r = 40 mm**, where all three solvers see the magnet edge
discontinuity in M_z and the P1-triangle axifemm result diverges by
...
axifemm assembly; the disk-convergence example exercises the
...
`W:\30_CauerLadderNetwork\2026_04_01_長方形CLN\axifemm`; the
