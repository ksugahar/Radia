...
def demag_singlelayer(h, curve_order, charge_order, intorder=12):
    """Sphere demag factor D_z = <n_z, V n_z>/V_vol via the ngsolve.bem Laplace single-layer.
    curve_order=0 -> flat (faceted) geometry; >0 -> mesh.Curve(curve_order) isoparametric geometry.
    charge_order -> SurfaceL2 polynomial order of the surface charge sigma."""
...
def run(h=0.6):
    D_an = 1.0 / 3.0
    out = {"analytic_demag_z": D_an, "h": h, "table": [], "mesh_conv": []}
    for curve in (0, 3):
        for order in (0, 1, 2):
