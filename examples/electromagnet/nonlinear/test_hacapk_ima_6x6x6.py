#!/usr/bin/env python
"""Quick test: Verify HACApK with IMA on 6x6x6 C-type mesh."""
import sys, os, time
import numpy as np
...
print(f"6x6x6: {len(hex_elements)} elements, {len(hex_elements)*6} DOF")
print(f"Expected: Bz ~ -963.8 mT (LU reference)")
...
if abs(B[2]*1000 - (-963.8)) < 1.0:
    print("\nPASS: Bz matches LU reference within 1 mT")
else:
    print(f"\nFAIL: Bz = {B[2]*1000:.2f} mT, expected ~ -963.8 mT")
