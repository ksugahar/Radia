"""dd_production_cuboid521.py — Production DD Cauer extraction for cuboid 5×2×1.

Runs the full DD pipeline at production quadrature (12, 24, 12, 6) and
extracts Cauer rungs with verified-interval to stage 12+ for cuboid order=3.

Output: physical Cauer rungs R_{2k}, L_{2k+1} in canonical normalization,
...
def main():
    print("=" * 72)
    print("Production DD Cauer extraction: cuboid 5×2×1, order=3")
    print("=" * 72)
    ...
    # DD K assembly (GPU)
    print(f"\n--- DD K assembly (GPU, production scale) ---")
    t0 = time.time()
    K_bare_hi, K_bare_lo = assemble_K_dd_gpu(p, A_M, B_M, C_M,
                                               n_th, n_ph, n_rh, n_c,
                                               use_gpu=True, verbose=True)
    t_K = time.time() - t0
    print(f"  Total: {t_K:.1f}s")

    # Apply mu0/(4 pi) prefactor
    pref = MU0 / (4 * math.pi)
    K_hi = K_bare_hi * pref
    K_lo = K_bare_lo * pref
    ...
    for k in range(len(rungs_raw) // 2):
        ...
        print(f"  {k:>3} {R_raw['mid']:>15.4e} {L_mid:>15.4e} "
              f"{tau_p:>13.4f} {R_raw['rel_width']:>10.2e} "
              f"{Linv_raw['rel_width']:>10.2e} {str(reliable):>10}")
        cauer_rungs.append({
    ...
    out_path = (Path(__file__).parent
                / "dd_production_cuboid521_order3_results.json")
    out_path.write_text(json.dumps({
        ...
        "K_assembly_s": t_K,
        "M_b_assembly_s": t_Mb,
        "DD_floor_K_lo_hi": float(
            np.max(np.abs(K_lo) / (np.abs(K_hi) + 1e-30))),
        "top_foster": [
