"""sphere_vim_axisym_linear_gpu_v3.py — GPU linear VIM v3.

v3 fixes:
  - AGM ellipe bug: `scale *= 2.0` placed AFTER `sum_c2 += scale * c2_new`
    (v2 had `scale *= 2.0` BEFORE, causing 2x sum and downstream
    factor-2 eigenvalue / tau_1 error).
  - Explicit del + cp.get_default_memory_pool().free_all_blocks()
    between iterations to release GPU memory.
"""
...
def assemble_K_M_b(N_rho, N_theta, n_quad, verbose=True):
    ...
    if verbose:
        print(f"  N={N_rho}x{N_theta} cells={N_rho*N_theta} DoF={n_interior}", flush=True)
    ...
    if verbose:
        print(f"  B_dense ({B_dense_cpu.shape}, "
              f"{B_dense_cpu.nbytes/1e9:.2f} GB CPU): {time.time()-t0:.2f}s", flush=True)
...
def extract(N_rho, N_theta, n_quad, top_n=10, n_stages=12):
    print(f"\n--- {N_rho}x{N_theta} = {N_rho*N_theta} cells, n_quad={n_quad} ---", flush=True)
    t0 = time.time()
    ...
    print(f"  leading tau_1 = {tau_pos[order[0]]*1e6:.4f} us", flush=True)
    if rungs:
        print(f"  tau_pair[0] = {rungs[0]['tau_pair_us']:.4f} us", flush=True)
...
def main():
    ...
    print("=" * 78, flush=True)
    print("Sphere VIM linear GPU v3 (ellipe AGM fixed, mem cleanup)", flush=True)
    ...
    for N_rho, N_theta, n_quad in levels:
        try:
            ...
            results.append(r)
            (here / f"sphere_vim_axisym_linear_gpu_v3_{N_rho}x{N_theta}.json").write_text(
    ...
    print(flush=True)
    print("=" * 78, flush=True)
    print(f"Leading Foster tau_1 vs Stoll = {stoll_tau[0]:.6f} us", flush=True)
    print("=" * 78, flush=True)
    for r in results:
        ...
        print(f"  {r['n_cells']:>7d} cells, {r['n_dofs']:>6d} DoF | "
              ...
              f"tau_1={r['leading_tau_us']:>10.6f} gap={g:>+8.4f}%", flush=True)

    print(flush=True)
    print("Cauer tau_pair[k] gap to Stoll [%]:", flush=True)
    ...
    out = {"stoll_tau_us": stoll_tau[:20], "stoll_cauer_tau_us": stoll_cauer,
           "results": results}
    (here / "sphere_vim_axisym_linear_gpu_v3_summary.json").write_text(
        json.dumps(out, indent=2), encoding="utf-8")
    print("\nSaved sphere_vim_axisym_linear_gpu_v3_summary.json", flush=True)
