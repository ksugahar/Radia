"""axifem_disk_kameari_v2.py — axifem cylinder Cauer-I in Kameari convention.

Direct Foster -> Kameari extraction from axifem K, M, b using Schur
...
sys.path.insert(0,
    str(Path("S:/Radia/01_GitHub/examples/axifem/research/tests")))
...
def schur_foster_kameari(K, M, b, cond_idx, air_idx, label=""):
    ...
    # tau_k = 1 / lambda_k  (axifem convention)
...
def solve_disk_kameari(NR_disk, Nz_disk, NR_air, Nz_air, R_air, Z_air,
                       label=""):
    print(f"\n=== axifem Kameari Cu disk: NR_d={NR_disk} Nz_d={Nz_disk} "
...
def main():
    ...
    print("\n" + "="*78)
    print("Summary: axifem Q1 Kameari Cauer-I (cylinder Cu disk)")
    print("="*78)
    print(f"  BEM Kameari leading tau (reference) = 209 us")
    print(f"  axifem Hiruma leading tau (reference) = 218.71 us (Q2 fine)")
    for lbl, r in all_res.items():
        rungs = r["Cauer_rungs_Kameari"]
        if rungs:
            tau0 = rungs[0]["tau_pair_us"]
            print(f"  {lbl:<10} axifem Q1 Kameari tau_pair[0] = {tau0:.4f} us")

    out_path = Path(__file__).parent / "axifem_disk_kameari_v2_results.json"
