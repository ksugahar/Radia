...
Tests the AxifemmQuadQ2Curved (curved 9-node Q2) against:
  1. axifemm_quad_q2.py axis-aligned Gauss reference (must agree to machine
...
def test_axis_aligned_reduction():
    """With quad-mean r mid-nodes, AxifemmQuadQ2Curved must reproduce the
    axifemm_quad_q2 element matrices to machine precision (both use the same
    (s, z)-monomial basis and the same physical integration domain).

    To make the test independent of axifemm_quad_q2.py's fixed 8-pt Gauss,
    ...
    # Patch axifemm_quad_q2's GL order to n=20 for the comparison.
    import axifemm_quad_q2 as q2mod
    ...
    print("[OK] axis-aligned reduction matches axifemm_quad_q2 to <1e-10 rel "
          "(both at n_quad=20)")
...
def test_axis_aligned_reduction_axis_touching():
    ...
    # axifemm_quad_q2 raises NotImplementedError for axis-touching — skip K/M
...
def test_curved_element_basic():
    ...
    rn[5] = rb + 0.05 * (rb - ra)
    elem = AxifemmQuadQ2Curved(rn, zn)
...
def main():
    print("=" * 70)
    print("axifemm Q2 curved element — Phase B5 element-level tests")
    print("=" * 70)
    print()
