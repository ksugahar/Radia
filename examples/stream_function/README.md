...
engineering standoff.  The remaining named next step is winding-SHAPE
optimisation (conformal Fourier modes at fixed standoff -- FOCUS's main
contribution) plus a structural hoop-stress model (the advanced demo reports the
magnetic force per area, the stress DRIVER, not the conductor stress itself).
