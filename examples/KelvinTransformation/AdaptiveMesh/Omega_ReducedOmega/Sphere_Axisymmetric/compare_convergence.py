"""
軸対称球問題の収束履歴比較グラフ
3つの手法 × 3つのorder (p=2, 3, 4) を1x3のサブプロットで比較
"""
import os
import numpy as np
...
methods = {
    'Refine_all_elements': ('Uniform (All Elements)', 'g^-', 1.5, 5),
    'Refine_with_zz_estimator': ('Adaptive (ZZ Estimator)', 'bo-', 2, 6),
    'metric_based': ('Metric-based Remesh', 'rs-', 1.5, 5),
}
...
for order in orders:
    ...
    for method_dir, (label, marker, lw, ms) in methods.items():
        if method_dir == 'metric_based':
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "Axisymmetric_sphere_metric_with_Kelvin.mat")
        else:
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "Axisymmetric_sphere_adaptive_with_Kelvin.mat")
...
for idx, order in enumerate(orders):
    ...
    # Reference line: O(N^{-p/2}) for current order only
    # Use metric-based final point as reference
    ndof_line = np.array([1e2, 1e6])

    # Draw theoretical slope for current order only
    ref_styles = {2: ('k--', '$O(N^{-2/2})$'),
                  3: ('k--', '$O(N^{-3/2})$'),
                  4: ('k--', '$O(N^{-4/2})$')}
    if 'metric_based' in data[order] and len(data[order]['metric_based'][0]) > 0:
        # Use final point of metric-based for this order as reference
        N_ref = data[order]['metric_based'][0][-1]  # final DOF
        err_ref = data[order]['metric_based'][1][-1]  # final error
        err_line = err_ref * (N_ref / ndof_line) ** (order / 2)
...
plt.suptitle('Axisymmetric Sphere: Convergence Comparison (Omega-formulation + Kelvin)',
