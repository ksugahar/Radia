...
Note: 立方体問題には解析解が存在しないため、誤差推定値の収束を比較
"""
import os
import numpy as np
    ...
    for method_dir, (label, marker, lw, ms) in methods.items():
        if method_dir == 'maxh':
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "Cube_3D_maxh_with_Kelvin.mat")
        else:
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "Cube_3D_adaptive_with_Kelvin.mat")
...
for idx, order in enumerate(orders):
    ...
    if 'maxh' in data[order] and len(data[order]['maxh'][0]) > 0:
        # Use final point of maxh for this order as reference
    ...
    ax.set_xlim(1e2, 1e5)
    ax.set_ylim(1e-2, 1e-0)  # Fixed scale for all orders
    ax.grid(True, alpha=0.3)
    ax.legend(loc='lower left', fontsize=9)
    ax.tick_params(which='both', direction='in', top=True, right=True)

plt.suptitle('3D Cube (1/8 Model): Convergence Comparison (Omega-formulation + Kelvin)',
