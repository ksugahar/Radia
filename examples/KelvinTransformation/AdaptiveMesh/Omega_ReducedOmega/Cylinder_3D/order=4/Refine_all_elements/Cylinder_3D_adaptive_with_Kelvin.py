...
print("UNIFORM REFINEMENT (All Elements) - 1/8 Model (Octant) - CYLINDER")
print("=" * 60)

# ============================================================
# Parameters
# ============================================================
cylinder_radius = 0.25   # Magnetic cylinder radius [m]
cylinder_height = 0.5    # Magnetic cylinder height [m] (total, using half: z>=0)
air_total_radius = 0.5   # Total region air sphere radius [m] (surrounds cylinder)
...
print(f"  Cylinder half-height: {half_height} m")
...
# ============================================================
# Geometry Definition for 3D - 1/8 Model (Cylinder)
# 4-region structure:
#   magnetic: cylinder (Total potential)
#   air_total: sphere around cylinder (Total potential)
...
def create_geometry():
	...
	print("\nCreating 1/8 cylinder geometry with 4-region structure...")
	print(f"  magnetic: cylinder (r={cylinder_radius}m, h={half_height}m)")
	print(f"  air_total: sphere (r={air_total_radius}m) around cylinder")
	...
	# ===== Region 1: Magnetic cylinder (Total potential) =====
	...
	# Name faces for magnetic cylinder
	...
	# Sphere of radius air_total_radius, minus the magnetic cylinder
	...
	air_total = air_total_sphere - mag_cylinder
	...
	# Glue all domains
	geo = Glue([mag_cylinder, air_total, air_inner, outer_sphere, vertex])
...
def output_vtk(mesh, iteration, output_data):
	...
	temp_basename = f"Cylinder_3D_adaptive_with_Kelvin_iter_{iteration:02d}"
...
def save_iteration_mat(iter_num, history, n_elements, n_vertices):
	...
	mat_iter_file = os.path.join(script_dir, f"cylinder_3d_iter_{iter_num:02d}.mat")
...
def generate_convergence_plot(iter_num, history, output_data):
	...
	# Cylinder boundary in x-z plane (y=0 slice)
	z_cyl = linspace(0, half_height, 20)
	x_cyl_left = [0] * len(z_cyl)
	x_cyl_right = [cylinder_radius] * len(z_cyl)
	...
	# Draw cylinder and air_total boundaries
	ax1.plot([cylinder_radius, cylinder_radius], [0, half_height], 'r-', linewidth=2, label='Cylinder')
	ax1.plot([0, cylinder_radius], [half_height, half_height], 'r-', linewidth=2)
	...
	plt.suptitle(f'Cylinder Iteration {iter_num}: DOFs={history["ndof"][-1]}, Error={history["error"][-1]:.2e}',
	             fontsize=14, fontweight='bold', y=1.01)
	plt.tight_layout()

	png_file = os.path.join(script_dir, f"cylinder_3d_iter_{iter_num:02d}.png")
...
# ============================================================
print("\n" + "=" * 60)
print("Creating 1/8 3D cylinder geometry...")
print("=" * 60)
...
while True:
	...
	print(f"  Marking ALL {mesh.ne} elements for uniform refinement")
