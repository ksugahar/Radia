    ...
    for method_dir, (label, marker, lw, ms) in methods.items():
        if method_dir == 'metric_based':
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "ParallelWires_2D_metric_remesh_with_Kelvin.mat")
        else:
            mat_file = os.path.join(script_dir, f"order={order}", method_dir,
                                    "ParallelWires_2D_adaptive_with_Kelvin.mat")
...
plt.suptitle('Parallel Wires: Convergence Comparison (A-formulation + Kelvin)',
