...
平行2線問題（2次元）のA法 + Kelvin変換 WITH UNIFORM MESH REFINEMENT

Uniform refinement algorithm:
  1. Solve on current mesh
  2. Compute ZZ-type error estimator (for monitoring only, not for marking)
  3. Refine ALL elements uniformly
  4. PNG and MAT output at each iteration
...
print("Parallel Wires - UNIFORM MESH REFINEMENT (All Elements)")
print("=" * 60)
...
max_iterations = 6      # Stop after this many iterations
...
def generate_iteration_plot(iter_idx, n_elements, history, inner_error_map, outer_error_map, gfu, mesh):
    ...
    ax3.loglog(history['ndof'][:iter_idx+1], history['error'][:iter_idx+1], 'ko-', linewidth=2, markersize=6, label='Uniform')
    ...
    png_file = os.path.join(script_dir, f"ParallelWires_2D_uniform_iter_{iter_idx+1:02d}.png")
...
def save_iteration_mat(iter_idx, history):
    ...
    mat_filename = os.path.join(script_dir, f"ParallelWires_2D_uniform_iter_{iter_idx+1:02d}.mat")
    mat_data = {
...
print("\n" + "=" * 60)
print("Starting Uniform Mesh Refinement")
print("=" * 60)
...
for iteration in range(max_iterations):
    ...
    # Compute error estimator (for monitoring only)
    ...
    # Refine ALL elements uniformly (skip on last iteration)
    if iteration < max_iterations - 1:
        ...
        mesh.Refine()
        print(f"  Refined all elements uniformly")
...
final_png = os.path.join(script_dir, f"ParallelWires_2D_uniform_iter_{max_iterations:02d}.png")
