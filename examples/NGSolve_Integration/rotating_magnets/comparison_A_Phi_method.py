...
for step in range(time_steps + 1):
    ...
    # ------------------------------------------------------------------
    # 3.5: VTK出力（磁場 + 渦電流 + 材料ID）- コメントアウト
    # ------------------------------------------------------------------
    # vtk_solution = output_dir / f"solution_step_{step:04d}"
    #
    # # gfA_extをGridFunctionとして作成（VTK出力用、curl演算用）
    # gfA_ext_vtk = GridFunction(fesVec_curl)
    # gfA_ext_vtk.Set(Av_new)
    #
    # # 材料IDを定義（デバッグ用）
    # material_id = CF([1 if mat == "copper" else 0 for mat in mesh.GetMaterials()])
    #
    # # 磁石領域マーカー（デバッグ用）
    # magnet_half_size = magnet_size / 2
    # in_magnet = IfPos(
    #     (x - x_pos)**2 + (y - y_fixed)**2 + (z - z_fixed)**2 - (magnet_half_size * 1.5)**2,
    #     0, 1
    # )
    #
    # # ローレンツ力密度の大きさ
    # try:
    #     f_lorentz_magnitude = sqrt(f_lorentz_density * f_lorentz_density)
    # except:
    #     f_lorentz_magnitude = CF(0)
    #
    # # VTK出力
    # try:
    ...
    #         ],
    #         legacy=False,
    #         filename=f"output_magnet1_A_method/solution_step_{step:04d}",
    #         subdivision=2
    #     )
    #     vtk.Do()
    #     print(vtk)
    #     print(vtk_solution.with_suffix('').as_posix())
    #     print(f"  VTK出力: {vtk_solution}.vtu")
    # except Exception as e:
    #     print(f"  VTK出力エラー: {e}")
    #     import traceback
    #     traceback.print_exc()
