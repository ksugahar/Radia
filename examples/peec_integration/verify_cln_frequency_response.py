"""
Verify CLN I frequency response matches original PEEC Loop impedance

This script verifies that the CLN I transformation preserves the
...
Test 1: Without surface impedance Z_s
    ...
    CLN I:    Z'(s) = R_diag + s*L_tridiag  (sparse matrices)

Test 2: With surface impedance Z_s (frequency-dependent)
    Original: Z(s) = R + s*L + Z_s(s)*I  (dense matrices)
    CLN I:    Z'(s) = R_diag + s*L_tridiag + Z_s(s)*I  (sparse matrices)
...
def test_cln_frequency_response():
    """Test that CLN I transformation preserves frequency response."""
    print("=" * 70)
    print("Verify CLN I Frequency Response Matches Original PEEC")
    print("=" * 70)
    ...
    # Apply CLN I transformation
    # lanczos(K=R, N=L) -> R_diag (diagonal), L_tridiag (tridiagonal)
    print("\n--- Applying CLN I Transformation ---")
    print("lanczos(K=R, N=L) -> R_diag, L_tridiag")

    result = lanczos(R_diag_orig, L_dense)

    R_cln = result.R_diag
    L_cln = result.L_tridiag
    U = result.U
    V = result.V

    print(f"\nCLN I transformed matrices:")
    print(f"  R_diag:    {R_cln.shape[0]}x{R_cln.shape[1]} diagonal")
    print(f"  L_tridiag: {L_cln.shape[0]}x{L_cln.shape[1]} tridiagonal")
    print(f"  n_output = {result.n_output}")

    # Verify structure (use relative tolerance)
    R_norm = np.linalg.norm(R_cln, 'fro')
    L_norm = np.linalg.norm(L_cln, 'fro')
    is_diag_R = np.allclose(R_cln, np.diag(np.diag(R_cln)), atol=1e-10 * R_norm)
    is_tridiag_L = True
    for i in range(L_cln.shape[0]):
        for j in range(L_cln.shape[1]):
            if abs(i - j) > 1 and abs(L_cln[i, j]) > 1e-10 * L_norm:
                is_tridiag_L = False
                break

    print(f"\n  R_cln is diagonal: {is_diag_R}")
    print(f"  L_cln is tridiagonal: {is_tridiag_L}")
    ...
    # Transform port vector for CLN: port_cln = U^T * port_orig
    port_cln = U.T @ port_orig
    ...
    # Compute impedance for CLN I
    Z_cln = compute_impedance(R_cln, L_cln, frequencies, port_cln)

    # Compare
    rel_error = np.abs(Z_cln - Z_orig) / np.abs(Z_orig)
    ...
    if max_rel_error < 1e-10:
        print("\n=> PASS: CLN I frequency response matches original PEEC!")
    else:
    ...
    ax1.loglog(frequencies, np.abs(Z_cln), 'r--', label='CLN I', linewidth=2)
    ...
    ax2.semilogx(frequencies, np.angle(Z_cln, deg=True), 'r--', label='CLN I', linewidth=2)
    ...
    ax3.semilogx(frequencies, np.real(Z_cln), 'r--', label='CLN I', linewidth=2)
    ...
    ax4.set_title('Relative Error |Z_cln - Z_orig| / |Z_orig|')
    ax4.legend()
    ax4.grid(True, which='both', alpha=0.3)

    plt.tight_layout()
    plt.savefig('cln_frequency_response_verification.png', dpi=150)
    print(f"\nPlot saved: cln_frequency_response_verification.png")
    plt.close()

    return max_rel_error < 1e-10


def test_cln_with_surface_impedance():
    """Test that CLN I transformation preserves frequency response with Z_s."""
    print("\n" + "=" * 70)
    print("Test 2: Verify CLN I Frequency Response with Surface Impedance Z_s")
    print("=" * 70)
    ...
    # For CLN I with Z_s, we need to transform the identity matrix:
    # In original: Z_matrix = R + sL + Z_s*I
    # In CLN I:    Z_matrix = R' + sL' + Z_s*I'
    ...
    # Compute impedance for CLN I with Z_s (using transformed identity)
    Z_cln = compute_impedance_with_transformed_Zs(R_cln, L_cln, frequencies, port_cln, Z_s, I_transformed)
    ...
    if max_rel_error < 1e-10:
        print("\n=> PASS: CLN I with Z_s matches original PEEC + Z_s!")
    else:
        print(f"\n=> WARNING: Relative error {max_rel_error:.2e} > 1e-10")

    # Plot results
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle('CLN I Frequency Response with Surface Impedance Z_s', fontsize=14)

    # Magnitude
    ax1 = axes[0, 0]
    ax1.loglog(frequencies, np.abs(Z_orig), 'b-', label='Original PEEC + Z_s', linewidth=2)
    ax1.loglog(frequencies, np.abs(Z_cln), 'r--', label='CLN I + Z_s', linewidth=2)
    ax1.set_xlabel('Frequency [Hz]')
    ax1.set_ylabel('|Z| [Ohm]')
    ax1.set_title('Impedance Magnitude')
    ax1.legend()
    ...
    ax2.semilogx(frequencies, np.angle(Z_cln, deg=True), 'r--', label='CLN I + Z_s', linewidth=2)
    ax2.set_xlabel('Frequency [Hz]')
    ax2.set_ylabel('Phase [deg]')
    ax2.set_title('Impedance Phase')
    ax2.legend()
    ...
    ax3.loglog(frequencies, np.real(Z_cln), 'r--', label='CLN I + Z_s', linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('cln_frequency_response_with_zs.png', dpi=150)
    print(f"\nPlot saved: cln_frequency_response_with_zs.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    success1 = test_cln_frequency_response()

    # Test 2: With surface impedance Z_s
    success2 = test_cln_with_surface_impedance()
