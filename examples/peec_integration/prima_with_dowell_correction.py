...
def calc_cln_dc_with_dowell_correction(R_dc, L_int_dc, d, sigma, mu, freqs):
    """
    CLN (DCパラメータ) + Dowell補正
...
def main():
    print("="*70)
    print("CLN (DC) + Dowell補正 vs Dowell式 一致検証")
    print("="*70)
    ...
    # CLN (DC) + Dowell補正
    Z_cln_dowell = calc_cln_dc_with_dowell_correction(R_dc, L_int_dc, d, sigma, mu, freqs)

    # 比較
    print(f"\n{'Freq':>12} {'|Z_Dowell|':>14} {'|Z_CLN+Dow|':>14} {'Err':>10}")
    print("-"*55)
    ...
    for f in test_freqs:
        ...
        z_dow = Z_dowell[idx]
        z_cln = Z_cln_dowell[idx]
        err = np.abs(z_cln - z_dow) / np.abs(z_dow) * 100
        freq_str = f"{f:.0f} Hz" if f < 1000 else f"{f/1e3:.0f} kHz" if f < 1e6 else f"{f/1e6:.0f} MHz"
        print(f"  {freq_str:>10} {np.abs(z_dow):>14.6e} {np.abs(z_cln):>14.6e} {err:>9.6f}%")

    max_err = np.max(np.abs(Z_cln_dowell - Z_dowell) / np.abs(Z_dowell)) * 100
    print(f"\n最大誤差: {max_err:.10f}%")

    if max_err < 1e-10:
        print("\n=> 完全一致! CLN(DC) + F_R/F_L補正 = Dowell式")
    else:
        print(f"\n=> 誤差あり")

    # 実部・虚部の詳細
    print(f"\n実部・虚部の詳細:")
    print(f"{'Freq':>12} {'Re(Dow)':>14} {'Re(CLN)':>14} {'Im(Dow)':>14} {'Im(CLN)':>14}")
    print("-"*70)
    ...
    for f in test_freqs:
        ...
        z_cln = Z_cln_dowell[idx]
        freq_str = f"{f:.0f} Hz" if f < 1000 else f"{f/1e3:.0f} kHz" if f < 1e6 else f"{f/1e6:.0f} MHz"
        print(f"  {freq_str:>10} {z_dow.real:>14.6e} {z_cln.real:>14.6e} {z_dow.imag:>14.6e} {z_cln.imag:>14.6e}")
    ...
    ax1.loglog(freqs, np.abs(Z_cln_dowell), 'b--', linewidth=2, label='CLN(DC) + F_R/F_L')
    ax1.set_xlabel('Frequency [Hz]')
    ax1.set_ylabel('|Z| [Ohm*m^2]')
    ax1.set_title('Impedance: Dowell vs CLN(DC) + Dowell Correction')
    ax1.legend()
    ax1.grid(True, which='both', alpha=0.3)

    ax2 = axes[1]
    err = np.abs(Z_cln_dowell - Z_dowell) / np.abs(Z_dowell) * 100
    ...
    plt.tight_layout()
    plt.savefig('cln_with_dowell_correction.png', dpi=150)
    print(f"\nSaved: cln_with_dowell_correction.png")
    plt.close()

    print("\n" + "="*70)
    print("結論:")
    print("  CLN(DC) + F_R(s)/F_L(s)補正 = Dowell式 (完全一致)")
    print("  ")
    print("  単一導体の場合:")
