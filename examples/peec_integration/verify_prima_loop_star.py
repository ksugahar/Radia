"""
CLN変換後のLoop-Star相互作用の周波数特性検証

Loop部分にLanczos変換（CLN I型）を適用した場合:
...
検証項目:
1. 元のLoop-Star系とCLN変換後の周波数特性が一致すること
...
def build_cln_loop_star_impedance(R_diag, L_tridiag, P, M_LS_reduced, omega):
    """CLN変換後のLoop-Star系インピーダンス行列を構築

    Z = [Z_LL'   Z_LS']
        [Z_SL'   Z_SS ]

    where:
        Z_LL' = R_diag + jw*L_tridiag  (CLN変換後)
...
def verify_loop_star_cln(n_segments=20, n_reduced=5):
    """Loop-Star系のCLN変換検証
    ...
    """
    print("=" * 70)
    print("CLN Loop-Star Transformation Verification")
    print("=" * 70)
    ...
    print("\n" + "-" * 70)
    print(f"{'Freq [Hz]':>12}  {'Z_orig':>24}  {'Z_cln':>24}  {'Error':>10}")
    print("-" * 70)
    ...
    for f in frequencies:
        ...
        # CLN変換後のLoop-Star系
        Z_matrix_cln = build_cln_loop_star_impedance(
            R_diag, L_tridiag, P, M_LS_reduced, omega
        )
        Z_cln = compute_port_impedance_from_matrix(Z_matrix_cln, port_vector_reduced)

        # 相対誤差
        rel_error = np.abs(Z_cln - Z_orig) / np.abs(Z_orig)

        Z_orig_list.append(Z_orig)
        Z_cln_list.append(Z_cln)
        errors.append(rel_error)

        # 一部の周波数のみ表示
        if f in [1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8]:
            print(f"{f:12.2e}  {Z_orig:24.6e}  {Z_cln:24.6e}  {rel_error:10.2e}")
    ...
    ax1.loglog(frequencies, np.abs(Z_cln_list), 'r--', label='CLN-reduced', linewidth=2)
    ...
    ax2.semilogx(frequencies, np.angle(Z_cln_list, deg=True), 'r--', label='CLN-reduced', linewidth=2)
    ...
    ax3.set_title('Relative Error |Z_cln - Z_orig| / |Z_orig|')
    ax3.legend()
    ...
    ax4.loglog(frequencies, np.abs(np.real(Z_cln_list)), 'r-', label='Re(Z_cln)', linewidth=1, alpha=0.7)
    ax4.loglog(frequencies, np.abs(np.imag(Z_cln_list)), 'r--', label='Im(Z_cln)', linewidth=1, alpha=0.7)
    ...
    plt.suptitle(f'CLN Loop-Star Verification\n'
                 f'n_loop: {matrices.n_loop} -> {n_reduced}, n_star: {matrices.n_star}',
                 fontsize=14)
    plt.tight_layout()
    plt.savefig('verify_cln_loop_star.png', dpi=150)
    print(f"\nPlot saved to: verify_cln_loop_star.png")
    plt.close()
...
def verify_loop_only_cln(n_segments=20, n_reduced=20):
    """Loop-only（Star成分なし）でのCLN変換検証

    Star成分を除外することで、純粋にLoop変換の正しさを検証
    """
    print("\n" + "=" * 70)
    print("Loop-Only CLN Verification (No Star)")
    print("=" * 70)
    ...
    print(f"\n{'Freq [Hz]':>12}  {'Z_orig':>24}  {'Z_cln':>24}  {'Error':>10}")
    print("-" * 70)
    ...
    for f in frequencies:
        ...
        # CLN変換後: Z' = R_diag + jw*L_tridiag
        Z_LL_cln = R_diag + 1j * omega * L_tridiag
        I_cln = np.linalg.solve(Z_LL_cln, port_vector_reduced)
        Z_cln = np.dot(port_vector_reduced, I_cln)
    ...
    if max_error < 1e-10:
        print("*** PASS: Loop-only CLN transformation is exact ***")
...
def main():
    ...
    success0, error0 = verify_loop_only_cln(n_segments=20, n_reduced=20)

    # テスト1: 直線ワイヤ（フル次元で変換の正しさを検証）
    # n_reduced = n_segments なら、変換は情報を失わない
    success1, error1 = verify_loop_star_cln(n_segments=20, n_reduced=20)
