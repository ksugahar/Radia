#!/usr/bin/env python
"""
Test quarter model with LINEAR material (mu_r=1000).
Compare IMA vs no IMA to isolate the issue.
"""
...
def test_with_ima(nodes, elements, use_ima=True):
    ...
    return {
        'max_M': result[0],
        'iterations': int(result[2]) if result[2] > 0 else int(result[3]),
        'time': t_solve,
        'Bz': B[2],
    }
...
print("=" * 70)
print("Quarter Model with LINEAR Material (mu_r=1000)")
print("=" * 70)
...
# Test WITHOUT IMA
print("-" * 70)
print("Test 1: Quarter model WITHOUT IMA (quarter geometry, full solution)")
print("-" * 70)
result_no_ima = test_with_ima(nodes, elements, use_ima=False)
...
print("-" * 70)
result_ima = test_with_ima(nodes, elements, use_ima=True)
...
# Compare
print("=" * 70)
print("COMPARISON (Linear Material)")
print("=" * 70)
print(f"{'Test':<25} {'Max |M| (A/m)':<15} {'Iter':<8} {'Bz (mT)':<12}")
print("-" * 60)
print(f"{'No IMA':<25} {result_no_ima['max_M']:<15.2f} {result_no_ima['iterations']:<8} {result_no_ima['Bz']*1000:<12.2f}")
print(f"{'IMA +x-z':<25} {result_ima['max_M']:<15.2f} {result_ima['iterations']:<8} {result_ima['Bz']*1000:<12.2f}")

if result_no_ima['iterations'] > 0 and result_ima['iterations'] == 0:
    print()
    print("WARNING: IMA model has 0 iterations while no-IMA model has iterations!")
    print("This indicates a bug in the IMA interaction matrix.")
