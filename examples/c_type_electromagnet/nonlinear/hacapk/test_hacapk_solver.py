#!/usr/bin/env python
"""
Test HACApK solver (Method 2) for nonlinear electromagnet.

HACApK solver: H-matrix accelerated BiCGSTAB with ACA+ compression.
- Best for large problems (N > 2000 elements)
- Memory efficient: O(N log N)
- Uses low-rank approximation for off-diagonal blocks
"""
...
def main():
    print("=" * 70)
    print("Nonlinear Electromagnet - HACApK Solver (Method 2)")
    print("=" * 70)
    ...
    print("\nSetting HACApK parameters...")
    eps = 1e-4  # ACA tolerance
    leaf_size = 10
    eta = 2.0
    rad.SetHACApKParams(eps, leaf_size, eta)
    print(f"  eps (ACA tolerance): {eps}")
    print(f"  leaf_size: {leaf_size}")
    print(f"  eta (admissibility): {eta}")

    # Solve with HACApK solver
    print("\n" + "-" * 50)
    print("Solving with HACApK solver (Method 2)...")
    print("-" * 50)
    ...
    t_start = time.time()
    try:
        result = rad.Solve(model, 0.001, 500, 2, image='+x-z')
    ...
    print("\n" + "=" * 70)
    print("HACApK Solver Test Complete")
    print("=" * 70)
