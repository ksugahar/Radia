#!/usr/bin/env python
"""Quick test: Verify HACApK with IMA on 10x10x10 C-type mesh."""
import sys, os, time
import numpy as np
...
print(f"10x10x10: {len(hex_elements)} elements, {len(hex_elements)*6} DOF")
print(f"Expected: Bz ~ -961.5 mT (LU reference)")
...
t0 = time.time()
result = rad.Solve(model, 0.001, 100, 2, image='+x-z')
t1 = time.time()
...
if stats:
    ...
    print(f"  Leaves: {stats.get('n_leaves', 'N/A')}")
    print(f"  H-matrix memory: {stats.get('memory_mb', 'N/A'):.1f} MB")
    print(f"  Compression: {stats.get('compression', 'N/A')}")

if abs(B[2]*1000 - (-961.5)) < 1.0:
    print("\nPASS: Bz matches LU reference within 1 mT")
else:
    print(f"\nFAIL: Bz = {B[2]*1000:.2f} mT, expected ~ -961.5 mT")
