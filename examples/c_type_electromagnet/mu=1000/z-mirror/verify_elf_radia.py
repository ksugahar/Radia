#!/usr/bin/env python
"""
Verify ELF vs Radia for the z-mirror model (26 hexahedral elements + image).

Uses Image symmetry: MIMA -Z (antisymmetric mirror on z=0 plane)
...
ELF_Z_MIRROR = r"S:\ELF_MAGIC\2020_03_07_CEFC_2020\model_C-Type\mu=1000\ELF_MMB8T_EIEM2_1x1x1\z-mirror"

if not os.path.isdir(ELF_Z_MIRROR):
    print(f"Skipping: ELF reference data not found at {ELF_Z_MIRROR}")
    sys.exit(0)
...
print("=" * 70)
print("ELF vs Radia Z-Mirror Model Verification")
print("Matrix: Row-Major [target][source] format")
print("Image:  MIMA -Z (antisymmetric mirror on z=0 plane)")
print("Coil:   Racetrack (from coil_model.py)")
print("=" * 70)

# Load ELF geometry
nodes, elements = load_elf_geometry(ELF_Z_MIRROR)
n_elem = len(elements)
n_dof = n_elem * 6
print(f"\nGeometry: {n_elem} hexahedral elements, {n_dof} DOF (with z-mirror)")

# Load ELF matrix
elf_mat = load_elf_matrix(ELF_Z_MIRROR, n_dof)
...
# Build and get Radia matrix WITH image symmetry
# MIMA -Z = antisymmetric mirror on z=0 plane (sign = -1)
handle = rad.BuildMatrix(container, image='-z')
radia_mat, dof = rad.GetInteractMatrix(handle)
print(f"Radia matrix (with z-image): {radia_mat.shape}")
...
elf_B = load_elf_field_at_origin(ELF_Z_MIRROR)
...
# Solve WITH image symmetry (antisymmetric, sign=-1)
print("Solving with z-mirror image (antisymmetric)...")
result = rad.Solve(model, 0.0001, 100, 0, image='-z')
...
if matrix_pass and field_pass:
    ...
    print("- Image:  Z-mirror (MIMA -Z) working correctly")
