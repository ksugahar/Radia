#!/usr/bin/env python
"""
Verify ELF vs Radia for the x-mirror model (26 hexahedral elements + image).

Uses Image symmetry: MIMA X (mirror on x=0 plane)
...
if not os.path.isdir(ELF_X_MIRROR):
    print(f"Skipping: ELF reference data not found at {ELF_X_MIRROR}")
    sys.exit(0)
...
print("=" * 70)
print("ELF vs Radia X-Mirror Model Verification")
print("Matrix: Row-Major [target][source] format")
print("Image:  MIMA X (mirror on x=0 plane)")
...
n_elem = len(elements)
n_dof = n_elem * 6
print(f"\nGeometry: {n_elem} hexahedral elements, {n_dof} DOF (with x-mirror)")
...
print(f"Radia matrix (with x-image): {radia_mat.shape}")
...
elf_B = load_elf_field_at_origin(ELF_X_MIRROR)
...
print("Solving with x-mirror image...")
result = rad.Solve(model, 0.0001, 100, 0, image='+x')
...
if matrix_pass and field_pass:
    ...
    print("- Image:  X-mirror (MIMA X) working correctly")
