...
The C++ source lives in `src/ext/axifem/` and the top-level
...
This `examples/axifem/research/` directory is now a **research workspace**
(tests / scripts / demos) — it is not installable as a separate
package.  The `pyproject.toml`, `CMakeLists.txt`, and `src/` were
removed when axifem was absorbed into radia-core (2026-05-10
cleanup).
