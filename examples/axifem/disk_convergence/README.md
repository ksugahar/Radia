...
(`axifem_core.py`, `sigma_mass.py`) so no extra path setup is
needed; just `numpy`, `scipy`, and `ngsolve`/`netgen-mesher` for the
OCC mesh generation.

```bash
cd examples/axifem/disk_convergence
...
`examples/axifem/research/verification/test_hiruma_disk_q2.py` for the Q2
...
admittance.  Verifying that the axifem Henrotte FE recovers
...
`tests/axifem/test_python_reference_consistency.py` for the
...
`W:\30_CauerLadderNetwork\2026_04_01_長方形CLN\axifem`; promoted
