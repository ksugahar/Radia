...
def benchmark_tetrahedra(maxh, solver_type, output_dir, hmat_eps=1e-4,
                         bicg_tol=1e-4):
    """Benchmark tetrahedral mesh with linear material.
    ...
    print('=' * 70)
    print('TETRAHEDRAL MESH: maxh=%.2fm, solver=%s' % (maxh, solver_type))
    print('=' * 70)
    ...
    print('Generated %d tetrahedral elements (%d DOF)' % (n_elements, ndof))
...
def main():
    parser = argparse.ArgumentParser(description='Tetrahedral benchmark - linear material (Radia)')
    ...
    print('=' * 70)
    print('TETRAHEDRAL BENCHMARK - LINEAR MATERIAL (Radia)')
    print('=' * 70)
    ...
    for label, results in [('LU', results_lu), ('BiCGSTAB', results_bicgstab), ('HACApK', results_hacapk)]:
        if results:
            print('\n%s Solver:\n' % label)
            print('%-12s %10s %10s %12s %10s' % ('maxh', 'Elements', 'Time (s)', 'M_avg_z', 'Error %'))
            print('-' * 60)
            for r in results:
                print('%-12s %10d %10.4f %12.2f %10.2f' % (
                    r['mesh_description'], r['n_elements'], r['t_solve'], r['M_avg_z'], r['error_percent']))
