...
The example strengthens the build123d API by making volume/area measurements
explicit and JSON-friendly, then checks the same STEP geometry with Cubit's
headless Python API:

    build123d shape -> STEP -> Cubit import -> volume/area
...
def build_summary(out_dir: Path, cubit_bin_arg: str | None, require_cubit: bool, rtol: float) -> dict:
    ...
    return {
        "kind": "build123d_cubit_volume_area_cross_validation",
...
def main() -> int:
    ...
    print("[build123d -> Cubit volume/area cross validation]")
    print(
