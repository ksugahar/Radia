...
**Status (2026-05-24)**: **VERIFIED** as the source for IGTE Symposium
2026 digest (`W:\02_学会資料\2026年度\2026_09_IGTE_Symposium\ESIM@久保田\digest\`).
The IGTE paper's Fig. 1 heatmap is generated by `plot_sweep_heatmap.py`
from the sweep results documented below.

### Run command

```bash
python examples/ih_esim_benchmark/sweep_f_I.py C:/temp/igte_bench/sweep
python examples/ih_esim_benchmark/plot_sweep_heatmap.py C:/temp/igte_bench/sweep
```

32 cases = 4 currents `{1, 10, 100, 300} A` × 4 frequencies `{10, 50,
100, 500} kHz` × 2 modes `{scalar, per-element}`.  Runtime ~2 hours.
Workpiece: steel cylinder R=5 mm, H=10 mm, σ=2×10⁶ S/m, CEFC 2020 BH
benchmark; PEEC coil from `ih_fem_kelvin_demo_coil.step`.
...
in 15 iter at the high-drive cases (see `_NEW.pdf` for narrative).

### Headline sweep_v2 verified values
...
### Convergence summary (per-panel mode)

| Region | Iter range | Anderson restarts |
|---|---|---|
| `I=1, 10 A` (linear μ) | 6-10 | 0-1 |
| `I=100 A` (knee) | 7-12 | 0 |
| `I=300 A, f=10-100 kHz` | 11-29 | 0 |
| `I=300 A, f=500 kHz` | **30 (max), dZ≈0.04** | 0 |

31 of 32 cases converge; the lone outlier is the most demanding
(deepest saturation + thinnest skin).  This is the `~30 iter` figure
cited in the paper (NOT `~50 iter` — earlier draft value was over-
estimated and corrected 2026-05-24).
