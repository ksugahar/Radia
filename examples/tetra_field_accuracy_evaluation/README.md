...
produces **identical results** to hexahedral (ObjDivMag) elements when given
...
- **Reference**: Hexahedral mesh using ObjDivMag (analytical formula)
...
Compares tetrahedral (Netgen) and hexahedral (ObjDivMag) meshes with
...
3. **Both tetrahedral (Netgen) and hexahedral (ObjDivMag) are validated**
   - Tetrahedral: Use `netgen_mesh_import.netgen_mesh_to_radia()`
   - Hexahedral: Use `rad.ObjDivMag()` for simple geometries
