# -*- coding: utf-8 -*-
r"""
act7_21_lowfreq_openbc_4way.py  (Track A -- kelvin branch)
==========================================================
...
low-freq demos are pairwise (Kelvin vs BEM cost act4_01_kelvin_vs_bem_cost; Kelvin vs BEM accuracy
act4_02_kelvin_approximates_bem/act4_05_bem_kelvin_equivalence; PML radial conditioning act7_08_pml_lowfreq_dtn), and the 3-way act7_07_threeway_kelvin_pml_bemfem is HIGH
...
PROVEN HERE (all asserted; no overclaim, every 'ok' gated on a computed number):
  ...
  [B] CONDITIONING (frequency robustness): cond(PML) grows toward DC while cond(Kelvin)
      stays flat (reuse of the act7_08_pml_lowfreq_dtn radial model) -> the real low-freq PML cost.
  [C] COST / SPARSITY: the volume/shell closures (Kelvin, PML, ballooning) give SPARSE
      ...
      in a guarded block; absolute timings in act4_01_kelvin_vs_bem_cost.)
...
# ----- PML + conditioning helpers (copied verbatim from the verified act7_08_pml_lowfreq_dtn) -----
def _sph(kind, n, z):
...
print("=" * 78)
print(" act7_21_lowfreq_openbc_4way: low-frequency open boundary -- Kelvin vs BEM vs PML vs ballooning")
print("=" * 78)
...
# ===========================================================================
print("\n[C2] Real 3D NGSolve cost (guarded; absolute timings in act4_01_kelvin_vs_bem_cost):")
try:
    import time
