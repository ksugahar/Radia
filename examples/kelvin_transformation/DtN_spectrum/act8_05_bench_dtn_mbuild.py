"""act8_05_bench_dtn_mbuild -- Step 5 benchmark: build the material-aware transfer matrix M
...
Three honest contrasts (all measured in-repo, JSON + figure committed next to this script):
  ...
  C1  SPARSITY + FACTORISATION COST of the M-generator (the Kelvin-FEM volume matrix A), swept to
        ...
        at every size, the dense factor time is measured up to the cap).  This is act4_06_reducedfem_vs_bem_taxonomy's "sparse
  ...
  C2  ACCURACY vs the GREEN-FUNCTION ROUTE (concentric shell, where the layered-sphere Green operator
        ...
        pattern, exactly as act8_03_general_iron_design scenario_A) reproduces the analytic layered transfer (analytic_R)
...
Run:  pip install -e packages/radia-mcp ; python act8_05_bench_dtn_mbuild.py
...
import act8_03_general_iron_design as hh   # build_fem / analytic_R / coil_dofs / a, R_out, r_t (NOT _CN: bench self-measures c_n)
...
ACC_ORDER, ACC_MAXH = 2, 0.25                       # act8_03_general_iron_design scenario_A anchor config
...
def accuracy_case():
    ...
    (best-fit over the angular pattern, exactly as act8_03_general_iron_design scenario_A)."""
    ...
    with TaskManager():
        ...
        for n in (1, 2, 3):
            ...
            # solve -- so this benchmark is INDEPENDENT of act8_03_general_iron_design._CN and robust to any future
...
def main():
    print("=== act8_05_bench_dtn_mbuild : sparse Kelvin-FEM M-build vs dense Green-function route ===")
    ...
    jpath = os.path.join(HERE, "act8_05_bench_dtn_mbuild.json")
...
def _make_figure(results):
    ...
    try:
        ...
        info = save_lab_figure(fig, os.path.join(HERE, "act8_05_bench_dtn_mbuild"), tighten=False)
        print("figure -> %s" % info.get("pdf", os.path.join(HERE, "act8_05_bench_dtn_mbuild.pdf")))
