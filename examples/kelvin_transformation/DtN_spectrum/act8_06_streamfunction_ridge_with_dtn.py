"""act8_06_streamfunction_ridge_with_dtn -- wire the cheap DtN material-aware M into the EXISTING stream-function solver.

This closes the Track-B loop: the DtN/Kelvin-FEM supplies the material-aware transfer matrix M
(cheap to build -- see act8_05_bench_dtn_mbuild), and the *existing*, kernel-agnostic
...
formed matrix; a randomized-SVD oracle (act8_04_aca_tsvd_on_dtn_matrix) is the alternative when M is not formed.

Run:  pip install -e packages/radia-mcp ; python act8_06_streamfunction_ridge_with_dtn.py
...
import act8_03_general_iron_design as hh   # build_fem / coil_dofs / targets_on_sphere / build_transfer / fresh_forward
...
def main():
    print("=" * 96)
    print("act8_06_streamfunction_ridge_with_dtn -- DtN material-aware M  ->  radia.stream_function (ACA+)+TSVD + ridge design")
    print("=" * 96)
    with TaskManager():
