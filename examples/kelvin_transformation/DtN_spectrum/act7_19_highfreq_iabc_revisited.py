# -*- coding: utf-8 -*-
r"""
act7_19_highfreq_iabc_revisited.py  (Track A -- kelvin branch)
=============================================================
RE-EXAMINING the high-frequency IABC (user: "review the high-frequency IABC a bit more").
This CORRECTS act7_15_iabc_timedomain_cost, which modelled the high-frequency IABC with a single (essentially real)
...
no external data read):
  ...
  (C) THE TIME-DOMAIN OBSTRUCTION (the real finding): the optimal shell has Im(epsilon) and Im(mu)
      ...
      (act6_12_iabc_diffusion_timedomain) whose sqrt(s) SIBC is passive and Foster-realizable.

CORRECTION TO act7_15_iabc_timedomain_cost: act7_15_iabc_timedomain_cost's "reduced M-pole rational DtN = the time-domain IABC" is a clean
model reduction of the TARGET (exact) DtN -- a valid cost-efficient absorber -- but it is NOT the
IABC's own (narrowband, possibly non-passive) materials. act7_15_iabc_timedomain_cost's cost-accuracy datasheet stands as
...
print("=" * 78)
print(" act7_19_highfreq_iabc_revisited : RE-EXAMINING the high-frequency IABC (Mie metamaterial multilayer)")
print("=" * 78)
...
[interpretation] -- re-examination of the high-frequency IABC:
  * The ACTUAL high-freq IABC is a complex-(eps,mu) Mie metamaterial multilayer (2 polarizations,
    4 DOF/shell), per-frequency optimized -- NOT the single index of act7_15_iabc_timedomain_cost.
  ...
  * CONTRAST: the eddy-current/diffusion case (act6_12_iabc_diffusion_timedomain) has the PASSIVE sqrt(s) SIBC, which Foster-
    realizes cleanly; and at LOW frequency Kelvin is exact and parameter-free (act7_18_lowfreq_kelvin_vs_iabc). So the
    time-domain story is clean for diffusion and low-freq, but genuinely HARD (passivity) for the
    high-frequency wave IABC -- this corrects act7_15_iabc_timedomain_cost's optimistic reduced-pole picture.
""")
