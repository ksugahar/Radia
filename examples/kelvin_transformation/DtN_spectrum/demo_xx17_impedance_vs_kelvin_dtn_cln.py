# -*- coding: utf-8 -*-
r"""
demo_xx17_impedance_vs_kelvin_dtn_cln.py  (HEAD-TO-HEAD: impedance-boundary vs Kelvin -> CLN)
============================================================================================
...
scattered evidence -- demo_xx3 Kelvin->CLN, demo_yy Kelvin-vs-IABC, demo_zz IABC passivity
...
ladder in DIFFERENT variables:
  ...
  impedance-DtN-CLN : an impedance boundary is a passive lumped network (rational in s, the
                      ...
                      variable (the IABC, demo_vv).  The s-network hits the WARBURG WALL (a
                      finite RLC ladder cannot be sqrt(s)); the shell transformer needs N
                      parameters for N modes and ill-conditions ~1.5 decades/shell (demo_yy).
...
VERIFIED HERE (asserted; self-contained numpy/scipy):
  ...
  [3] SPATIAL (multi-mode) axis: Kelvin closes ALL modes with ZERO parameters (the conformal
      ...
      conditioning GROWS geometrically in N (reproduced) -- consistent with demo_yy's
...
real-axis DtN does not apply (HOIBC/IABC: demo_zz/wb/wc).

NON-CLAIM: for the SPHERE the EXACT exterior DtN is ONE operator; an exact-DtN-as-Robin
(Grote-Keller, demo_uu2) EQUALS the Kelvin DtN -> the SAME CLN.  The gap measured here is
...
# ---- reverse-Bessel theta_n + the exact eddy DtN (reuses demo_xx3's structure) ----
def theta_coeffs(n):
    c = np.zeros(n + 1)
    for k in range(n + 1):
...
# ---- IABC nested-shell amplitude basis (reuses demo_vv's 2x2 mode basis) ----
def iabc_basis(r, n):
...
print("=" * 78)
print(" demo_xx17 : impedance-boundary-DtN-CLN  vs  Kelvin-DtN-CLN  (same 4 axes)")
print("=" * 78)
...
print(f"    -> cond GROWS ~{dec_per_shell:.1f} decades/shell (cf. demo_yy measured ~1.5: 2.6e1->2.4e7)")
