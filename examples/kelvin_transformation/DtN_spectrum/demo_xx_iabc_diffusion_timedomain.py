# -*- coding: utf-8 -*-
r"""
demo_xx_iabc_diffusion_timedomain.py  (Track A -- kelvin branch)
===============================================================
...
demo_ww). This completes development-direction D's parabolic branch.

PHYSICS (the SA-correct setting): in a magneto-quasistatic (eddy-current) problem the EXTERIOR
AIR is non-conducting -> its field is instantaneously Laplace (the static ladder, demo_vv), NO
...
It INTERPOLATES the two known regimes:
   s -> 0   (DC / thick skin):   Lambda_n -> -(n+1)          (the STATIC multipole ladder, demo_vv)
   s -> inf (high freq / thin skin): Lambda_n -> -a*gamma = -a*sqrt(s*mu*sigma)  (the sqrt(s) SIBC).

KEY CONTRAST WITH THE WAVE CASE (demo_ww):
...
print("=" * 78)
print(" demo_xx : PARABOLIC (eddy-current/diffusion) TIME-DOMAIN IABC")
print("=" * 78)
...
print("    ok  (algebraic convergence, never ~1e-15 -- contrast demo_ww WAVE DtN: exact at M=n)")
...
print("""
    READING (cost model, structural -- not wall-clock):
      ...
      * KEY CONTRAST vs the WAVE case (demo_ww): the wave DtN is rational (finite n poles) -> EXACT
