...
# linear least-norm over psi (act8_10_current_sheet_design_matrix), we run a NONLINEAR fit over the path parameters, evaluating
# the field through act8_13_sf_contour_filament_fmm_source's already-verified pipeline:
#
...
#
# The same pipeline that turns SF contour loops into a material-aware field (act8_13_sf_contour_filament_fmm_source) is geometry-
...
#
# Narrative (mirrors act8_10_current_sheet_design_matrix): target = the iron-system field of a REFERENCE single loop (so it is
