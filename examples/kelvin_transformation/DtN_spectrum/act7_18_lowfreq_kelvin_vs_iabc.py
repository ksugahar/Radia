# -*- coding: utf-8 -*-
r"""
act7_18_lowfreq_kelvin_vs_iabc.py  (Track A -- kelvin branch)
============================================================
...
IABC nested isotropic shells. The DtN spectrum settles it:
  ...
  KELVIN  = an EXACT conformal compactification of the exterior -> its truncation DtN IS the exact
            ...
            optimization (established: Nabizadeh-Ramamoorthi-Chern 2021; well-conditioned, act7_08_pml_lowfreq_dtn;
            carries exterior material/scatterers, act7_11_kelvin_pml_merit_scatterer).
  IABC    = N isotropic shells whose permeabilities are OPTIMIZED to null only the first N modes;
            cannot be exact for all modes (the Liouville isotropy obstruction, act7_14_iabc_static_elegant); and the
...
VERIFIED HERE (all asserted):
  ...
  (C) the IABC AMPLITUDE-transfer condition number grows ~one order of magnitude PER SHELL
      ...
      continued-fraction recursion of act7_14_iabc_static_elegant stays well-conditioned -- it solves in float64 --
  ...
  (D) verdict: low frequency -> Kelvin (exact all modes + parameter-free + well-conditioned +
      ...
      eddy-current regime (act7_15_iabc_timedomain_cost / act6_12_iabc_diffusion_timedomain) where the extended-Kelvin needs the "uncool" centre-PML.
...
# --- IABC machinery (from act7_14_iabc_static_elegant; sphere, static) -------------------------
def basis(r, n):
...
print("=" * 78)
print(" act7_18_lowfreq_kelvin_vs_iabc : LOW FREQUENCY -> Kelvin is the one choice (vs IABC)")
print("=" * 78)
...
print("         needed; the act7_14_iabc_static_elegant scalar recursion avoids it, but Kelvin needs NO system at all)")
...
[D] VERDICT -- low frequency = KELVIN (one choice):
      ...
      conditioning        | well-conditioned (act7_08_pml_lowfreq_dtn)     | amplitude form ~10^N (Meeker 1e3 digits)
      exterior material   | carried exactly (act7_11_kelvin_pml_merit_scatterer)      | obstruction (isotropic shells)
