...
# operator that demo_x's Kelvin-FEM is isomorphic to (here for a genuine MULTILAYER stack, where the
# two-media image of demo_x no longer suffices). This is the open-math BEM kernel that NGSolve's ngbem
...
# For two half-spaces R(k)=r01 (const) and the integral collapses to the single image (demo_x). For a
...
print("   MULTILAYER reference for the Kelvin-FEM isomorphism (demo_x), in open numpy/scipy. The")
