# -*- coding: utf-8 -*-
r"""
demo_xx13_geometry_floor_law.py  (Kelvin mesh-adequacy: the GEOMETRY-FLOOR side)
================================================================================
The OTHER half of the mesh-adequacy criterion (demo_xx12 did the SOURCE side -- the
...
floor: the isoparametric order-k sphere approximating the true sphere.  floor_vs_curve
...
double-precision quadrature/round-off floor (~1e-6) instead -- this IS floor_vs_curve's
saturation.  Practical rule: k=2 (rate 4) already reaches ~1e-6 at modest h; higher k
buys nothing in double precision.

Together with demo_xx12 the total Kelvin error is
        err  ~  max( (d_max/R)^p  [source side, demo_xx12],  C*(h/R)^(2k)  [this] )
which is the abstract's "required surface resolution AND element order in closed form."

VERIFIED HERE (asserted; NGSolve sphere DtN, reuses floor_vs_curve's lam_err):
...
def lam_err(n, p, maxh, curve_k):
    """Relative error of the Kelvin DtN eigenvalue -(n+1)/R (reuses floor_vs_curve)."""
    with ng.TaskManager():
...
print("=" * 78)
print(" demo_xx13 : Kelvin GEOMETRY-FLOOR law  floor ~ (h/R)^(2k)  (energy superconv.)")
print("=" * 78)
...
print(f"  double-precision floor ~1e-6 (floor_vs_curve's saturation).  Practical: k=2 suffices.")
print(f"  With demo_xx12 the TOTAL error ~ max((d_max/R)^p, C(h/R)^(2k)) -- the required ELEMENT")
