...
#
# Findings (LAB, see bench_fmm_vs_aca_biotsavart.json / .png next to this file):
...
def main():
    ...
    out = dict(timestamp=datetime.now().isoformat(), hostname=platform.node(),
               ...
               results=rows)
    jpath = os.path.join(here, "bench_fmm_vs_aca_biotsavart.json")
    ...
    fig.tight_layout()
    ppath = os.path.join(here, "bench_fmm_vs_aca_biotsavart.png")
