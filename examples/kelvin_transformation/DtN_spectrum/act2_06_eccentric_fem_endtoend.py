# -*- coding: utf-8 -*-
r"""
act2_06_eccentric_fem_endtoend.py  (Kelvin mesh-adequacy: FEM end-to-end)
===========================================================================
act2_04_adequacy_eccentric_multibody derived the source-side criterion p* = ceil(ln eps / ln(d/R)) from the
...
act3_05_exterior_field_recovery's machinery) and compared to the analytic dipole -- confirming the recovered-field
...
The off-centre dipole's multipole content decays as ~(d/R)^(m-1) (act2_04_adequacy_eccentric_multibody), and the
...
VERIFIED HERE (asserted; NGSolve Kelvin solve + numpy):
  ...
  [2] the criterion p* = ceil(ln eps / ln(d/R)) (act2_04_adequacy_eccentric_multibody, derived from the TRACE
...
(act2_04_adequacy_eccentric_multibody); the exterior FIELD converges at ~(d/R)^(2p) (the extra radial suppression),
so p* is a safe upper bound for field accuracy, not tight for it.  3-D dipole content also
carries a mild polynomial-in-n prefactor on top of (d/R)^(m-1) (act2_04_adequacy_eccentric_multibody's exact tower
...
def solve_ball_g(g_cf, order, maxh=0.5):
    """Kelvin-ball solve with Dirichlet datum g on Gamma (reuses act3_05_exterior_field_recovery's solve_ball)."""
    with ng.TaskManager():
...
print("=" * 78)
print(" act2_06_eccentric_fem_endtoend : Kelvin FEM end-to-end -- off-centre dipole, recovered field err(p)")
print("=" * 78)
...
print("\n[verdict]")
print(f"  End-to-end FEM confirmation of act2_04_adequacy_eccentric_multibody's criterion: an off-centre dipole solved")
