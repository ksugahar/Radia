...
The audit classifies every production Python file under `src/radia` and fails
when a new module lacks a named MATLAB route. Current counts are generated by
`tools/check_matlab_python_parity.py`.

The MEX gateway currently exposes 334 commands and covers all 94 public
...
| Callback-driven harmonic balance | `radia.python.harmonicBalance` | Retain Python callable composition at batch boundaries; promote stable numeric kernels independently |
| Xsuite magnetic tracking bridge | `radia.python.xsuiteBridge` | Retain Xtrack particle identity in Python; use `radia.tracking` for MATLAB-native Lorentz tracking |
