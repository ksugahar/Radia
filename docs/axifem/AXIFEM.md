...
The Mathematica derivation lives in the repository at
`validation_test/maglev/research_cln/axifem/`
(`derive_quad_q2_henrotte.wls` and `quad_q2_henrotte_matrices.json`;
migrated 2026-05-12 from the historical external CLN working tree, which
no longer exists).
If the derivation JSON changes, regenerate `q2_henrotte_generated.hpp`, copy it
