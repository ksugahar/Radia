...
| Surface de Rham / HOIBC | `surface_derham.wls` | HOIBC has a topological part (surface harmonic 1-forms counted by `b1`) and a geometric part (local Pade/Laplace-Beltrami approximation of the analytic exterior DtN). | `surface_derham` |

The suite index is exposed as MCP topic `differential_geometry`.
