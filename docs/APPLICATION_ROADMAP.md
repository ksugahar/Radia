...
| Field on trajectory | OK | `beam_tracking.py` |
| Particle tracking | Partial | `beam_tracking.py` |
...
**Examples**: `examples/beam_tracking/` (to be created)

**GPU Acceleration (jhpcn申請中)**:
...
- **Space charge**: N-body with FMM on GPU (ExaFMM-T)
