...
|---|---|---|
| Resolved (2026-09-07) | Full-versus-IMA `rad.Fld` roundoff | The curved TET BDM2 check passes at 10 eps on the pair-domain rule.  The two HEX checks are bounded by the numerical near rule, not by symmetrization: their Gram energies agree to 10 eps, the near blocks now use Neumaier-compensated accumulation (order-independent to a few eps), and the remaining field gap is the `sqrt(n_points) eps` per-term rounding of a 6^6-point rule evaluated through reflected node orders (2.2e-14 single cell, 5.7e-14 multi-cell).  The HEX field contract is locked at 1.2e-13 with that derivation in the test; TET keeps 10 eps. |
