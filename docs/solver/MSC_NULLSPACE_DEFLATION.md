...
> **Status (2026-06-09): the RUNTIME loop-mode handling once described here
...
> from the solver.** The project consolidated on the **HDiv-VIM** operator
> (`radia.hdiv_vim`): its charge map `B` makes the loop space exactly `ker(B)`,
...
> analysis below remain valid -- they are precisely *why* HDiv-VIM is the right
> consolidation -- but the API-level "how to remove the modes at runtime"
