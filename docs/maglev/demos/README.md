...
|-----------------|-------|----------|
| `cube_alpha_sweep.py` | Mixed-Galerkin `alpha(s)` sweep (CAD-direct edges vs mesh-derived), the headline package API demo: the projected `BoxMixedGalerkin` (bulk modes + surface envelope in one Galerkin space, exact at DC) beside the additive `Y_mixed` (the LTI-export form) | Cu cube |
| `cube_alpha_sweep_figure.py` | Paper figure: `alpha(s)/V` frequency response (Re exclusion + \|Im\| loss) from 1 Hz to 1 GHz, projected form as the main curves and the additive tail in grey where `a/delta >= 1`; IEEE single-column; emits `cube_alpha_sweep_results.json` + `.pdf`/`.png` | Cu cube |
