...
def create_representative_eis_csv(output_dir: str):
    ...
    # Save with header
    header = """# SYNTHETIC 18650 Li-ion Battery EIS Demonstration
# Source: Cole-Cole plus Warburg model in create_representative_eis_csv
# Dataset: Li-ion Battery Aging Datasets (B0005-B0018)
# URL: https://data.nasa.gov/dataset/li-ion-battery-aging-datasets
# SYNTHETIC DATA - Not extracted from NASA measurements
#
# Reference:
#   B. Saha and K. Goebel (2007). "Battery Data Set", NASA Ames
...
"""
    ...
    output_file = os.path.join(output_dir, "synthetic_18650_eis.csv")
...
def create_readme(output_dir: str):
    """Write synthetic-data guidance without replacing the measured-data README."""
    text = """# Synthetic 18650 EIS demonstration

`synthetic_18650_eis.csv` is generated by a Cole-Cole plus Warburg model.
It is not extracted from NASA measurements and must not be used as independent
experimental validation. Parameters and frequency sampling are model inputs.
Measured-data extraction is a separate workflow in `extract_real_eis.py`;
that workflow still requires verification of source files and frequency mapping.
"""
    with open(os.path.join(output_dir, "SYNTHETIC_DATA.md"), "w", encoding="utf-8") as stream:
        stream.write(text)
...
if __name__ == '__main__':
    ...
    # Do not overwrite the measured-data README with synthetic-data guidance.
    create_readme(script_dir)

    print("\n" + "="*60)
    print("Synthetic battery EIS demonstration prepared; not measurement validation.")
    print("="*60)
