...
\item Run 5--8 independent training runs (5000--8000 epochs each)
...
URN determines the optimal stage count based on (1) basis function type, (2) frequency range coverage, and (3) target approximation error. Table~\ref{tab:stages} summarizes typical stage requirements.

\begin{table}[!t]
\caption{Ladder Stage Count by Basis Function Type}
\label{tab:stages}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
...
Cole-Cole & 5--7 & Charef: $\sim$1.5/decade \\
CPE/Warburg & 5--10 & Valsa: 1/decade \\
...
\end{equation}
where $k$ is the number of stages and $n$ is the number of frequency points. The algorithm evaluates $k = 2, 3, \ldots, 12$ and selects the stage count minimizing AIC, balancing accuracy against model complexity.
...
\textbf{Open-source availability}: PyTorch implementation (700 lines), CPU-only, training time 100--150 seconds per case. No GPU required.
