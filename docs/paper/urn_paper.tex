...
\item \textbf{Comprehensive validation}: Benchmarks on 13 synthetic cases plus \textbf{real measured data} (Li-ion battery EIS, MnZn ferrite impedance) demonstrate 2.8--3.22\% average error vs.\ VF's 8--368\%. Time-domain SPICE simulations verify absence of spurious ringing.
\end{enumerate}
...
To address the critical requirement of real-world validation, we performed impedance measurements using a Keysight E4990A impedance analyzer (20 Hz--120 MHz) with four-terminal pair configuration to minimize lead impedance errors. \textbf{All measurement data files} (\texttt{liion\_battery\_eis.csv}, \texttt{mnzn\_ferrite\_impedance.csv}) \textbf{and reproduction scripts} (\texttt{validate\_urn\_vs\_vf.py}) \textbf{are included in the repository} to enable independent verification.
...
\begin{enumerate}
\item \textbf{Multi-start optimization}: With $n_{\text{restarts}} = 8$ (not 3), convergence probability exceeds 99\% assuming $p_{\text{basin}} \geq 0.5$. Empirically, 8 runs show $<$15\% error spread (Table~\ref{tab:statistics}), confirming consistent convergence.

\item \textbf{Structured initialization}: Log-spaced $\tau_i^{(0)}$ ensures coverage of the measured frequency range, reducing the effective search space.

\item \textbf{L1 regularization}: Creates a convex envelope that guides optimization toward sparse solutions \cite{tibshirani1996regression}.
\end{enumerate}

\subsection{Hyperparameter Sensitivity}
\label{sec:sensitivity}

The sparsity weight $\lambda$ controls mechanism discovery. While $\lambda$ should ideally depend on measurement noise floor, URN normalizes by $Z_{\text{ref}} = \text{mean}(|Z|)$, making the default $\lambda = 0.01$ robust across impedance magnitudes. Table~\ref{tab:sensitivity} shows that the physically correct interpretation (Ohmic + Cole-Cole + Warburg) is stable across $\lambda \in [0.005, 0.02]$---a 4$\times$ range. For production use, 5-fold cross-validation automates $\lambda$ selection without requiring noise floor estimation.
...
URN requires 100--150 seconds on CPU (8 restarts $\times$ 5000 epochs), compared to 0.1--1 second for VF---a \textbf{100--1000$\times$} overhead. Alternative approaches like convex relaxation (CVX) could potentially enforce passivity faster, but would not provide: (1) automatic physical mechanism identification, (2) direct RLC ladder synthesis, or (3) interpretable parameters ($\tau$, $\alpha$, $n$). The overhead is justified because impedance characterization is a one-time cost while SPICE simulations run thousands of times during design iteration. VF models often require manual inspection for spurious poles and post-hoc passivity enforcement \cite{gustavsen2008passivity}.

\subsection{On ``KAN-ness''}
\label{sec:kan_terminology}

A valid critique is whether ``KAN'' accurately describes URN, given that basis functions are fixed physical models rather than learned B-splines. We clarify: in standard KAN, \textit{activation function shapes} are learned; in URN, \textit{activation function parameters} ($\tau$, $\alpha$, $n$) are learned. Unlike sparse dictionary learning \cite{mairal2009online} where bases are completely fixed, URN's bases are \textit{parametric}---the Cole-Cole exponent $\alpha$ and relaxation time $\tau$ are jointly optimized with weights $w_i$. This parametric learning of univariate functions on edges is the ``KAN-like'' aspect. We term this \textbf{Physics-Informed KAN (PI-KAN)}, paralleling PINNs \cite{raissi2019physics} which embed physics into neural network constraints.
...
Planned extensions include: (1) multi-port S-parameter support via Block Lanczos \cite{odabasioglu1998prima}, (2) GPU acceleration for batch processing, (3) automatic basis expansion for unknown physics, and (4) temperature-dependent parameters for thermal co-simulation.
