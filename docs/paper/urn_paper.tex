...
The optimization landscape for time constant parameters $\tau_i$ in (\ref{eq:optimization}) is non-convex. Unlike Vector Fitting, which guarantees global convergence for fixed poles via linear least squares, URN relies on gradient-based optimization. We analyze convergence behavior through both theoretical arguments and empirical validation.

\subsubsection{Multi-Start Strategy Justification}
URN uses $n_{\text{restarts}} = 8$ independent optimization runs with random initialization. The probability of missing the global minimum decreases exponentially with the number of restarts:
\begin{equation}
P(\text{miss}) = (1 - p_{\text{basin}})^{n_{\text{restarts}}}
\end{equation}
where $p_{\text{basin}}$ is the probability of initializing within the attraction basin of the global minimum.

Empirical analysis across 13 benchmark cases shows that different random starts converge to solutions with relative error spread of $<$15\% (Table~\ref{tab:statistics}). The low variance indicates that:
\begin{enumerate}
\item The loss landscape has a dominant basin containing near-optimal solutions
\item Multiple local minima, if they exist, have similar objective values
\item The L1 regularization term creates a convex envelope that guides optimization toward sparse solutions
\end{enumerate}
...
\begin{equation}
\tau_i^{(0)} = 10^{\log_{10}(\omega_{\min}^{-1}) + i \cdot \Delta}, \quad \Delta = \frac{\log_{10}(\omega_{\max}^{-1}/\omega_{\min}^{-1})}{M-1}
\end{equation}
...
\subsubsection{Convergence Guarantee via Regularization}
The L1 regularization term $\lambda \|\mathbf{w}\|_1$ ensures that the optimization problem has a unique minimum when $\lambda > 0$ \cite{tibshirani1996regression}. While the overall problem remains non-convex due to the nonlinear basis functions, the regularization prevents pathological cases where multiple basis functions with different $\tau$ values produce identical fits.

\subsection{Hyperparameter Sensitivity Analysis}
\label{sec:sensitivity}

The primary hyperparameter in URN is the sparsity weight $\lambda$ (denoted \texttt{sparsity\_weight} in the implementation, default $\lambda = 0.01$). We analyze sensitivity through systematic variation.

\subsubsection{Sparsity Weight Sweep}
Table~\ref{tab:sensitivity} shows fitting error and discovered mechanisms for the Li-ion battery case across a range of $\lambda$ values.
...
The physically correct interpretation (Ohmic + Cole-Cole + Warburg) is stable across $\lambda \in [0.01, 0.02]$. Smaller $\lambda$ over-fits with redundant bases; larger $\lambda$ under-fits by eliminating the Warburg tail. This demonstrates that the discovered physics is robust to moderate hyperparameter variation.

\subsubsection{Cross-Validation Strategy}
For production use, we recommend $k$-fold cross-validation with $k = 5$:
\begin{enumerate}
\item Split frequency points into 5 folds
\item Train URN on 4 folds, evaluate on held-out fold
\item Select $\lambda$ minimizing average validation error
\end{enumerate}
...
\begin{itemize}
\item \textbf{Vector Fitting}: $O(N_p \cdot K)$ per iteration, where $N_p$ is pole count and $K$ is frequency points. Typically converges in 5--10 iterations. Total: 0.1--1 second.
\item \textbf{URN}: $O(M \cdot K \cdot E)$, where $M = 29$ bases and $E = 5000$ epochs. With 8 restarts: $\approx$100--150 seconds on CPU.
\end{itemize}

URN is approximately \textbf{100--1000$\times$ slower} than VF for a single fit.

\subsubsection{Cost-Benefit Analysis}
The computational overhead is justified by:
\begin{enumerate}
\item \textbf{One-time cost}: Impedance characterization is performed once per component; SPICE simulation runs thousands of times during design iteration.
\item \textbf{Avoided rework}: VF models with spurious poles require manual inspection and re-fitting. URN's guaranteed stability eliminates this iteration loop.
\item \textbf{Physical insight}: Automatic mechanism discovery provides engineering value beyond fitting accuracy.
\item \textbf{Passivity guarantee}: VF may require post-hoc passivity enforcement \cite{gustavsen2008passivity}, adding complexity and potential accuracy loss.
\end{enumerate}
...
\subsection{On ``KAN-ness'': Terminology Clarification}
\label{sec:kan_terminology}

A natural question arises: \textit{Is URN truly a ``KAN'' if the basis functions are fixed physical models rather than learned splines?}

We clarify the relationship between URN and KAN along two dimensions:

\subsubsection{Shared Philosophy}
URN adopts KAN's core insight: \textbf{learnable univariate functions on edges} rather than fixed activations on nodes. In standard KAN, these are B-splines with learnable knots. In URN, they are physics-based functions with learnable parameters ($\tau$, $\alpha$, $n$).

The key innovation of KAN---that the \textit{form} of the activation function should be learned from data---is preserved. URN simply constrains this learning to circuit-compatible forms.

\subsubsection{Distinction from Sparse Dictionary Learning}
Sparse dictionary learning \cite{mairal2009online} also uses fixed basis functions with sparse weights. The distinction is:
\begin{itemize}
\item \textbf{Dictionary learning}: Bases are fixed; only weights are learned
\item \textbf{URN}: Both basis parameters ($\tau_i$, $\alpha_i$) \textit{and} weights ($w_i$) are jointly learned
\end{itemize}

This joint optimization over continuous parameters is the ``KAN-like'' aspect. URN learns \textit{which relaxation times exist} and \textit{what exponents describe the distribution}---not just \textit{how much of each pre-defined basis to use}.

\subsubsection{Physics-Informed KAN}
We propose the term \textbf{Physics-Informed KAN (PI-KAN)} to describe architectures like URN that:
\begin{enumerate}
\item Use learnable basis functions (KAN philosophy)
\item Constrain the function space to physically meaningful forms (physics-informed)
\item Guarantee output realizability (e.g., circuit compatibility)
\end{enumerate}

This parallels the distinction between Physics-Informed Neural Networks (PINNs) \cite{raissi2019physics} and standard neural networks. Just as PINNs embed physical laws as constraints, PI-KAN embeds physical basis functions as the learnable components.
