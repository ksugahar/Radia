...
The optimization landscape for $\tau_i$ in (\ref{eq:optimization}) is non-convex. URN addresses this through:

\begin{enumerate}
\item \textbf{Multi-start optimization}: 8 independent runs with $P(\text{miss}) = (1 - p_{\text{basin}})^8$. Empirical analysis shows $<$15\% error spread across runs (Table~\ref{tab:statistics}).

\item \textbf{Structured initialization}: Log-spaced $\tau_i^{(0)}$ covering the measured frequency range reduces search space dimension.

\item \textbf{L1 regularization}: Ensures unique minimum and prevents redundant bases \cite{tibshirani1996regression}.
\end{enumerate}

\subsection{Hyperparameter Sensitivity}
\label{sec:sensitivity}

Table~\ref{tab:sensitivity} shows sparsity weight $\lambda$ sensitivity for the Li-ion battery case. The physically correct interpretation (Ohmic + Cole-Cole + Warburg) is stable across $\lambda \in [0.01, 0.02]$. For production use, 5-fold cross-validation automates $\lambda$ selection.
...
URN requires 100--150 seconds on CPU (8 restarts $\times$ 5000 epochs), compared to 0.1--1 second for VF. This \textbf{100--1000$\times$} overhead is justified because: (1) impedance characterization is a one-time cost while SPICE runs thousands of times, (2) VF models may require manual re-fitting due to spurious poles, and (3) VF may need post-hoc passivity enforcement \cite{gustavsen2008passivity}. Memory usage is negligible ($<$1 MB).

\subsection{On ``KAN-ness''}
\label{sec:kan_terminology}

Unlike sparse dictionary learning \cite{mairal2009online} where only weights are learned, URN jointly optimizes both basis parameters ($\tau_i$, $\alpha_i$) \textit{and} weights ($w_i$). This preserves KAN's core insight---learnable univariate functions---while constraining to circuit-compatible forms. We term this \textbf{Physics-Informed KAN (PI-KAN)}, paralleling the distinction between PINNs \cite{raissi2019physics} and standard neural networks.
