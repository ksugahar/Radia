...
\subsection{Automatic Stage Determination}

For each selected basis function, URN determines the optimal ladder stage count using the Akaike Information Criterion (AIC) \cite{akaike1974new}:
\begin{equation}
\text{AIC} = 2k + n \cdot \ln(\text{MSE})
\end{equation}
where $k$ is the number of stages (model complexity) and $n$ is the number of frequency points.
